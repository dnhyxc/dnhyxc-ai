...
| `menu` | 可选。有 `menu` 才出现侧栏图标；`icon` **推荐 SVG 图片 URL**（`https://…` / `/ext-cos/…` / `/remotes/…`），Host 用 `PluginIcon` 内联渲染；也可写 Lucide 名（仅当 Host 侧栏 `ICON_MAP` 命中）。`order` 控制排序。详见 [implements-guide/09](../implements-guide/插件图标.md) |
...
> **Vue 子应用**：不要套本章 React Context；用 **vue-router + 嵌入 `createMemoryHistory` / 预览 `createWebHistory`**，完整示例见 [09 §5](./Vue插件开发.md)（参考 `remote-vue-shadcn`）。
