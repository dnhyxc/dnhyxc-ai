...
> **非目标**：不重写 kit 内核（见 `React宿主视图.md` 等）；不整文件复述 Header/Sidebar/ebook（只引用调用点）。
...
| F12 | 动态插件 SVG 图标 | 侧栏/Surface 自定义图标跟选中色 | `host/PluginIcon*` | [09](./插件图标.md) |
...
| 9b | `host/PluginIcon.tsx` + `pluginIconUrl.ts` | 动态 SVG 图标 | 见 [09](./插件图标.md) |
| 10 | `index.ts` | 再导出 | 全部 |
...
> **图标专章**：`PluginIcon` / `pluginIconUrl` 的逐行实现见 [插件图标.md](./插件图标.md)。下文已去掉已废弃的 `DEFAULT_PLUGIN_HOST_ICONS` 白名单。
...
- 图标加载/消毒/动画完整实现见 [插件图标.md](./插件图标.md)。
...
kit 内核、`PluginHostView` 细节见 `React宿主视图.md`；样式隔离内部、bridge 协议见同目录其他篇；插件开发指南见 `apps/frontend/src/federation/docs/`。
