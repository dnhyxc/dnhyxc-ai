...
> **前置阅读**：[01 总览](./架构概览.md)；细节与逐行代码见 [02](./运行时与桥接.md)–[05](./宿主适配层.md)。  
...
| 若忘记… | 会出现… | 对照 |
|---------|----------|------|
| 双入口共用默认 Host / 捕获栈 | 样式偶发不隔离或 release 错乱 | [03 F9](./样式隔离实现.md) |
| 挂载长窗仍 `claimUnmarked=true` | Host Markdown/Toast 被 scope | [03 F2](./样式隔离实现.md) |
| portal z-index 高于 Toast | Toast 不能悬停/关不掉观感异常 | [03 F8](./样式隔离实现.md) |
| App 根无 `data-mf-host-portal` | Host portal 被收进插件域 | [05](./宿主适配层.md) |
| Bridge 不 `deepFreeze` | 插件可篡改 Host 函数 | [02 F15](./运行时与桥接.md) |
| 业务直连 kit 包名 | 换适配困难、双实例风险 | [05](./宿主适配层.md) |
...
| 复刻阶段 | 精读 |
|----------|------|
| Phase A–B | [02](./运行时与桥接.md) F1–F7、F13–F14 |
| Phase C | [04](./React宿主视图.md) |
| Phase D | [05](./宿主适配层.md) + [02](./运行时与桥接.md) F15 |
| Phase E | [03](./样式隔离实现.md) 全文 |
| Phase F | [02](./运行时与桥接.md) F10/F16、[04](./React宿主视图.md) F6/F8 |
