...
> **分册**：[README](./README.md) · [07 方法字典](./API方法参考.md) · [02 Runtime](./运行时与桥接.md) · [03 样式隔离](./样式隔离实现.md) · [04 React](./React宿主视图.md) · [05 适配层](./宿主适配层.md) · [06 复刻](./复刻方案.md) · [09 插件图标](./插件图标.md)
...
| 序 | 路径 | 职责 | 详解 |
|----|------|------|------|
| 1 | `federation-kit/src/types/*`、`config/types.ts` | 契约 | [02](./运行时与桥接.md) |
| 2 | `host-api/*`、`enabled/*`、`registry/cache.ts` | 总线/偏好/缓存 | [02](./运行时与桥接.md) |
| 3 | `mf/*`、`runtime/*`、`bridge/*`、`inject/*` | 加载与注入 | [02](./运行时与桥接.md) |
| 4 | `createFederation.ts`、`index.ts` | 门面 | [02](./运行时与桥接.md) |
| 5 | `style-isolation/**` | CSS + Portal | [03](./样式隔离实现.md) |
| 6 | `react/**` | 声明式挂载 | [04](./React宿主视图.md) |
| 7 | `apps/frontend/src/federation/**` | 本仓接线 | [05](./宿主适配层.md) |
| 8 | `federation/host/PluginIcon*` + Sidebar / Surface | 动态插件 SVG 图标 | [09](./插件图标.md) |

### 0.5 全库功能点索引（跨分册）

| 编号 | 简述 | 分册 |
|------|------|------|
| — | **查某个方法的作用/目的/调用链** | **[07 方法字典](./API方法参考.md)** |
| R1–R19 | createFederation、加载、Bridge、注入器等 | [02](./运行时与桥接.md) F1–F19 |
| S1–S12 | realm、claimUnmarked、head/CSSOM、Portal、z-index | [03](./样式隔离实现.md) F1–F12 |
| V1–V10 | FederationPlugin、slots、iframe、hooks | [04](./React宿主视图.md) F1–F10 |
| A1–A11 | `@/federation`、Surface、ebook、全屏 | [05](./宿主适配层.md) F1–F11 |
| I1–I7 | 动态插件 SVG 图标：URL / 消毒 / 内联 / 上传 / CSS | [09](./插件图标.md) |
...
|------|-------------|------|------------|------|
| P1 | Remote CSS 污染 Host | 共享 document | 选择器前缀 + realm | [03](./样式隔离实现.md) |
| P2 | EP/antd 弹层丢样式或点不到 | Teleport 到 body | portal scope + stamp realm | [03](./样式隔离实现.md) |
| P3 | Markdown / Toast 样式或交互坏 | 挂载长窗误认领 Host CSS；portal 盖住 Toast | `claimUnmarked=false`；portal z-index 低于 Toaster | [03](./样式隔离实现.md) |
| P4 | `.` 与 `./react` 双份单例 | tsup 双入口 | `globalThis` 共享栈/patch/默认 Host | [02](./运行时与桥接.md) [03](./样式隔离实现.md) |
| P5 | 插件乱调 Host API | 直接传可变对象 | permissions ∩ capabilities + `deepFreeze` | [02](./运行时与桥接.md) |
| P6 | 不受信代码同域执行 | 一律 MF | `trust: untrusted` + iframe RPC | [02](./运行时与桥接.md) [04](./React宿主视图.md) |
| P7 | 刷新闪 404 | 路由注入异步 | Host `pluginsReady` 占位（适配/路由层） | [05](./宿主适配层.md) |
| P8 | registry 断网空白 | 只信网络 | localStorage/缓存回退 | [02](./运行时与桥接.md) |
| P9 | Vue Remote 无法挂 React Host | 框架不同 | `normalizePluginModule` + `createVueHostBridge` | [02](./运行时与桥接.md) |
| P10 | 业务到处依赖 kit 包名 | 耦合 | 约定只从 `@/federation` 导入 | [05](./宿主适配层.md) |
...
| 你想搞清… | 打开 |
|-----------|------|
| **某个方法干什么、为啥存在** | **[07 方法字典](./API方法参考.md)**（`ensurePlugin` / `runLoad` / `resolvePluginBust`…） |
| `start` 之后谁调谁 | [02 §3–§4](./运行时与桥接.md) |
| `verify` / bust / Vue 归一 | [02 F8–F10](./运行时与桥接.md) / [07 §1–§2](./API方法参考.md) |
| Bridge 权限与 iframe RPC | [02 F15–F16](./运行时与桥接.md) / [07 §4](./API方法参考.md) |
| 为何 Markdown 曾坏、如何修 | [03 F2/F5/F8](./样式隔离实现.md) |
| Portal 样式每一行含义 | [03 F7–F8](./样式隔离实现.md) + scopeDom |
| `<FederationPlugin />` | [04](./React宿主视图.md) |
| ebook 抽屉插件怎么挂 | [05 F6/F10](./宿主适配层.md) |
| 换项目怎么从零搭 | [06](./复刻方案.md) |
...
| T8 | registry `menu.icon`/`host.icon` 为 SVG URL | 侧栏与 Surface 触发器出自定义图标；选中色对齐 | [09](./插件图标.md) |

更细验收见 [06](./复刻方案.md)、图标专章 [09](./插件图标.md)。
...
| 2026-08-11 | 增补 [09 插件图标](./插件图标.md) 与功能点 I1–I7 / 验收 T8 |
