...
- **位置**：`packages/federation-kit/src/mf/normalizePluginModule.ts` → `normalizePluginModule / isVueRemoteModule`（全文）
- **说明**：下列为磁盘源码全文；每一行可执行/可配置代码的上方均有中文意图注释。
...
// 内部函数 `looksLikeVueMount`：收拢可复用逻辑，避免调用处复制粘贴
function looksLikeVueMount(comp: unknown): boolean {
	// 把结果返回给调用方：(
	return (
		// 条件表达式续行：与上一行组成完整判断
		!!comp &&
		// 赋值更新 `typeof comp`，让后续逻辑看到最新状态
		typeof comp === 'object' &&
		// 赋值更新 `typeof (comp as { mount?: unknown }).mount`，让后续逻辑看到最新状态
...
// 导出函数 `isVueRemoteModule`：本模块对外可直接调用的能力入口
...
// 结束当前字面量、参数列表或语句，回到外层继续
	...
	// 若满足条件则进入本分支：(meta.framework === 'vue') return true;
	if (meta.framework === 'vue') return true;
	// 若满足条件则进入本分支：(meta.framework === 'react') return false;
	if (meta.framework === 'react') return false;
	// 声明 `tag`，承接本段计算/配置结果供后续使用
	const tag = raw.framework ?? raw.mfFramework;
	// 若满足条件则进入本分支：(tag === 'vue') return true;
	if (tag === 'vue') return true;
	// 若满足条件则进入本分支：(tag === 'react') return false;
	if (tag === 'react') return false;
	// 把结果返回给调用方：looksLikeVueMount(raw.default);
 ...
 */
// 导出函数 `normalizePluginModule`：本模块对外可直接调用的能力入口
...
// 结束当前字面量、参数列表或语句，回到外层继续
	...
	// 若满足条件则进入本分支：(isVueRemoteModule(raw, meta))
