...
`host.icon` 推荐存 **SVG 图片 URL**，由 `PluginIcon` 动态内联（**不必**再维护 Lucide 白名单）。完整实现见 [implements-guide/插件图标.md](../implements-guide/插件图标.md)。
...
> 下一步：[桥接权限.md](./桥接权限.md) 插件能拿到什么（HostBridge + permissions + capabilities）。
