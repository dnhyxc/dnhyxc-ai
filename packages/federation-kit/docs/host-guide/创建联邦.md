...
**本仓两者都用过**：最简演示用 `registryUrl`；真实项目用 `fetchRegistry: fetchPluginRegistry`（本仓自定义实现，接 COS + 平台 fetch + 缓存，见 [08](./上架偏好实现.md)）。
...
缺省实现：`localStorage` 存 `{ [storagePrefix].enabled.v1: { [id]: true } }`。本仓把它换成账号服务（[08](./上架偏好实现.md)）。
...
> 为什么需要重建 router？React Router 的路由表是构建期的；插件路由是运行期注入的。宿主通过「epoch 递增 → `useMemo` 重建 `createBrowserRouter`」把动态路由并入路由表。详见 [05](./路由注入.md)。
...
> 下一步：[路由注入.md](./路由注入.md) 启动 + 动态路由注入 + 防闪 404 + 侧栏。
