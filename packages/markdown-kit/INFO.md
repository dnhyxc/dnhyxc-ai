...
| Mermaid 占位 + 浏览器渲染 | `enableMermaid`（默认开）+ **`@dnhyxc-ai/markdown-kit/react`**；流式聊天拆块示例见 **README §4.6**，架构说明见 **`docs/tools.md` §11.9**。 |
...
### 源码目录（`packages/markdown-kit/src/`）
...
| `index.ts` | 主入口：聚合导出（对外路径不变，仍从 `@dnhyxc-ai/markdown-kit` import） |
...
| 子路径 | 用途 |
| ------ | ---- |
| `@dnhyxc-ai/markdown-kit` | 主入口：`MarkdownParser`、主题 API、样式元数据等。 |
| `@dnhyxc-ai/markdown-kit/styles` | `highlightJsThemes` / `highlightJsThemeIds` 等运行时元数据 + 类型。 |
| `@dnhyxc-ai/markdown-kit/styles.css` / `markdown-styles.css` | 合并默认样式（含 github-markdown + katex + 默认 hljs 等）。 |
| `@dnhyxc-ai/markdown-kit/markdown-base.css` | 正文 + KaTeX，**不含**完整 hljs（可再自选主题 CSS）。 |
| `@dnhyxc-ai/markdown-kit/styles/hljs/<主题>.min.css` | 单个 highlight.js 主题文件。 |
| `@dnhyxc-ai/markdown-kit/react` | `useMermaidInMarkdownRoot`、`runMermaidInMarkdownRoot` 及类型（peer：`react`）。 |
...
| `codeBlockTabSize` | 代码块缩进控制：将 `\t`（Tab，制表符）展开为空格；默认 `2`。传 `0` 可保留 Tab。 |
...
| `enableMermaid` | 默认 `true`：` ```mermaid ` 输出占位 DOM；`false` 则按普通代码块。与 **`@dnhyxc-ai/markdown-kit/react`** 配合渲染 SVG。 |
...
// 得到如 @dnhyxc-ai/markdown-kit/styles/hljs/atom-one-dark.min.css
...
从 **`import { highlightJsThemes, highlightJsThemeIds, defaultHighlightJsThemeId, styles, styleUrls, styleContents } from '@dnhyxc-ai/markdown-kit'`** 或 **`@dnhyxc-ai/markdown-kit/styles`** 读取主题 id 列表、相对路径映射及少量内联 CSS 字符串（**不含**全部主题内联）。

---

## 3. `@dnhyxc-ai/markdown-kit/react`（Mermaid）
...
依赖：**`mermaid`** 已由本包 **`dependencies`** 声明；打包器需能解析 `import 'mermaid'`（Vite 可将 `@dnhyxc-ai/markdown-kit/react` 与 `mermaid` 列入 `optimizeDeps.include`）。
...
- 修改 **`markdown/parser.ts`**、主题注入等：执行 **`pnpm --filter @dnhyxc-ai/markdown-kit run build`**。
