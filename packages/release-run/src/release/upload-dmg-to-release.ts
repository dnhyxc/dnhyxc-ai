...
import { DMG_BUNDLE_DIR } from '../lib/paths.ts';
import {
	getReleaseId,
 ...
 * 1. 命令行第一个参数
 * 2. 环境变量 DMG_PATH
 * 3. DMG_BUNDLE_DIR 下按修改时间最新的 *.dmg
 */
function resolveDmgPath(cliArg?: string): string {
	...
	if (!fs.existsSync(DMG_BUNDLE_DIR)) {
		console.error(`❌ 默认 dmg 目录不存在: ${DMG_BUNDLE_DIR}`);
		console.error(
	...
	const entries = fs.readdirSync(DMG_BUNDLE_DIR);
	const dmgs = entries
		.filter((name) => name.toLowerCase().endsWith('.dmg'))
		.map((name) => path.join(DMG_BUNDLE_DIR, name));

	if (dmgs.length === 0) {
		console.error(`❌ 目录内未找到 .dmg 文件: ${DMG_BUNDLE_DIR}`);
		process.exit(1);
	}
