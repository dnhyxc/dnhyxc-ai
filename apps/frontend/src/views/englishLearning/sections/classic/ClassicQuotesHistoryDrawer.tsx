...
export type ClassicQuotesHistoryDrawerProps = {
	...
	entries: EnglishClassicQuoteHistoryEntry[];
	loading: boolean;
	loadingMore: boolean;
...
function ClassicQuotesHistoryDrawerInner({
	open,
	onOpenChange,
	entries,
	loading,
					...
					<div className="flex min-h-0 w-full flex-1 flex-col gap-2">
						{showInitialLoading ? (
							<div className="text-textcolor/60 flex flex-1 flex-col items-center justify-center py-6 text-center text-sm">
								<Loading text={t('englishLearning.classic.historyLoading')} />
							</div>
						) : null}
						{entries.map((h) => {
									...
									{isStreaming ? (
										...
										<div className={ROW_ACTIONS_CLASS}>
											...
											<Export
												...
												streamId={h.streamId}
												title={h.topic?.trim() || undefined}
												quoteCount={h.quoteCount > 0 ? h.quoteCount : undefined}
												disabled={busy || deleting}
												onBeforeClick={(e) => {
													e.stopPropagation();
												}}
						...
						{showEmpty ? (
							<div className="text-textcolor/60 py-8 text-center text-sm">
								{t('englishLearning.classic.historyEmpty')}
							</div>
						) : null}
					</div>
				</ScrollArea>
...
export const ClassicQuotesHistoryDrawer = observer(
	ClassicQuotesHistoryDrawerInner,
);
