/** 与侧栏「今日记词」按钮一致的 emerald·teal 预设按钮底样式 */
export const DAILY_COUNT_PRESET_BTN_CLASS =
	'flex-1 rounded-md border bg-emerald-500/15 px-0 py-1 text-xs font-medium transition-colors hover:bg-emerald-500/20';

export const DAILY_COUNT_PRESET_BTN_SELECTED_CLASS =
	'border-emerald-500/35 bg-emerald-500/20 text-emerald-600';

export const DAILY_COUNT_PRESET_BTN_UNSELECTED_CLASS =
	'border-emerald-500/10 text-textcolor hover:border-emerald-500/25 hover:bg-emerald-500/20 hover:text-emerald-600';

export const QUIZ_OPTION_CLASS =
	'h-auto min-h-11 w-full justify-start rounded-md border border-theme/10 bg-theme/5 px-3.5 py-2.5 text-left text-sm font-normal leading-snug shadow-none transition-colors hover:border-teal-500/35 hover:bg-teal-500/10 focus-visible:ring-teal-500/30';
