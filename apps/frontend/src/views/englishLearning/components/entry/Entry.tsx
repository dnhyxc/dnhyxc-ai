 ...
 * 听写 / 拼写练习入口（通用）
 *
 * - 组件：`Entry`（link / button / text / icon）
 * - 逻辑：`openPractice`、`useOpenPractice`
 */
...
export type OpenPracticeOptions = {
...
export function normalizePoolTotal(
	poolTotal: number | undefined,
): number | undefined {
	return poolTotal != null && poolTotal > 0 ? poolTotal : undefined;
}

export type EntryVariant = 'link' | 'button' | 'text' | 'icon';
...
export function useOpenPractice() {
	...
	return useCallback(
		(
			...
			openPractice(navigate, practice, options);
		},
		[navigate],
	);
}

const VARIANT_ICON_SIZE: Record<EntryVariant, string> = {
	link: 'size-3.5',
	button: 'size-3.5',
	text: 'size-4',
	icon: 'size-3.5',
	...
	const handleClick = useCallback(
		(event: MouseEvent<HTMLButtonElement>) => {
			...
			openPractice(
				...
				{
					...practice,
					poolTotal: normalizePoolTotal(practice.poolTotal),
				},
				{ syncPoolMeta },
			);
		},
