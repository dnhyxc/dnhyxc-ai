...
export function useListCornerFab(
	...
	const onCornerFab = useCallback<UIEventHandler<HTMLDivElement>>(() => {
		updateMode();
	}, [updateMode]);

	const onCornerFabClick = useCallback(() => {
		const vp = viewportRef.current;
		if (!vp) return;
	...
	return { mode, onCornerFab, onCornerFabClick };
}
