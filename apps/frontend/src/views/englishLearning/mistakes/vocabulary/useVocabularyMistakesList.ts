...
export function useVocabularyMistakesList(active: boolean) {
	const { t } = useI18n();
	const [entries, setEntries] = useState<EnglishVocabularyMistakeListEntry[]>(
		[],
	);
	...
	const fetchFirstPage = useCallback(
		async (gen: number) => {
			...
			try {
				...
				Toast({
					type: 'error',
					title: t('englishLearning.mistakes.listLoadFailed'),
				});
			} finally {
	...
	const onBatchRemove = useCallback(
		async (selected: EnglishVocabularyMistakeListEntry[]) => {
			if (selected.length === 0) return;
			await removeEnglishVocabularyMistakesBatch(selected.map((it) => it.id));
			const gen = ++loadGenRef.current;
