	...
	},
	{
		...
		items: [
			...
			},
			{
				...
				description:
					'回答会边生成边展示；可随时停止生成；可在当前回答基础上续写。同一会话内支持消息树形态下的分支与重生成；分享只读页与在线阅读在复杂分支场景下也会尽量保持消息顺序与排版一致。',
			},
			{
				id: 'pg-s4-3',
				title: '4.3 联网检索与引用',
	...
	},
	{
		...
		items: [
			...
			},
			{
				...
				description:
					'已登录时在编辑器底部可使用知识库文档助手，结合正文多轮对话。未登录不展示该区域。长对话可用输入框右上方滚动到底部/顶部；流式生成时可滚回底部继续跟随。可将编辑器中选中文本发送到文档助手，再结合 AI 模式或 RAG 模式提问；重叠选区会做上下文收敛（以当前版本为准）。桌面端已登录时助手输入区支持文本/语音切换，随界面语言显示；语音模式下识别文字实时填入输入框。',
			},
		],
	},
	{
