...
> = {
	...
	'pg-s16-1': {
		...
		description:
			'Entry: sidebar Bookshelf or /ebook. Managing the shelf, uploads, and progress sync requires sign-in; the page may open when logged out but shelf features will not work.\nDesktop (Tauri): Select local file and pick epub or pdf. The book is added to the shelf immediately and you can tap Read right away; a cloud backup runs in the background (progress bar for large files). The file stays on disk; reading prefers local, with cloud fallback if the file is moved or deleted and backup succeeded. One book, one progress record.\nBrowser: Import file uploads epub/pdf to your account shelf; you stay on the shelf and tap Read on the card to open.\nSize limits: about 120MB per file for browser/cloud upload; desktop local reading allows larger files (about 512MB)—you can still read locally if cloud backup fails.\nShelf list: scroll down to load more; the header count is total books, not just the visible page.\nShelf cards: vertical layout with optional custom cover, format (EPUB/PDF), and four-edge progress ring. Hover the card for a overlay with Read/Continue, Set cover, progress, or Remove (with confirmation). Tap the title below the card to edit; Enter saves, Esc or blur cancels; success toasts on save.',
	},
	'pg-s16-2': {
