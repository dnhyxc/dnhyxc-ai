...
> = {
	...
	'pg-s8-4': {
		...
		description:
			'Settings → Voice settings (/setting/cloud-tts), available to all users. Top: Local voice settings—pick browser voices by female/male groups, preview, prefs saved per account on this device.\nActive members can enable “Use local voice for playback” in the local block or “Use cloud voice for playback” in the cloud block below—the two switches are mutually exclusive and set whether English learning speaker buttons use local or cloud by default (cloud may fall back to local when unavailable). Non-members have no switches and use local only.\nWhen cloud playback is on, adjust model, voice, speed, volume, pitch, emotion, format, language boost, etc. With Language boost set to Chinese, the voice list shows Chinese system voices (Mandarin and Cantonese); English shows English voices only; Auto shows all. The parameter form is disabled when cloud is off. Use ? beside Playback parameters for help. Cloud preview and restore-default apply to the cloud block; prefs sync across devices for the same account. Local preview always uses local voice.\nLong passages (cloud): with cloud playback on, longer text (e.g. EPUB Listen excerpts, long classic sentences) is synthesized in sentence-sized segments—the first segment starts as soon as it is ready and the next is prefetched while the current one plays; short words and phrases still use a single request.',
	},
	'pg-s8-5': {
