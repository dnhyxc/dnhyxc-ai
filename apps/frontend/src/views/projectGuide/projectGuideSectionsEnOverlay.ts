...
> = {
	...
	'pg-s8-3': {
		...
		description:
			'After sign-in: Settings → LLM (/setting/llm). Fill API Key (a local default may pre-fill on first visit; after save, only your account applies), Base URL, and model name.\nType directly or use the button beside Base URL / model to pick Zhipu GLM, SiliconFlow, or DeepSeek presets; switching preset or model clears the API Key so you must enter the matching provider key.\nSave when all fields are complete to enable custom config—the footer shows the active model in green for your account only. Without custom config, gray text shows the default model (SiliconFlow for active members, Zhipu GLM for non-members). Unsaved edits or incomplete fields show hints and keep Save disabled. Use the eye icon to show/hide the key. Use Restore default to revert.\n\nVector model (Knowledge RAG, saved separately from chat LLM): scroll to the Vector model block on the same page. Fill API Key, Embedding / Rerank endpoint URLs (full URLs), embedding and rerank model names, and collection name; presets are available. Save vector config to enable; each save records collections you have used and lists them under collections included in search. RAG searches those collections in parallel and always includes the system default bge collection; new articles index into the currently selected collection. Restore vector default clears only vector settings, not chat LLM.',
	},
	'pg-s8-4': {
