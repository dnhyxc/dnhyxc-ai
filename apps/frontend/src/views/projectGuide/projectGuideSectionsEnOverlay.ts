...
> = {
	...
	'pg-s8-4': {
		...
		description:
			'Settings → Voice settings (/setting/cloud-tts), available to all users. Top: Local voice settings—pick browser voices by female/male groups, preview, prefs saved per account on this device.\nActive members choose one playback source at the top: Local voice, MiniMax cloud, or iFlytek cloud (mutually exclusive; sets the default synthesis path for English learning and EPUB listen-to-book; may fall back to local when unavailable). Non-members have no picker and use local only.\nWith MiniMax cloud selected, adjust model, voice, speed, volume, pitch, emotion, format, language boost, etc. With Language boost set to Chinese, the voice list shows Chinese system voices (Mandarin and Cantonese); English shows English voices only; Auto shows all. The MiniMax form is disabled when MiniMax is not selected. Use ? beside Playback parameters for MiniMax field help.\nWith iFlytek cloud selected, configure voice, speed, volume, and pitch (0–100, 50 default) in the iFlytek block, with preview and restore-default; the block is disabled when iFlytek is not selected. MiniMax and iFlytek prefs sync across devices for the same account. Local preview always uses local voice.\nLong passages (cloud): with MiniMax or iFlytek cloud, longer text (e.g. EPUB Listen excerpts, long classic sentences) is synthesized in sentence-sized segments—the first segment starts as soon as it is ready and the next is prefetched while the current one plays; short words and phrases still use a single request.',
	},
	'pg-s8-5': {
	...
	'pg-s16-9': {
		...
		description:
			'Entry: while reading EPUB, select text → floating toolbar → Listen; or tap Listen on the quote card footer in the reading-notes list or details.\nPlayback: same TTS stack as English learning (active members default to cloud voice; local browser speech when not a member or when cloud fails). Longer excerpts use segmented cloud synthesis when cloud is on—the first segment starts as soon as it is ready. The button shows Stop while playing—tap again to stop.\nBar: while quote Listen is playing, the same bottom bar as listen-while-reading appears (pause/resume, stop, sentence list jump, previous/next sentence, speed 0.75×–3×, sentence progress).\nSentence highlight: while playing, the sentence currently being read gets a light yellow background; it clears when that sentence ends and moves to the next; when your selection spans line breaks, the previous sentence tint clears immediately on handoff. All tint is removed when you stop or when playback finishes. Independent from user highlights and reading-note underlines—you can still highlight while listening. When you resize the right sidebar or toggle the reading-notes panel, the tint stays aligned with the reflowed sentence.\nAuto-follow: long selections scroll the current sentence into view while playing. Manual scroll or wheel pauses follow; a bottom-right button returns to the playing passage and resumes auto-follow.\nNotes: the selection toolbar can stay visible when you tap Listen (same as Copy and Highlight); the MK assistant side panel is unaffected. PDF has no Listen entry yet.',
	},
	'pg-s16-10': {
		title: '16.10 EPUB listen while reading (EPUB only)',
		description:
			'Entry: while reading EPUB, tap the headphones icon left of the Bot icon in the header to start Listen to book; tap again to stop.\nPlayback: continuous sentence-by-sentence TTS from the visible chapter (or the sentence at your current reading position); advances to the next spine section at section end; shows a message when the book is finished. In continuous scroll reading mode, playback continues into the next on-screen block after the current section finishes.\nBar: a fixed bottom bar with pause/resume, stop, sentence list jump, previous/next sentence, playback speed 0.75×–3×, and chapter/sentence progress. Previous/next sentence and jumps from the sentence list no longer exit playback by mistake.\nSentences menu: tap Sentences (list icon) on the bar to open a per-chapter list; tap any line to start from that sentence and scroll it to the center of the screen—the list scrolls to the line currently playing when opened.\nSpeed: tap the speed button (e.g. 1 X) on the bar and pick from the popup; with cloud voice, the change applies to the current sentence immediately.\nHighlight: light yellow tint on the current sentence (same as quote Listen); clears at sentence end; independent from user highlights and reading-note underlines; stays aligned when you resize the right sidebar or toggle the reading-notes panel.\nAuto-follow: the current sentence scrolls into view while playing; manual scroll pauses follow—a bottom-right button returns to the playing line and resumes follow.\nTOC: jumping via the table of contents while listening resumes from the new location (continues playing if already playing).\nVs quote Listen: header Listen to book and quote Listen are mutually exclusive; quote Listen uses the same bottom bar while playing. PDF has no listen-while-reading yet.',
	},
};
