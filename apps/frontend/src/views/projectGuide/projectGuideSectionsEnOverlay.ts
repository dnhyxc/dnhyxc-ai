...
> = {
	...
	'pg-s17-4': {
		...
		description:
			'Enable: turn on the Video Player card in the Plugin Center; a TV-icon entry appears in the sidebar. Upload: on the page, click the drop zone or drag-and-drop local video files into the area; up to 100 files can be selected at once. Playback modes: the top-left mode button switches between Auto Play, Loop One, and Stop When Finished. Custom control bar: the progress bar at the bottom is clickable to seek; hovering shows a time tip and ruler, and dragging the thumb fine-seeks. Icons on the right are, in order: mirror flip, settings panel (playback mode / rate), Picture-in-Picture, fullscreen, and playlist. The volume button opens a vertical slider with mute toggle. The playback rate button opens a dial that supports arrow-key adjustments between 0.5X and 3.0X in 0.1 steps, plus five preset buttons (0.5X / 1.0X / 1.5X / 2.0X / 3.0X). Tick labels and preset buttons now auto-adapt to light / dark themes, so they are always readable. Tooltips: the small hints on progress, volume, rate, and playlist controls now appear consistently in both the standalone preview and the host-mounted plugin page. Theater fullscreen: clicking the fullscreen button hides the host sidebar, header, and ICP footer for a true theater experience. On Tauri the app window is taken to system fullscreen; in the browser it falls back to document fullscreen. Press Esc or click the fullscreen button again to exit. Multi-file playlist: when multiple files are uploaded, the list icon in the bottom-right opens a scrollable playlist for switching. When playback ends, the next file starts or stops depending on the current mode. Cleanup: leaving the player page automatically exits theater mode and destroys the player instance so other pages are not affected. Add more / reset: while playing, click the "+ Continue" button in the bottom-right to add more files; click "Reset" to clear the current playlist and release the local file handles held by the player.',
	},
};
