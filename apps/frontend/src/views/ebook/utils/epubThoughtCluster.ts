/**
 * 本文件提供「想法簇」（Ebook Thought Cluster，想法区划线）相关的核心工具函数，其作用与原理详述如下：
 *
 * # 应用背景
 * 在 EPUB 电子书中，「想法」（EbookThought）代表用户对一段文本的注释、划线或评论。由于内容块可能重叠或彼此接近，需对这些想法进行分组聚合，以便在阅读器前端实现高亮、互动、批量渲染与点击等功能。想法通常通过 EPUB CFI（Canonical Fragment Identifier）精确定位到电子书内部的文档节点和范围。
 *
 * # 主要功能
 * 1. **按 CFI 分组与聚合**
 *    - 根据每条想法的 `cfiRange` 字段，对想法进行归类，实现同一区段的想法可以合并展示，降低视觉冗余，方便事件分发。
 *
 * 2. **排序、优先级与显示规则**
 *    - 实现想法的时间倒序、长度优先等多样排序逻辑，决定在同组中主展示哪一条想法，以及如何在视图聚合块中选取主 CFI（primaryCfi）。
 *
 * 3. **连通性判定（跨行/跨段/跨章节）**
 *    - 提供想法间相交、嵌套、端点连接等判定手段，构建「连通图」（adjacency），从种子想法出发快速收敛所有间接相关联的 CFI，保证高亮区块在任意复杂结构下都能合适联动。
 *
 * 4. **quote 合并与 DOM 范围解析**
 *    - 对引用片段（quote）做拼接或合并，必要时解析为 DOM Range，实现精准的高亮与定位。聚合时，自动避免范围取并集导致 CFI 不可逆的问题（如优先采用最长片段的主 CFI）。
 *
 * 5. **高亮区域与点击处理**
 *    - 根据簇结构与主 CFI 计算高亮展示区，用于点击、hover、批量弹窗等交互场景，并支持与 DOM 班级同步。特殊场景（如边界覆盖、未标注间隙）下自动避免多余的盲区或遮挡。
 *
 * 6. **性能与缓存**
 *    - 适配大章节、多簇、多想法高频操作，缓存 chapter 级的连通关系，以提升滚动、展开、收起等交互性能，保障流畅体验。
 *
 * # 适用场景举例
 * - 电子书中多条想法（划线/批注/引述）“重叠”或跨段分布时，需要自动归并为可交互的展示块。
 * - 想法的 hover/click 联动时，需一键高亮全部相关区块。
 * - 用户高亮、想法划线、自动引用等复合交互统一复用底层聚合与分组逻辑。
 *
 * # 主要导出工具
 * - groupThoughtsByCfi: 将想法按 CFI 分组。
 * - thoughtGroupSpanLength: 返回分组文本长度，用于确定主 CFI。
 * - collectConnectedClosureAroundCfis: 连通闭包收集，抓取所有间接相关联 CFI。
 * - mergeDomRangeUnion: 合并多个 Range 得到并集。
 * - pickPrimaryCfiFromQuoteGroups: 按规则挑选簇中主展示的 CFI。
 * - 其余高亮、排序、分组等辅助判定工具。
 *
 * # 依赖关系
 * - 依赖 ebookjs 的 Rendition 结构与 DOM CFI 解析方案。
 * - 需配合 ebookThoughtAnnotations、epubRangeGeometry 等工具模块共同使用。
 *
 * 本文件为想法聚合、展示与交互的底层核心依赖，为前端引用区、批注高亮、划线点击等入口复用。
 */
