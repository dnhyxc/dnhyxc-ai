	...
	},
	{
		...
		items: [
			...
			},
			{
				...
				description:
					'账户头像等云图片改由服务端上传至腾讯云对象存储，不再使用七牛前端直传；上传成功后仍保存完整图片地址。Web 端通过同源 /ext-cos/ 代理展示云对象（不限于图片），避免混合内容拦截；大图预览与上传区下载云图已修复，并在成功或失败时给出提示。对话中的聊天附件仍为本地上传，行为不变。部署需在服务端配置 COS 密钥与桶信息，并保证对象可被正常读取（推荐公有读或等价策略）。',
			},
		],
	},
	{
