export const zhCN = {
	'common.language': '语言',
	'common.theme': '主题',
	'common.settings': '设置',
	'common.appTitle': '智能对话',
	'common.loading': '加载中...',
	'common.loadingEffort': '正在奋力加载中...',
	'common.loadingMore': '加载更多...',
	'common.noMore': '没有更多了',
	'common.loadFailed': '加载失败',
	'common.deleteFailed': '删除失败',
	'common.tryLater': '请稍后重试',
	'common.networkErrorTryAgain': '网络异常，请检查网络后重试',
	'common.requestFailed': '请求失败，请稍后重试',
	'common.delete': '删除',
	'common.cancel': '取消',
	'common.confirm': '确定',
	'common.copy': '复制',
	'common.download': '下载',
	'common.downloading': '下载中...',
	'common.loaded': '已加载 {count} {type}',
	'common.type-1': '个',
	'common.type-2': '条',
	'common.type-3': '项',
	'common.publicBadge': '公开',
	'common.makePrivate': '取消公开',

	'image.loadFailed': '图片加载失败',

	'imagePreview.title': '图片预览',
	'imagePreview.mermaidTitle': 'Mermaid 图表预览',
	'imagePreview.zoomIn': '放大',
	'imagePreview.zoomOut': '缩小',
	'imagePreview.rotate': '旋转',
	'imagePreview.reset': '重置',
	'imagePreview.prev': '上一张',
	'imagePreview.next': '下一张',
	'imagePreview.close': '关闭',

	'upload.tooltip.default': '仅支持PDF、Word、Excel文件',
	'upload.uploading': '上传中...',
	'upload.button': '上传文件',
	'upload.error.maxCount': '最多只能同时上传 {maxCount} 个文件',
	'upload.error.invalidType': '不支持的文件类型: {type}',
	'upload.error.maxSize': '文件大小不能超过 {maxSizeMb} MB',

	'dragUpload.tip.prefix': '拖拽图片到此处或',
	'dragUpload.tip.clickToSelect': '点击选择',
	'dragUpload.tip.info': '支持 JPEG, PNG, GIF, SVG, WebP 格式，最大5MB',
	'dragUpload.error.invalidType': '不支持的文件类型: {type}',
	'dragUpload.error.maxSize': '文件大小不能超过 {maxSizeMb} MB',

	'nav.home': '首页',
	'nav.knowledge': '知识库',
	'nav.skills': '技能',
	'nav.ebook': '书架',
	'nav.chat': '智能会话',
	'nav.englishLearning': '英语学习',
	'nav.plugins': '插件中心',
	'nav.remoteDemo': '插件演示',
	'nav.remoteCounter': '计数插件',
	'nav.pay': '会员充值',
	'nav.profile': '我的主页',
	'nav.switchAccount': '切换账号',

	'profile.section.basic': '基本信息',
	'profile.section.voice': '语音转换',
	'profile.fields.roles': '角色',
	'profile.badge.member': '会员',
	'profile.badge.nonMember': '非会员',
	'profile.membership.title': '升级会员权益',
	'profile.membership.desc':
		'购买或续费会员，获取更多额度与权益；支付将在安全收银台完成。',
	'profile.membership.benefitsTitle': '会员权益说明',
	'profile.membership.benefit1':
		'提升智能对话、文档等场景的使用额度，减少频繁限额打断。',
	'profile.membership.benefit2':
		'通过 Stripe 嵌入式收银台完成支付，流程透明、可追踪。',
	'profile.membership.benefit3':
		'权益随产品迭代持续更新，具体额度与条款以会员充值页展示为准。',
	'profile.actions.buyMembership': '购买会员',
	'profile.actions.renewMembership': '续费会员',
	'profile.membership.expiresAt': '会员有效期至 {date}',
	'profile.membership.nonMemberHint':
		'您暂时还不是会员，点击会员购买成为尊贵的会员用户吧！',
	'profile.actions.editAccount': '编辑账户资料',
	'profile.actions.goPay': '前往会员充值',
	'profile.empty.title': '暂未获取到用户信息',
	'profile.empty.hint': '请重新登录后再试。',
	'profile.actions.goLogin': '去登录',
	'profile.voice.desc':
		'开启后，桌面端（Tauri）对话输入区可使用语音转写，识别文字实时填入输入框；关闭后仅保留文本输入与发送。',
	'profile.voice.enableLabel': '启用语音转文字',

	'setting.menu.system': '系统设置',
	'setting.menu.theme': '主题设置',
	'setting.menu.language': '语言设置',
	'setting.menu.about': '关于应用',
	'setting.menu.llm': '模型配置',
	'setting.menu.cloudTts': '语音设置',
	'setting.llm.title': '大模型配置',
	'setting.llm.desc':
		'填写 API Key、Base URL 与模型名称后，点击「保存模型配置」即可启用自定义大模型（对话、知识库助手、问答、英语学习等模块共用）。「恢复默认配置」将关闭自定义并回到按会员状态使用的默认模型。',
	'setting.llm.connectionTitle': '连接参数',
	'setting.llm.connectionDesc':
		'请填写 OpenAI 兼容接口的 API Key、Base URL 与模型名称，填写完成后点底部保存。',
	'setting.llm.baseUrl': 'Base URL',
	'setting.llm.baseUrlPlaceholder': '输入 Base URL',
	'setting.llm.openPresets': '选择预设',
	'setting.llm.presetOrCustomHint': '在输入框内直接填写，或点右侧按钮选择预设',
	'setting.llm.baseUrlOption.siliconflow': '硅基流动（api.siliconflow.cn）',
	'setting.llm.baseUrlOption.glm': '智谱 GLM（open.bigmodel.cn）',
	'setting.llm.baseUrlOption.deepseek': 'DeepSeek（api.deepseek.com）',
	'setting.llm.modelName': '模型名称',
	'setting.llm.modelNamePlaceholder': '输入模型名称',
	'setting.llm.modelOption.glmFlash': 'glm-4.7-flash',
	'setting.llm.modelOption.glm51': 'Pro/zai-org/GLM-5.1',
	'setting.llm.modelOption.deepseekChat': 'deepseek-chat',
	'setting.llm.apiKey': 'API Key',
	'setting.llm.apiKeyPlaceholder': '请输入 API Key',
	'setting.llm.apiSecretPlaceholder': '请输入 API Secret',
	'setting.llm.appIdPlaceholder': '请输入 APP ID',
	'setting.llm.showApiKey': '显示 API Key',
	'setting.llm.hideApiKey': '隐藏 API Key',
	'setting.llm.apiKeyKeepPlaceholder': '留空则保留已保存的密钥',
	'setting.llm.apiKeyConfigured': '已配置：{{mask}}',
	'setting.llm.save': '保存模型配置',
	'setting.llm.clear': '恢复默认配置',
	'setting.llm.saveSuccess': '大模型配置已保存',
	'setting.llm.clearSuccess': '已恢复使用环境变量',
	'setting.llm.activeHint': '{modelName} 模型配置生效中',
	'setting.llm.unsavedHint': '有未保存更改，请保存',
	'setting.llm.readyToSaveHint': '可保存设置让自定义模型生效',
	'setting.llm.incompleteDraftHint': '请补全三项内容后再保存让模型生效',
	'setting.llm.defaultHint': '当前默认 {modelName} 模型生效中',
	'setting.llm.incompleteHint': '已开启但配置不完整，仍使用环境变量',
	'setting.llm.vectorTitle': '向量模型配置',
	'setting.llm.vectorDesc':
		'用于知识库入库与 RAG 检索的 embedding / rerank 模型。请分别填写 embedding 与 rerank 的完整接口地址。',
	'setting.llm.vectorEmbeddingUrl': '向量模型 URL',
	'setting.llm.vectorEmbeddingUrlPlaceholder': '输入向量模型完整接口地址',
	'setting.llm.vectorEmbeddingUrlOption.siliconflow':
		'硅基流动 embeddings（api.siliconflow.cn/v1/embeddings）',
	'setting.llm.vectorRerankUrl': '重排模型 URL',
	'setting.llm.vectorRerankUrlPlaceholder': '输入重排模型完整接口地址',
	'setting.llm.vectorRerankUrlOption.siliconflow':
		'硅基流动 rerank（api.siliconflow.cn/v1/rerank）',
	'setting.llm.vectorEmbeddingModel': '向量模型名称',
	'setting.llm.vectorEmbeddingModelPlaceholder': '输入 embedding 模型名称',
	'setting.llm.vectorModelOption.bgeLargeZh': 'BAAI/bge-large-zh-v1.5',
	'setting.llm.vectorModelOption.qwen3Emb': 'Qwen/Qwen3-Embedding-4B',
	'setting.llm.vectorRerankModel': '重排模型名称',
	'setting.llm.vectorRerankModelPlaceholder': '输入 rerank 模型名称',
	'setting.llm.vectorRerankOption.bgeReranker': 'BAAI/bge-reranker-v2-m3',
	'setting.llm.vectorRerankOption.qwen3Rerank': 'Qwen/Qwen3-Reranker-4B',
	'setting.llm.vectorCollectionName': '向量库名称',
	'setting.llm.vectorCollectionNamePlaceholder': 'Qdrant collection 名称',
	'setting.llm.vectorCollectionOption.default':
		'knowledge_chunks_v2（默认 1024 维）',
	'setting.llm.vectorCollectionOption.qwen3':
		'knowledge_chunks_qwen3_2560（2560 维）',
	'setting.llm.vectorSearchProfilesTitle': '已纳入检索的向量库',
	'setting.llm.vectorSearchProfilesDesc':
		'每次保存会累积记录向量库与对应模型；RAG 检索时并行查询下列库，并始终包含系统默认 bge 库（knowledge_chunks_v2）。',
	'setting.llm.vectorApiKey': 'API Key',
	'setting.llm.vectorSave': '保存向量配置',
	'setting.llm.vectorClear': '恢复向量默认',
	'setting.llm.vectorSaveSuccess': '向量模型配置已保存',
	'setting.llm.vectorClearSuccess': '已恢复向量环境变量默认',
	'setting.llm.vectorActiveHint': '{embeddingModel} 向量配置生效中',
	'setting.llm.vectorUnsavedHint': '向量配置有未保存更改，请保存',
	'setting.llm.vectorReadyToSaveHint': '可保存向量配置让自定义模型生效',
	'setting.llm.vectorIncompleteDraftHint': '请补全向量配置各项后再保存',
	'setting.llm.vectorDefaultHint': '当前使用默认向量模型 {embeddingModel}',
	'setting.llm.vectorBgeOnlyLabel': '仅使用 BGE 向量库',
	'setting.llm.vectorBgeOnlyDesc':
		'开启后入库、RAG 检索与重排固定为 BAAI/bge-large-zh-v1.5、BAAI/bge-reranker-v2-m3、knowledge_chunks_v2，不再查询其它向量库。',
	'setting.llm.vectorBgeOnlyActiveHint':
		'仅 BGE 向量库模式生效中（knowledge_chunks_v2）',
	/** 云端朗读 */
	'setting.cloudTts.title': '云端语音设置',
	'setting.cloudTts.desc':
		'选择上方「MiniMax 云端」后生效，API Key、模型填写后使用自定义设置，留空或点击「恢复默认参数」则使用默认配置。',
	'setting.cloudTts.playbackSourceTitle': '朗读来源',
	'setting.cloudTts.playbackSourceHelp':
		'有效会员四选一，互斥生效：本机 Web Speech、Edge 云端、MiniMax 云端或讯飞在线合成。',
	'setting.cloudTts.playbackSourceHelpFree':
		'二选一，互斥生效：本机 Web Speech 或 Edge 云端。',
	'setting.cloudTts.playbackSource.local': '本机语音',
	'setting.cloudTts.playbackSource.cloud': 'MiniMax 云端',
	'setting.cloudTts.playbackSource.xfyun': '讯飞云端',
	'setting.cloudTts.playbackSource.edge': 'Edge 云端',
	'setting.cloudTts.playbackSourceHelp.local':
		'使用浏览器 Web Speech，音色由下方本机设置决定。',
	'setting.cloudTts.playbackSourceHelp.cloud':
		'使用 MiniMax 流式合成，参数见下方 MiniMax 区块。',
	'setting.cloudTts.playbackSourceHelp.xfyun':
		'使用讯飞在线合成，适合中文听书；参数见下方讯飞区块。',
	'setting.cloudTts.playbackSourceHelp.edge':
		'使用 Microsoft Edge 在线合成；参数见下方 Edge 区块。',
	'setting.cloudTts.xfyunTitle': '讯飞语音设置',
	'setting.cloudTts.xfyunCredentialsHint':
		'选择上方「讯飞云端」后生效；APPID、APIKey、APISecret 均填写后设置生效；留空或点击「恢复默认参数」则使用默认配置。',
	'setting.cloudTts.xfyunAppId': 'APPID',
	'setting.cloudTts.xfyunApiKey': 'APIKey',
	'setting.cloudTts.xfyunApiSecret': 'APISecret',
	'setting.cloudTts.xfyunVoice': '发音人',
	'setting.cloudTts.xfyunSpeed': '语速',
	'setting.cloudTts.xfyunVolume': '音量',
	'setting.cloudTts.xfyunPitch': '音高',
	'setting.cloudTts.xfyunPreview': '讯飞试听',
	'setting.cloudTts.edgeTitle': 'Edge 语音设置',
	'setting.cloudTts.edgeHint':
		'选择上方「Edge 免费语音」后生效；基于 Microsoft Edge 在线 TTS。',
	'setting.cloudTts.edgeVoice': '发音人',
	'setting.cloudTts.edgePreview': 'Edge 试听',
	'setting.cloudTts.paramsTitle': '朗读参数',
	'setting.cloudTts.paramsDesc': '音色、语速与音频格式等，可按需调整。',
	'setting.cloudTts.minimaxCredentialsHint':
		'API Key 留空时使用服务端环境变量；模型名称留空时同样回退服务端配置。',
	'setting.cloudTts.minimaxApiKey': 'API Key',
	'setting.cloudTts.paramsHelpAria': '查看朗读参数字段说明',
	'setting.cloudTts.paramsHelpTitle': '字段说明',
	'setting.cloudTts.fieldHelp.model':
		'云端朗读合成模型，如 speech-2.8-turbo（默认，更快）或 hd 系列（音质更好）。',
	'setting.cloudTts.fieldHelp.voiceId':
		'朗读音色，决定由谁朗读；从预设列表中选择即可。',
	'setting.cloudTts.fieldHelp.speed': '语速，范围 0.5–2，1 为正常速度。',
	'setting.cloudTts.fieldHelp.vol':
		'音量，范围 0.01–10；默认 5，数值越大越响（1 为标准音量）。',
	'setting.cloudTts.fieldHelp.pitch':
		'音高，范围 -12–12，0 为标准；正数更尖、更亮，负数更低沉。',
	'setting.cloudTts.fieldHelp.emotionIntro':
		'可选情感风格；选「无」则使用音色默认表现。',
	'setting.cloudTts.emotion.happy': '开心',
	'setting.cloudTts.emotion.sad': '难过',
	'setting.cloudTts.emotion.angry': '生气',
	'setting.cloudTts.emotion.fearful': '害怕',
	'setting.cloudTts.emotion.disgusted': '厌恶',
	'setting.cloudTts.emotion.surprised': '惊讶',
	'setting.cloudTts.emotion.calm': '中性',
	'setting.cloudTts.emotion.fluent': '生动',
	'setting.cloudTts.fieldHelp.format':
		'输出音频格式，mp3 通用；pcm、wav 等可按播放器或下游需求选择。',
	'setting.cloudTts.fieldHelp.languageBoost':
		'告诉朗读引擎按哪种语言优化发音，减少读错语言、口音偏调等问题。文本以英文为主时选「英语」，以中文为主时选「中文」；语言混杂或不确定时选「自动」。',
	'setting.cloudTts.fieldHelp.sampleRate':
		'采样率（Hz），默认 32000；越高潜在细节越多，文件通常更大。',
	'setting.cloudTts.fieldHelp.bitrate':
		'比特率，影响压缩音质与体积，默认 128000。',
	'setting.cloudTts.fieldHelp.channel':
		'声道数：1 为单声道（推荐），2 为立体声；朗读场景一般保持 1 即可。',
	'setting.cloudTts.enabledLabel': '使用自定义朗读参数',
	'setting.cloudTts.enabledHelp':
		'开启后，云端朗读使用下方参数；关闭时使用服务端默认合成。',
	'setting.cloudTts.playbackLabel': '使用云端语音朗读',
	'setting.cloudTts.playbackHelp':
		'与上方「使用本机语音朗读」互斥；开启后下方参数表单生效。',
	'setting.cloudTts.openPresets': '选择预设',
	'setting.cloudTts.model': '模型',
	'setting.cloudTts.modelPlaceholder': '输入模型名称',
	'setting.cloudTts.modelOption.speech28Hd': 'speech-2.8-hd（音质更好）',
	'setting.cloudTts.modelOption.speech28Turbo': 'speech-2.8-turbo（默认）',
	'setting.cloudTts.voiceId': '音色类别',
	'setting.cloudTts.voiceGroup.female': '女性',
	'setting.cloudTts.voiceGroup.male': '男性',
	'setting.cloudTts.speed': '语速',
	'setting.cloudTts.vol': '音量',
	'setting.cloudTts.pitch': '音高',
	'setting.cloudTts.emotion': '情感',
	'setting.cloudTts.emotionNone': '无（默认）',
	'setting.cloudTts.format': '音频格式',
	'setting.cloudTts.languageBoost': '语言增强',
	'setting.cloudTts.languageBoostOption.auto': '自动',
	'setting.cloudTts.languageBoostOption.English': '英语',
	'setting.cloudTts.languageBoostOption.Chinese': '中文',
	'setting.cloudTts.sampleRate': '采样率',
	'setting.cloudTts.bitrate': '比特率',
	'setting.cloudTts.channel': '声道',
	'setting.cloudTts.advancedHint': '以下为高级音频参数，一般保持默认即可。',
	'setting.cloudTts.reset': '恢复默认参数',
	'setting.cloudTts.decreaseValue': '减小{label}',
	'setting.cloudTts.increaseValue': '增大{label}',
	'setting.cloudTts.preview': '云端试听',
	'setting.cloudTts.previewText': '这是基于上方参数的云端朗读效果预览。',

	/** 主题设置 */
	'setting.theme.colorTitle': '彩色主题',
	'setting.theme.color.white': '白色',
	'setting.theme.color.black': '黑色',
	'setting.theme.color.green': '绿色',
	'setting.theme.color.purple': '紫色',
	'setting.theme.color.bluePurple': '蓝紫',
	'setting.theme.color.blue': '蓝色',
	'setting.theme.color.cyanBlue': '青蓝',
	'setting.theme.color.orange': '橙色',
	'setting.theme.color.red': '红色',
	'setting.theme.color.beige': '米色',
	'setting.theme.accentTitle': '主题色设置',
	'setting.theme.accent.teal': '默认',
	'setting.theme.accent.teal.desc': '经典青绿，清晰醒目',
	'setting.theme.accent.yuebai': '青柠',
	'setting.theme.accent.yuebai.desc': '鲜活明快，春芽初绽',
	'setting.theme.accent.zhizi': '桃红',
	'setting.theme.accent.zhizi.desc': '明艳活泼，花事正浓',
	'setting.theme.accent.xueqing': '靛青',
	'setting.theme.accent.xueqing.desc': '沉稳开阔，深远海空',
	'setting.theme.accent.canglang': '赭石',
	'setting.theme.accent.canglang.desc': '温润厚实，大地赤焰',
	'setting.theme.accent.wuxin': '缃黄',
	'setting.theme.accent.wuxin.desc': '明亮通透，金辉暖意',
	'setting.theme.accent.yushi': '杏橙',
	'setting.theme.accent.yushi.desc': '温暖亲和，秋日斜晖',
	'setting.theme.accent.tuoyan': '黛青',
	'setting.theme.accent.tuoyan.desc': '静谧沉着，深夜湖色',
	'setting.theme.accent.xicao': '松花',
	'setting.theme.accent.xicao.desc': '清浅柔和，晨雾初开',
	'setting.theme.accent.qiansui': '苍翠',
	'setting.theme.accent.qiansui.desc': '深邃沉静，林荫苔色',
	'setting.theme.previewTitle': '主题预览',
	'setting.theme.preview.cardBg.title': '当前卡片背景',
	'setting.theme.preview.cardBg.desc':
		'本预览区域容器使用 theme-card；切换上方主题即可看到变化。',
	'setting.theme.preview.tip':
		'选择彩色主题后，整个页面的背景色将跟随主题色变化',
	'setting.theme.preview.pageBg.title': '页面背景',
	'setting.theme.preview.foreground.title': '标题 / 前景字',
	'setting.theme.preview.foreground.desc': '用于标题、强调段落的主要前景色',
	'setting.theme.preview.text.title': '正文',
	'setting.theme.preview.text.desc':
		'这是一段示例文本，展示在不同主题下的显示效果。页面的主要背景色、卡片背景、文字颜色等都会随主题色变化。',
	'setting.theme.preview.mutedText.title': '次要说明字',
	'setting.theme.preview.mutedText.desc':
		'辅助说明、时间戳、占位提示等次要信息。',
	'setting.theme.preview.accent.title': '主题色',
	'setting.theme.preview.accent.desc': 'hover、选中、链接与按钮等交互强调色',
	'setting.theme.preview.mutedBg.title': '次要区域背景',
	'setting.theme.preview.mutedBg.desc':
		'muted 背景上的正文仍使用 textcolor，层次靠底色区分。',
	'setting.theme.preview.secondary.title': '次要填充',
	'setting.theme.preview.secondary.desc': '侧栏条目、次级面板等使用的次要底色',
	'setting.theme.preview.selection.title': '文本选区',
	'setting.theme.preview.selection.desc': '拖选文本时的背景与字色示意',
	'setting.theme.preview.border.title': '边框',
	'setting.theme.preview.ring.title': '聚焦环',

	'setting.system.storage.title': '文件存储',
	'setting.system.storage.defaultPath': '默认存储路径',
	'setting.system.storage.changeDir': '更改目录',
	'setting.system.storage.selectDirOnlyDesktop':
		'选择存储目录仅在桌面客户端可用',

	'setting.system.startup.title': '启动设置',
	'setting.system.startup.autoStart': '设置开机自启',
	'setting.system.startup.autoStartOff': '开机不自动启动',
	'setting.system.startup.autoStartOn': '开机自动启动',
	'setting.system.startup.autoStartOnlyDesktop': '开机自启仅在桌面客户端可用',

	'setting.system.close.title': '关闭设置',
	'setting.system.close.closeApp': '关闭应用程序',
	'setting.system.close.minimizeToTray': '最小化到托盘，不退出程序',
	'setting.system.close.quit': '退出程序',

	'setting.system.localTts.title': '本机语音设置',
	'setting.system.localTts.desc':
		'英语学习使用浏览器本机语音；下拉按女声、男声分组，可选具体音色并试听。偏好按当前登录账号保存在本设备。',
	'setting.system.localTts.enabledLabel': '使用本机语音朗读',
	'setting.system.localTts.enabledHelp':
		'开启后，英语学习中的喇叭统一使用上方本机音色；与下方「云端语音设置」开关互斥。非会员始终使用本机朗读。',
	'setting.system.localTts.voiceLabel': '朗读音色',
	'setting.system.localTts.autoOption': '默认（Karen）',
	'setting.system.localTts.groupFemale': '女声',
	'setting.system.localTts.groupMale': '男声',
	'setting.system.localTts.genderTag.female': '女声',
	'setting.system.localTts.genderTag.male': '男声',
	'setting.system.localTts.preview': '试听',
	'setting.system.localTts.previewText':
		'This is a preview of the cloud reading effect with the parameters set above.',
	'setting.system.localTts.unsupported': '当前环境不支持本机语音合成。',
	'setting.system.localTts.noVoices':
		'未检测到英语语音，请在系统「辅助功能 → 朗读内容」中下载语音后重试。',

	'setting.system.shortcuts.title': '快捷键设置',
	'setting.system.shortcuts.desc':
		'「应用显示/刷新」相关快捷键（含窗口操作）会同步到菜单栏加速键；知识库等其余项仅在对应窗口内生效。',
	'setting.system.shortcuts.group.other': '其他',
	'setting.system.shortcuts.group.knowledge': '知识库',
	'setting.system.shortcuts.group.appVisibility': '应用显示/刷新',
	'setting.system.shortcuts.separator': '：',
	'setting.system.shortcuts.pressKey': '按键盘输入快捷键',
	'setting.system.shortcuts.conflictTitle': '快捷键冲突',
	'setting.system.shortcuts.conflictMessage':
		'该快捷键与「{label}」冲突，请更换其他组合',
	'setting.system.shortcuts.globalOnlyDesktop': '全局快捷键仅在桌面客户端可用',
	'setting.system.shortcuts.registerFailed': '全局快捷键注册失败',

	'setting.system.shortcuts.item.hide': '隐藏程序',
	'setting.system.shortcuts.item.hideOrShowApp': '显示隐藏应用',
	'setting.system.shortcuts.item.reload': '刷新应用',
	'setting.system.shortcuts.item.openSubwindow': '打开子窗口',
	'setting.system.shortcuts.item.window.minimize': '隐藏窗口',
	'setting.system.shortcuts.item.window.close': '关闭窗口',
	'setting.system.shortcuts.item.window.scale': '缩放窗口',
	'setting.system.shortcuts.item.window.fill': '填充窗口',
	'setting.system.shortcuts.item.window.center': '居中窗口',
	'setting.system.shortcuts.item.window.fullscreen': '全屏窗口',
	'setting.system.shortcuts.item.file.about': '关于应用',
	'setting.system.shortcuts.item.file.logout': '退出登录',
	'setting.system.shortcuts.item.file.quit': '退出应用',
	'setting.system.shortcuts.item.knowledge.save': '通用：保存',
	'setting.system.shortcuts.item.knowledge.clearDraft': '通用：新建',
	'setting.system.shortcuts.item.knowledge.import': '知识库：导入文件',
	'setting.system.shortcuts.item.knowledge.share': '知识库：分享文章',
	'setting.system.shortcuts.item.knowledge.openList': '知识库：打开列表',
	'setting.system.shortcuts.item.knowledge.openTrash': '知识库：打开回收站',
	'setting.system.shortcuts.item.knowledge.sendSelectionToAssistant':
		'知识库：发送选中内容到助手输入框',
	'setting.system.shortcuts.item.knowledge.toggleActionBar':
		'编辑器：切换操作栏',
	'setting.system.shortcuts.item.knowledge.bar.editSource': '编辑器：编辑源码',
	'setting.system.shortcuts.item.knowledge.bar.diff':
		'编辑器：分屏对照（Diff）',
	'setting.system.shortcuts.item.knowledge.bar.previewRender':
		'编辑器：预览渲染',
	'setting.system.shortcuts.item.knowledge.bar.toggleAssistant':
		'编辑器：开关 AI 助手',
	'setting.system.shortcuts.item.knowledge.bar.splitPreview':
		'编辑器：分屏预览',
	'setting.system.shortcuts.item.knowledge.bar.followBidirectional':
		'编辑器：双边跟随',
	'setting.system.shortcuts.item.knowledge.bar.followPreviewFollowsEditor':
		'编辑器：右边跟随左边',
	'setting.system.shortcuts.item.knowledge.bar.followEditorFollowsPreview':
		'编辑器：左边跟随右边',
	'setting.system.shortcuts.item.knowledge.bar.toggleOverrideSave':
		'编辑器：开关覆盖保存',
	'setting.system.shortcuts.item.knowledge.bar.toggleAutoSave':
		'编辑器：开关自动保存',
	'setting.system.shortcuts.item.knowledge.bar.resetPosition':
		'编辑器：复位初始位置',

	'setting.about.toast.latestVersion': '已经是最新版本',
	'setting.about.toast.fetchVersionFailed': '获取版本信息失败',
	'setting.about.toast.relaunchFailed': '重启失败',
	'setting.about.toast.cacheCleared': '缓存清理完成',
	'setting.about.latestVersion': '最新版本',
	'setting.about.releaseDate': '发布时间',
	'setting.about.checkUpdate': '检查更新',
	'setting.about.updateAndRelaunch': '更新并重启',
	'setting.about.viewReleaseNotes': '查看更新内容',
	'setting.about.download.downloading': '正在下载',
	'setting.about.download.done': '下载完成',
	'setting.about.update.title': '软件更新',
	'setting.about.update.notifyMe': '新版本发布时提醒我',
	'setting.about.cache.title': '应用缓存',
	'setting.about.cache.size': '当前缓存大小 ',
	'setting.about.cache.clear': '清空缓存',
	'setting.about.relaunchDialog.title': '确定要现在重启应用吗?',
	'setting.about.relaunchDialog.desc':
		'即将自动重启完成更新，为确保数据安全，请在重启前保存所有未保存的工作。重启后未保存的数据将无法恢复。建议点击"立即重启"前再次确认重要数据已妥善保存。',
	'setting.about.relaunchDialog.later': '稍后重启',
	'setting.about.relaunchDialog.now': '立即重启',

	'router.window.aboutTitle': '关于 dnhyxc-ai',

	'setting.language.title': '语言设置',
	'setting.language.zh': '中文（简体）',
	'setting.language.en': 'English',
	'setting.language.current': '当前语言：{lang}',

	'header.toggleLanguage': '切换语言',
	'header.breadcrumbNav': '面包屑导航',

	'monaco.placeholder': '# 输入内容...',
	'monaco.topBar.bottomBar': '操作栏',
	'monaco.topBar.bottomBar.expand': '展开 Markdown 底部操作栏',
	'monaco.topBar.bottomBar.collapse': '收起 Markdown 底部操作栏',
	'monaco.loading.editor': '正在加载编辑器...',
	'monaco.loading.diffEditor': '正在加载对照编辑器...',
	'monaco.autoSaveInterval.seconds': '{n} 秒',
	'monaco.autoSaveInterval.minutes': '{m} 分钟',
	'monaco.bottomBar.aria.toolbar': 'Markdown 底部操作',
	'monaco.bottomBar.aria.view': 'Markdown 视图',
	'monaco.bottomBar.tip.drag': '拖动调整操作栏位置',
	'monaco.bottomBar.aria.drag': '拖动底部操作栏位置',
	'monaco.bottomBar.tip.edit': '编辑源码',
	'monaco.bottomBar.aria.edit': '编辑源码',
	'monaco.bottomBar.tip.diff.open': '分屏对照修改：左编右只读 Diff',
	'monaco.bottomBar.tip.diff.close': '关闭分屏对照：回到单栏编辑',
	'monaco.bottomBar.aria.diff': '开关分屏 Markdown 修改对照（Diff）',
	'monaco.bottomBar.tip.preview': '预览渲染',
	'monaco.bottomBar.aria.preview': '预览渲染',
	'monaco.bottomBar.tip.assistant.open': '开启 AI 助手',
	'monaco.bottomBar.tip.assistant.close': '关闭 AI 助手',
	'monaco.bottomBar.aria.assistant': '开关 Markdown 右侧 AI 助手',
	'monaco.bottomBar.tip.split': '分屏：左编辑右预览',
	'monaco.bottomBar.aria.split': '分屏：左编辑右预览',
	'monaco.bottomBar.tip.follow.bidirectional':
		'双边跟随：编辑区与预览区双向同步滚动',
	'monaco.bottomBar.aria.follow.bidirectional':
		'双边跟随：编辑与预览互相同步滚动',
	'monaco.bottomBar.tip.follow.previewFollowsEditor':
		'右边跟随左边：滚动编辑区时预览区同步滚动',
	'monaco.bottomBar.aria.follow.previewFollowsEditor':
		'右边跟随左边：预览跟随编辑滚动',
	'monaco.bottomBar.tip.follow.editorFollowsPreview':
		'左边跟随右边：滚动预览区时编辑区同步滚动',
	'monaco.bottomBar.aria.follow.editorFollowsPreview':
		'左边跟随右边：编辑区跟随预览滚动',
	'monaco.bottomBar.tip.overwrite.enabled':
		'已开启覆盖保存：同名文件将直接覆盖写入',
	'monaco.bottomBar.tip.overwrite.disabled':
		'开启覆盖保存：同名文件不再弹窗确认，直接覆盖写入',
	'monaco.bottomBar.aria.overwrite.on': '开启覆盖保存',
	'monaco.bottomBar.aria.overwrite.off': '关闭覆盖保存',
	'monaco.bottomBar.tip.autosave.enabled':
		'已开启自动保存：按所选间隔在有修改时保存',
	'monaco.bottomBar.tip.autosave.disabled':
		'开启自动保存：按间隔自动保存（无标题/正文或同名冲突未开覆盖时会静默跳过）',
	'monaco.bottomBar.aria.autosave.on': '开启自动保存',
	'monaco.bottomBar.aria.autosave.off': '关闭自动保存',
	'monaco.bottomBar.label.autosaveInterval': '自动保存间隔',
	'monaco.bottomBar.aria.autosaveInterval': '自动保存间隔',
	'monaco.bottomBar.tip.reset': '复位操作栏初始位置',
	'monaco.bottomBar.aria.reset': '复位操作栏位置',

	'markdown.preview.empty': '预览内容为空',
	'markdown.preview.loading': '内容加载中…',
	'markdown.preview.scroll.toBottom': '滚动到底部',
	'markdown.preview.scroll.toTop': '滚动到顶部',

	'about.appVersion': 'dnhyxc-ai 版本 {version}',
	'about.copyright': 'dnhyxc 版权所有',
	'about.copyrightYears': 'Copyright © 2025 - 2026 Dnhyxc',
	'about.rightsReserved': 'All Rights Reserved',
	'about.links.policy': '用户服务政策',
	'about.links.terms': '用户服务协议',
	'about.links.updateInfo': '更新说明',

	'legal.backHome': '返回首页',
	'legal.servicePolicy.title': '用户服务政策',
	'legal.userAgreement.title': '用户服务协议',

	'notFound.title': '404 Not Found',
	'notFound.backHome': '返回首页',

	'account.fields.nickname': '昵称',
	'account.fields.gender': '性别',
	'account.fields.email': '邮箱',
	'account.fields.address': '地址',

	'account.placeholders.nickname': '请输入昵称',
	'account.placeholders.email': '请输入邮箱',
	'account.placeholders.address': '请输入地址',
	'account.placeholders.oldVerifyCode': '请输入原邮箱验证码',
	'account.placeholders.newEmail': '请输入新邮箱',
	'account.placeholders.newVerifyCode': '请输入新邮箱验证码',

	'account.gender.male': '男',
	'account.gender.female': '女',
	'account.gender.secret': '不便透露',

	'account.avatar.change': '更换',
	'account.modal.editEmail': '修改邮箱',

	'account.actions.getVerifyCode': '获取验证码',

	'account.toast.updateSuccess': '修改成功',
	'account.toast.updateFailed': '修改失败',
	'account.toast.testAccountForbidden': '测试账号，禁止操作',
	'account.toast.verifyCodeSent': '获取验证码成功',
	'account.toast.verifyCodeKeyMissing': '验证码 Key 不能为空',
	'account.toast.emailUpdated': '邮箱修改成功',

	'account.resetEmail.oldEmail': '原邮箱',
	'account.resetEmail.oldVerifyCode': '原邮箱验证码',
	'account.resetEmail.newEmail': '新邮箱',
	'account.resetEmail.newVerifyCode': '新邮箱验证码',

	'account.validation.emailInvalid': '请输入合法的邮箱地址',
	'account.validation.verifyCode6Digits': '验证码必须为6位数字',

	'account.wechat.title': '微信小程序',
	'account.wechat.hint':
		'生成 6 位关联码后，在读书小程序「绑定账号」页输入该码，即可与本 Web 账号合并。',
	'account.wechat.createCode': '生成关联码',
	'account.wechat.refreshCode': '刷新关联码',
	'account.wechat.codeLabel': '请在 {seconds} 秒内在小程序输入',
	'account.wechat.codeCreated': '关联码已生成',
	'account.wechat.codeCopied': '关联码已复制',
	'account.wechat.codeCopyFailed': '复制失败，请重试',
	'account.wechat.codeFailed': '生成关联码失败',
	'account.wechat.bound': '已关联微信 {id}',
	'account.wechat.unbind': '解除关联',
	'account.wechat.unbindSuccess': '已解除微信关联',
	'account.wechat.unbindFailed': '解除关联失败',

	'chat.newSession': '新对话',
	'chat.sessionList.title': '历史对话',
	'chat.sessionList.empty': '暂无历史对话',
	'chat.sessionList.confirmDelete.title': '确认删除',
	'chat.sessionList.confirmDelete.desc':
		'确定要删除这个会话吗？此操作无法撤销。',
	'chat.sessionList.toast.titleUpdated': '会话标题更新成功',
	'chat.share.selectAll': '全选',
	'chat.share.selectedPairs': '已选 {count} 组',
	'chat.share.createLink': '创建分享链接',
	'chat.newSession.welcomeTitle': '欢迎来到 dnhyxc-ai 智能聊天',
	'chat.newSession.welcomeHint': '有什么我可以帮您的？',

	'chat.entry.placeholder': '请输入您的问题',
	'chat.entry.attachments.textOnlyHint': '只识别附件中的文字',
	'chat.entry.newChat': '新对话',
	'chat.entry.upload.button': '上传附件',
	'chat.entry.upload.maxFiles': '最多只能支持 5 个文件',
	'chat.entry.upload.tooltip.supportedTypes':
		'仅支持 PDF、DOCX、XLSX、PNG、JPG、JPEG、WEBP 格式！',
	'chat.entry.upload.tooltip.maxFilesAndSize':
		'最多同时支持 5 个文件，每个文件最大 20 MB！',
	'chat.entry.webSearch': '联网搜索',
	'chat.entry.send': '发送',
	'chat.entry.sendDisabledHint':
		'请先输入内容；桌面端可悬停发送按钮打开菜单并选择语音输入。',
	'chat.entry.sendDisabledHintWeb': '请先输入内容',
	'chat.entry.inputMode.label': '输入模式',
	'chat.entry.inputMode.text': '文本输入',
	'chat.entry.inputMode.voice': '语音输入',
	'chat.entry.voice.noRecorder': '当前环境不支持录音或无法创建录音器',
	'chat.entry.voice.micDenied': '无法使用麦克风',
	'chat.entry.voice.liveListening': '聆听中，识别文字将填入此处（约每秒更新）…',
	'chat.entry.voice.stop': '停止录音并识别',
	'chat.entry.voice.transcribing': '识别中',
	'chat.entry.voice.start': '开始录音',

	'chat.bot.toast.noContent.title': '暂无内容',
	'chat.bot.toast.noContent.desc': '该条回复为空，无法保存到知识库',

	'chat.messageActions.saveToKnowledge': '保存到知识库',
	'chat.messageActions.shareAnswer': '分享此回答',

	'chat.assistant.thinking': '思考中...',
	'chat.assistant.readWebPages': '已阅读 {n} 个网页',
	'chat.assistant.thinkProcess': '思考过程',
	'chat.assistant.generating': '正在生成中...',
	'chat.assistant.aiDisclaimer': '本回答由 AI 生成，内容仅供参考，请仔细甄别',
	'chat.assistant.webPagesCount': '{n} 个网页',
	'chat.assistant.continueGenerate': '继续生成',
	'chat.assistant.maxTokensExceededPrefix': '超出最大输出长度，',
	'chat.assistant.clickContinueAnswer': '点击接着回答',

	'chat.controls.backToStreamingBranch': '回到正在生成的分支',
	'chat.controls.backToLatestBranch': '回到最新分支',
	'chat.controls.scroll.toTop': '滚动到顶部',
	'chat.controls.scroll.toBottom': '滚动到底部',

	'chat.anchorNav.prev': '上一条',
	'chat.anchorNav.next': '下一条',
	'chat.anchorNav.item': '对话 {index}：{content}',

	'chat.codeToolbar.aria': '代码块工具栏',
	'chat.codeToolbar.copy': '复制',
	'chat.codeToolbar.copied': '已复制',
	'chat.codeToolbar.download': '下载',

	'chat.textArea.placeholder': '请输入您的问题',
	'chat.textArea.send': '发送',

	'chat.searchOrganics.title': '联网搜索结果',

	'mermaid.toolbar.code': '代码',
	'mermaid.toolbar.diagram': '图表',
	'mermaid.toolbar.copy': '复制',
	'mermaid.toolbar.copied': '已复制',
	'mermaid.toolbar.preview': '预览',
	'mermaid.toolbar.download': '下载',

	'auth.logout': '退出登录',
	'auth.loginRequired': '请先登录',
	'auth.login.tab.username': '账号密码登录',
	'auth.login.tab.email': '邮箱登录',
	'auth.login.go': '已有账号，前往登录',
	'auth.register.go': '没有账号，点我注册',
	'auth.register.title': '注册账号',
	'auth.resetPassword.title': '重置密码',
	'auth.forgotPassword': '忘记密码',

	'auth.username': '用户名',
	'auth.password': '密码',
	'auth.captcha': '验证码',
	'auth.username.placeholder': '请输入用户名',
	'auth.password.placeholder': '请输入密码',
	'auth.showPassword': '显示密码',
	'auth.hidePassword': '隐藏密码',
	'auth.captcha.placeholder': '请输入验证码',
	'auth.login.submit': '登录',
	'auth.captcha.fetchFailed': '获取验证码失败！',

	'auth.validation.usernameMin': '用户名至少输入两个字符',
	'auth.validation.passwordMin': '密码至少输入8个字符',
	'auth.validation.passwordComplex': '密码必须包含英文、数字和特殊字符',
	'auth.validation.captchaMin': '验证码至少输入4个字符',
	'auth.validation.emailInvalid': '请输入合法的邮箱地址',
	'auth.validation.verifyCodeMin': '验证码至少输入6个字符',
	'auth.validation.passwordMismatch': '两次输入的密码不一致',

	'auth.email': '邮箱',
	'auth.verifyCode': '验证码',
	'auth.confirmPassword': '确认密码',
	'auth.email.placeholder': '请输入邮箱',
	'auth.verifyCode.placeholder.email': '请输入邮箱收到的验证码',
	'auth.confirmPassword.placeholder': '请输入确认密码',

	'auth.verifyCode.send': '获取验证码',
	'auth.verifyCode.sentSuccess': '发送验证码成功',
	'auth.username.requiredFirst': '请先输入用户名',

	'auth.register.submit': '注册',
	'auth.resetPassword.success': '重置密码成功',
	'auth.resetPassword.submit': '确定重置',
	'auth.login.back': '返回登录',

	'home.hero.welcome': '欢迎使用',
	'home.hero.product': '智能工作台',
	'home.hero.subtitle':
		'桌面端一站式集成智能对话、知识库、书架、英语学习与插件扩展。流式问答可沉淀为笔记，阅读与背词同屏可用，问、记、学在同一应用内闭环。',
	'home.hero.quickStart': '快速开始',
	'home.hero.learnMore': '了解更多',
	'home.hero.downloadDesktop': '下载桌面版',
	'home.hero.knowledge.title': '知识库',
	'home.hero.knowledge.subtitle': 'Markdown 笔记',
	'home.hero.knowledge.desc':
		'所见即所得的 Markdown 编辑与双栏预览，支持本地导入导出、草稿自动保存与回收站恢复。按分类与标签整理，全文检索快速定位，把灵感沉淀成可复用的知识资产。',
	'home.hero.ebook.title': '书架',
	'home.hero.ebook.subtitle': '电子书阅读',
	'home.hero.ebook.desc':
		'导入 EPUB，支持目录导航、全文搜索、阅读进度与书签管理。可调节字号与主题，随时续读未完内容，让数字书架变成可检索、可管理的私人书库。',
	'home.hero.english.title': '英语学习',
	'home.hero.english.subtitle': '单词与句型',
	'home.hero.english.desc':
		'每日单词推送与经典句练习，错题本与收藏夹同步沉淀。按记忆曲线安排复习，进度与掌握度清晰可见，从输入练习到输出巩固形成完整学习闭环。',
	'home.hero.plugins.title': '插件中心',
	'home.hero.plugins.subtitle': '可扩展架构',
	'home.hero.plugins.desc':
		'安装社区插件扩展能力，或按开发手册自建插件。支持按需启停与权限控制，把对话、知识与外部工具接入你的工作流，让工作台随需求持续扩展。',
	'home.stage.users': '模块精选',
	'home.stage.services': '覆盖对话、知识、阅读与学习，核心能力一屏直达。',
	'home.stage.servicesLink': '特色功能',
	'home.stage.headline': '快问 · 快记 · 快学',
	'home.stage.status': '桌面 AI 工作台',
	'home.stage.stat.modules': '精选模块',
	'home.stage.stat.entries': '核心入口',
	'home.stage.stat.local': '本地优先',
	'home.stage.stat.localValue': '100%',
	'home.stage.metricLabel': '能力矩阵',
	'home.stage.metricHint': '覆盖工作台核心场景',
	'home.stage.statsLink': '查看介绍',
	'home.features.enter': '进入模块',
	'home.sections.showcase': '特色功能',
	'home.sections.showcaseStatus': '4 项核心能力',
	'home.sections.steps': '快速开始',
	'home.sections.stepsStatus': '4 步快速上手',
	'home.sections.quicklinks': '快捷链接',
	'home.sections.quicklinksStatus': '快捷入口直达',
	'home.features.chat.title': '智能对话',
	'home.features.chat.subtitle': '自然语言处理',
	'home.features.chat.desc':
		'与 AI 助手自然对话，获取实时解答、创作建议与学习辅导',
	'home.features.coding.title': '代码助手',
	'home.features.coding.subtitle': '编程支持',
	'home.features.coding.desc': '智能生成、调试与优化代码，多语言开发全流程提效',
	'home.features.document.title': '文档处理',
	'home.features.document.subtitle': '智能分析',
	'home.features.document.desc':
		'Image、PDF、Word、Excel 等格式的解析、总结与内容提取',
	'home.showcase.plugin.title': '插件系统',
	'home.showcase.plugin.desc': '可扩展架构',
	'home.showcase.fast.title': '快速响应',
	'home.showcase.fast.desc': '毫秒级响应速度',
	'home.showcase.privacy.title': '隐私保护',
	'home.showcase.privacy.desc': '本地化处理',
	'home.showcase.i18n.title': '多语言',
	'home.showcase.i18n.desc': '中英双语支持',
	'home.showcase.lightweight.title': '轻量高效',
	'home.showcase.lightweight.desc': '低资源占用',
	'home.steps.install.title': '安装应用',
	'home.steps.install.desc': '下载并安装桌面版应用',
	'home.steps.register.title': '注册账号',
	'home.steps.register.desc': '创建您的个人账户',
	'home.steps.start.title': '开始使用',
	'home.steps.start.desc': '享受智能助手服务',
	'home.steps.pluginDev.title': '插件开发',
	'home.steps.pluginDev.guide': '开发指南',
	'home.steps.pluginDev.desc': '为平台开发专属插件',
	'home.quicklinks.dnhyxc-ai.title': 'dnhyxc-ai',
	'home.quicklinks.dnhyxc-ai.desc': 'dnhyxc-ai 网页版',
	'home.quicklinks.dnhyxc-ai-admin.title': 'dnhyxc-ai-admin',
	'home.quicklinks.dnhyxc-ai-admin.desc': 'dnhyxc-ai 后台管理',
	'home.quicklinks.blog.title': '我的博客',
	'home.quicklinks.blog.desc': '我的博客网站',
	'home.quicklinks.github.title': 'GitHub',
	'home.quicklinks.github.desc': '我的 GitHub 仓库',

	'knowledge.common.untitled': '未命名',
	'knowledge.toolbar.save': '保存',
	'knowledge.toolbar.import': '导入',
	'knowledge.import.success': '已导入到编辑器',
	'knowledge.import.empty': '文件内容为空',
	'knowledge.import.failed': '导入失败',
	'knowledge.import.tooLarge': '文件过大（上限 5MB）',
	'knowledge.import.notMd': '仅支持导入 .md 文件',
	'knowledge.toolbar.clear': '新建',
	'knowledge.toolbar.share': '分享',
	'knowledge.toolbar.library': '知识库',
	'knowledge.toolbar.trash': '回收站',
	'knowledge.shortcuts.save': 'Meta + S / Control + S',
	'knowledge.shortcuts.import': 'Meta + I',
	'knowledge.shortcuts.clear': 'Meta + Shift + N',
	'knowledge.shortcuts.share': 'Meta + Shift + O',
	'knowledge.shortcuts.openLibrary': 'Meta + Shift + L',
	'knowledge.shortcuts.openTrash': 'Meta + Shift + T',

	'knowledge.list.openInEditor': '在 Cursor 或 Trae 中打开',
	'knowledge.list.revealInFolder': '在访达中显示',
	'knowledge.list.revealInFolderFailed': '无法在访达中显示',
	'knowledge.list.deleteLocalMdAria': '删除本地 Markdown 文件',
	'knowledge.list.deleteFromLibraryAria': '从知识库删除',
	'knowledge.list.publicConfirmTitle': '确定公开这条知识文档？',
	'knowledge.list.publicConfirmDesc':
		'公开后，所有登录用户都可以在列表中拉取并查看该文档',
	'knowledge.list.privateConfirmTitle': '确定取消公开？',
	'knowledge.list.privateConfirmDesc': '取消后仅你本人可以查看该文档',
	'knowledge.list.madePublic': '已公开知识文档',
	'knowledge.list.madePrivate': '已取消公开',
	'knowledge.list.updatedAt': '更新 {time}',
	'knowledge.list.localLoadFailed': '加载本地列表失败',
	'knowledge.list.readFailed': '读取失败',
	'knowledge.list.cloudOpenLoginTip': '登录后可从云端知识库打开条目',
	'knowledge.list.detailMissing': '无法获取该条详情',
	'knowledge.list.cloudDeleteLoginTip': '未登录时无法删除云端知识库记录',
	'knowledge.list.fileDeleted': '文件已删除',
	'knowledge.list.localFileDeleted': '本地文件已删除',
	'knowledge.list.deletedBoth': '已同时删除',
	'knowledge.list.localFileDeleteFailed': '本地文件删除失败',
	'knowledge.list.dirNotSelected': '未选择目录',
	'knowledge.list.openedInExternalEditor': '已在外部编辑器打开',
	'knowledge.list.openedWithEditor': '使用 {editor} 打开文件',
	'knowledge.list.openFailed': '打开失败',
	'knowledge.list.deleteRecordTitle': '删除知识库记录？',
	'knowledge.list.deleteRecordDesc.tauri':
		'本地目录未找到对应文件，是否仅从数据库删除该条目？',
	'knowledge.list.deleteRecordDesc.web': '是否从数据库删除该条目？',
	'knowledge.list.fileNameLabel': '文件名称：「{name}」',
	'knowledge.list.deleteFileTitle': '删除文件？',
	'knowledge.list.deleteFileDesc.localOnly':
		'将仅从磁盘删除该文件，不涉及云端知识库数据。',
	'knowledge.list.deleteFileDesc.linked':
		'已关联云端知识库条目与本地 Markdown。可选择仅删本地、仅删在线，或使用「同时删除」移除两侧数据。',
	'knowledge.list.deleteBoth': '同时删除',
	'knowledge.list.deleteLocal': '本地删除',
	'knowledge.list.deleteOnline': '在线删除',
	'knowledge.list.dataSource': '数据来源',
	'knowledge.list.source.db': '数据库',
	'knowledge.list.source.local': '本地文件夹',
	'knowledge.list.pickFolder': '选择文件夹',
	'knowledge.list.localOnlyInDesktop':
		'本地文件夹列表仅在桌面端（Tauri）可用。',
	'knowledge.list.cloudDisabledWhenLoggedOut':
		'未登录：仅可使用本地文件夹（不请求云端）',
	'knowledge.list.localOpsOnly': '该列表的数据操作仅支持本地文件夹',
	'knowledge.list.localAndDbSync': '该列表的数据操作支持本地与数据库同步',
	'knowledge.list.empty.local': '该文件夹下暂无 .md 文件',
	'knowledge.list.empty.cloud': '暂无知识库条目',
	'knowledge.list.searchPlaceholder': '根据文档名称搜索',
	'knowledge.list.empty.search': '未找到匹配的文档',
	'knowledge.list.category.manage': '管理分类',
	'knowledge.list.category.add': '添加分类',
	'knowledge.list.category.rename': '重命名',
	'knowledge.list.category.delete': '删除分类',
	'knowledge.list.category.deleteConfirm':
		'删除后，该分类中的文档将移至「未分类」。',
	'knowledge.list.category.move': '移动到分类',
	'knowledge.list.category.all': '全部',
	'knowledge.list.category.uncategorized': '未分类',
	'knowledge.list.category.empty': '此分类暂无文档。',
	'knowledge.list.category.duplicateName': '分类名称已存在',

	'knowledge.assistant.prompts.polish.title': '润色文档',
	'knowledge.assistant.prompts.polish.desc': '精修字句，提升质感',
	'knowledge.assistant.prompts.summarize.title': '总结文档',
	'knowledge.assistant.prompts.summarize.desc': '提炼要点，掌握核心',
	'knowledge.assistant.prompts.outline.title': '生成目录',
	'knowledge.assistant.prompts.outline.desc': '按标题层级生成可导航目录',
	'knowledge.assistant.prompts.expand.title': '扩写文档',
	'knowledge.assistant.prompts.expand.desc': '充实论述，丰富细节',
	'knowledge.assistant.modes.ai': 'AI 对话',
	'knowledge.assistant.modes.rag': 'RAG 对话',
	'knowledge.assistant.loadingConversation': '正在加载对话…',
	'knowledge.assistant.ragIntro':
		'基于您账号下已入库的知识进行问答！系统将严格限定在您上传的私有数据范围内进行信息检索与推理，不会越界获取互联网公域信息。',
	'knowledge.assistant.aiIntro':
		'Hi，我是您的专属知识库助手！从日常的资料查阅、流程指引，到棘手难题的排查与解答，我都会为您提供即时、精准的信息支持。您可以把我当作随时在线的业务智囊，帮您大幅节省检索时间，提升工作效能。',
	'knowledge.assistant.newConversation': '新对话',
	'knowledge.assistant.scrollToBottom': '滚动到底部',
	'knowledge.assistant.scrollToTop': '滚动到顶部',
	'knowledge.assistant.sessionSaving': '正在保存对话，请稍后再新建对话',
	'knowledge.assistant.sessionSavingViewHistory':
		'正在保存对话，请稍后再查看历史对话',
	'knowledge.assistant.deleteConversationTitle': '删除对话？',
	'knowledge.assistant.deleteConversationDesc':
		'确定要删除该历史对话吗？此操作无法撤销。',
	'knowledge.assistant.conversationNameLabel': '对话名称：「{name}」',
	'knowledge.assistant.conversationFallback': '对话 {id}',
	'knowledge.assistant.editConversationTitle': '编辑标题',
	'knowledge.assistant.titleRequired': '标题不能为空',
	'knowledge.assistant.history': '历史对话',
	'knowledge.assistant.historyEmpty': '暂无历史对话',
	'knowledge.assistant.noBodyToWrite': '没有可写入的正文',
	'knowledge.assistant.appendedToCurrentDoc': '已追加到当前知识文档',
	'knowledge.assistant.tocAlreadyAtTop': '文档开头已有目录，未重复插入',
	'knowledge.assistant.tocPrependedToDoc': '已将生成的目录插入文档顶部',
	'knowledge.assistant.loginToUse': '请先登录后再使用助手',
	'knowledge.assistant.enterBodyFirst': '请先在左侧编辑器输入正文',
	'knowledge.assistant.waitForCurrentReply': '请等待当前回复结束后再试',
	'knowledge.assistant.placeholder.rag': '向知识库提问',
	'knowledge.assistant.placeholder.ai': '请输入您的问题',
	'knowledge.assistant.placeholder.aiNeedsBody':
		'请先在左侧编辑器输入正文后再向我提问',

	'knowledge.assistant.share.selectAll': '全选',
	'knowledge.assistant.share.createLink': '创建分享链接',

	'knowledge.trash.title': '回收站',
	'knowledge.trash.checkbox.select': '选择',
	'knowledge.trash.checkbox.unselect': '取消选择',
	'knowledge.trash.deleteForever': '从回收站彻底删除',
	'knowledge.trash.deletedAt': '删除 {time}',
	'knowledge.trash.deleted': '已从回收站删除',
	'knowledge.trash.detailMissing': '无法获取该条回收站详情',
	'knowledge.trash.selectedCount': '已选 {count} 条',
	'knowledge.trash.batchDeleteFailed': '批量删除失败',
	'knowledge.trash.batchDeleteDone': '批量删除完成',
	'knowledge.trash.batchDeleteDoneMessage': '已删除 {count} 条',
	'knowledge.trash.confirm.batchTitle': '批量删除回收站文件？',
	'knowledge.trash.confirm.singleTitle': '删除回收站文件？',
	'knowledge.trash.confirm.desc': '该操作将从回收站中彻底删除，无法恢复。',
	'knowledge.trash.confirm.count': '数量：{count}',
	'knowledge.trash.confirm.fileName': '文件名称：「{name}」',
	'knowledge.trash.selectAll': '全选',
	'knowledge.trash.selectedRatio': '已选 {selected} / {total}',
	'knowledge.trash.batchDelete': '批量删除',
	'knowledge.trash.loadingDetail': '正在加载...',
	'knowledge.trash.empty': '回收站暂无内容',
	'knowledge.trash.searchPlaceholder': '搜索文档名称',
	'knowledge.trash.empty.search': '未找到匹配的文档',

	'share.header.chatTitle': '分享对话内容',
	'share.header.knowledgeTitle': '分享文章内容',
	'share.header.chatDisclaimer': '该分享内容由 AI 生成，请仔细甄别',
	'share.header.knowledgeDisclaimer': '该文章来自分享，请仔细甄别',
	'share.knowledge.updatedAt': '更新 {time}',
	'share.scroll.toTop': '回到顶部',
	'share.scroll.toBottom': '滚动到底部',
	'share.scroll.ariaToTop': '滚动到顶部',
	'share.scroll.ariaToBottom': '滚动到底部',

	'share.modal.title.session': '创建分享链接',
	'share.modal.title.knowledge': '创建文章分享链接',
	'share.modal.disclaimer':
		'分享链接公开可见，任何人获取后均可查看。请在分享前仔细检查内容，确保不包含敏感信息或隐私数据。',
	'share.modal.copied': '已复制',
	'share.modal.copy': '复制',
	'share.modal.createAndCopy': '创建并复制链接',

	'knowledge.save.fileSaved': '文件已保存',
	'knowledge.save.savedToPath': '已保存到：{path}',
	'knowledge.save.savedToDefaultDir': '已保存到默认目录',
	'knowledge.save.failed': '保存失败',
	'knowledge.save.updateFailedTryLater': '更新知识失败，请稍后重试',
	'knowledge.save.createFailedTryLater': '新建知识失败，请稍后重试',
	'knowledge.save.loginTip':
		'请登录后保存到云端知识库，或使用桌面端保存到本地文件夹。',
	'knowledge.save.noChangesTitle': '暂无修改',
	'knowledge.save.noChangesMessage': '标题与内容与上次保存一致，未执行保存',

	'knowledge.validation.titleRequired': '请先输入文件名「标题」',
	'knowledge.validation.contentRequired': '请先输入内容',

	'knowledge.share.saveBeforeShare': '请先保存到知识库后再分享',

	'knowledge.overwrite.title': '覆盖已有文件？',
	'knowledge.overwrite.desc':
		'当前目录下已存在同名文件「{name}」，确定要覆盖吗？此操作不可撤销。',
	'knowledge.overwrite.saveAsTip': '也可选择「另存为」将文件保存为新文件',
	'knowledge.overwrite.confirm': '覆盖保存',
	'knowledge.overwrite.cancel': '取消保存',
	'knowledge.overwrite.saveAs': '另存为',

	'knowledge.title.unsavedChanges': '有未保存的修改',
	'knowledge.title.document': '知识文档',
	'knowledge.title.placeholder': '请先输入文件名「标题」...',
	'knowledge.title.aria': '知识标题',

	'route.chat.title': '智能对话',
	'route.englishLearning.title': '英语学习',
	'route.englishLearning.import.title': '导入学习资料',
	'route.englishLearning.library.title': '资源库',
	'route.englishLearning.toLibrary': '前往资源库',
	'route.englishLearning.favorites.title': '我的收藏',
	'route.englishLearning.notes.title': '学习笔记',
	'route.englishLearning.mistakes.title': '错题集',
	'route.englishLearning.daily.title': '今日记词',
	'route.englishLearning.review.title': '今日复习',
	'route.englishLearning.stream.title': '拉取结果',
	'route.englishLearning.annotate.title': '标注进度',
	'route.englishLearning.morphology.title': '词根词缀大全',
	'route.englishLearning.grammar.title': '英语语法大全',
	'route.englishLearning.practice.title': '听写与拼写练习',
	'route.englishLearning.practice.classicTitle': '语句听写与拼写',

	'englishLearning.practice.entry': '听写 / 拼写',
	'englishLearning.practice.setupTitle': '单词练习',
	'englishLearning.practice.setupPickMode': '练习模式',
	'englishLearning.practice.setupRoundOptions': '本轮设置',
	'englishLearning.practice.classicSetupTitle': '语句练习',
	'englishLearning.practice.classicDictationHint': '听读音，在下方按词填入英文',
	'englishLearning.practice.classicSpellingPrompt':
		'看中文释义，在下方按词填入英文',
	'englishLearning.practice.classicInputPlaceholder': '输入英文语句',
	'englishLearning.practice.togglePos': '词性（Ctrl+Shift+P）',
	'englishLearning.practice.toggleIpa': '音标（Ctrl+Shift+I）',
	'englishLearning.practice.slotCheck': '提交',
	'englishLearning.practice.sourceClassicFavorites': '经典句收藏',
	'englishLearning.practice.sourceClassicMistakes': '语句错题集',
	'englishLearning.practice.sourceClassicLibrary': '当前语句库',
	'englishLearning.practice.sourceClassicPack': '本次语句包',
	'englishLearning.practice.sourceClassicLive': '当前语句包（进行中）',
	'englishLearning.practice.modeLabel': '练习模式',
	'englishLearning.practice.modeDictation': '听写',
	'englishLearning.practice.modeDictationVocab': '单词听写',
	'englishLearning.practice.modeDictationClassic': '语句听写',
	'englishLearning.practice.modeDictationFit': '适合练习听辨能力与拼写反应速度',
	'englishLearning.practice.modeSpelling': '看中写',
	'englishLearning.practice.modeSpellingVocab': '单词看中写',
	'englishLearning.practice.modeSpellingClassic': '语句看中写',
	'englishLearning.practice.modeSpellingFit':
		'适合练习中英对照记忆与拼写准确度',
	'englishLearning.practice.setupModeDictationHint':
		'听读音后按词填写，作答中默认会自动连续播放三次读音',
	'englishLearning.practice.setupModeSpellingHint':
		'看中文释义写拼写，作答中默认不播放读音，可手动播放',
	'englishLearning.practice.shortcuts.triggerAria': '查看练习快捷键',
	'englishLearning.practice.shortcuts.title': '键盘快捷键',
	'englishLearning.practice.shortcuts.sectionPrompt': '作答中',
	'englishLearning.practice.shortcuts.sectionSoftWrong': '首次答错',
	'englishLearning.practice.shortcuts.sectionRevealed': '看答案后',
	'englishLearning.practice.shortcuts.check': '检查答案',
	'englishLearning.practice.shortcuts.play': '播放 / 停止',
	'englishLearning.practice.shortcuts.togglePos': '显示 / 隐藏词性',
	'englishLearning.practice.shortcuts.toggleIpa': '显示 / 隐藏音标',
	'englishLearning.practice.shortcuts.showAnswer': '看答案',
	'englishLearning.practice.shortcuts.previous': '上一题',
	'englishLearning.practice.shortcuts.retry': '再试一次',
	'englishLearning.practice.shortcuts.next': '下一题',
	'englishLearning.practice.shortcuts.footnote':
		'首次答错与看答案后：焦点在输入框内时不响应方向键；↑ 上一题仅非首题可用。全程 Shift + 空格 播放/停止。经典句 Ctrl+Shift+P / Ctrl+Shift+I 切换词性与音标。',
	'englishLearning.practice.shortcuts.keyEnter': 'Enter 键',
	'englishLearning.practice.shortcuts.keyShiftSpace': 'Shift + 空格',
	'englishLearning.practice.shortcuts.keySpace': '空格',
	'englishLearning.practice.shortcuts.keyCtrlShiftP': 'Ctrl + Shift + P',
	'englishLearning.practice.shortcuts.keyCtrlShiftI': 'Ctrl + Shift + I',
	'englishLearning.practice.shortcuts.keyLeft': '左方向键',
	'englishLearning.practice.shortcuts.keyRight': '右方向键',
	'englishLearning.practice.shortcuts.keyUp': '上方向键',
	'englishLearning.practice.shortcuts.keyDown': '下方向键',
	'englishLearning.practice.sourceLabel': '词表来源',
	'englishLearning.practice.sourceResolving': '加载中…',
	'englishLearning.practice.sourceFavorites': '我的收藏',
	'englishLearning.practice.sourceLibrary': '当前资源库',
	'englishLearning.practice.sourcePack': '本次词包',
	'englishLearning.practice.sourceLive': '当前词包（进行中）',
	'englishLearning.practice.sourceFixed': '词表：{label}',
	'englishLearning.practice.countLabel': '题目数量',
	'englishLearning.practice.orderLabel': '出题顺序',
	'englishLearning.practice.orderRandom': '随机',
	'englishLearning.practice.orderRandomHint': '每次开始都从打乱词表抽题',
	'englishLearning.practice.orderRandomFit': '适合查漏补缺，不依赖固定顺序',
	'englishLearning.practice.orderSequential': '顺序',
	'englishLearning.practice.orderSequentialHint':
		'按词表原有顺序出题，多次不变',
	'englishLearning.practice.orderSequentialFit': '适合按章节系统推进',
	'englishLearning.practice.start': '开始练习',
	'englishLearning.practice.loadingWords': '加载词表…',
	'englishLearning.practice.emptyPool': '请先选择对应的词库/收藏/历史记录',
	'englishLearning.practice.emptyTitle': '没有可用的单词',
	'englishLearning.practice.loadFailed': '加载词表失败',
	'englishLearning.practice.progress': '{current}/{total}',
	'englishLearning.practice.exit': '结束练习',
	'englishLearning.practice.dictationHint': '听读音，在下方按词填入英文',
	'englishLearning.practice.dictationStepListen': '听音',
	'englishLearning.practice.dictationStepSpell': '拼写',
	'englishLearning.practice.playAgain': '再听一遍',
	'englishLearning.practice.spellingPrompt': '看中文释义，在下方输入英文拼写',
	'englishLearning.practice.inputLabel': '你的拼写',
	'englishLearning.practice.inputPlaceholder': '输入英文单词',
	'englishLearning.practice.check': '检查',
	'englishLearning.practice.tryAgain': '再试一次',
	'englishLearning.practice.showAnswer': '看答案',
	'englishLearning.practice.softWrongHint':
		'可再试一次，也可以查看正确答案后继续',
	'englishLearning.practice.previous': '上一题',
	'englishLearning.practice.next': '下一题',
	'englishLearning.practice.viewResults': '查看练习结果',
	'englishLearning.practice.incorrect': '拼写错误',
	'englishLearning.practice.correct': '回答正确',
	'englishLearning.practice.hintShow': '展开提示',
	'englishLearning.practice.hintHide': '收起提示',
	'englishLearning.practice.hintUnavailable': '本题暂无额外提示',
	'englishLearning.practice.hintLabelIpa': '音标',
	'englishLearning.practice.hintLabelSegmentation': '分词',
	'englishLearning.practice.hintLabelTranslation': '中文释义',
	'englishLearning.practice.hintLabelSource': '出处',
	'englishLearning.practice.hintLabelNote': '说明',
	'englishLearning.practice.hintLabelExample': '例句',
	'englishLearning.practice.yourAnswer': '你的答案：{answer}',
	'englishLearning.practice.yourAnswerLabel': '你的答案',
	'englishLearning.practice.correctAnswer': '正确答案',
	'englishLearning.practice.summaryTitle': '练习报告',
	'englishLearning.practice.summaryScore': '正确 {correct} / {total}',
	'englishLearning.practice.summaryAccuracy': '正确率',
	'englishLearning.practice.summaryAccuracyRound': '本轮正确率',
	'englishLearning.practice.summaryAccuracyOverall': '总正确率',
	'englishLearning.practice.summaryStatCorrect': '正确',
	'englishLearning.practice.summaryStatWrong': '错误',
	'englishLearning.practice.summaryStatTotal': '本轮练习数',
	'englishLearning.practice.summaryStatPracticed': '已练习数',
	'englishLearning.practice.wrongListTitle': '错题',
	'englishLearning.practice.roundWordListTitle': '作答明细',
	'englishLearning.practice.sessionRoundLabel': '第 {n} 轮',
	'englishLearning.practice.roundWordListWrongCount': '错误 {count}',
	'englishLearning.practice.roundWordListCorrectCount': '正确 {count}',
	'englishLearning.practice.retryWrong': '重练错题',
	'englishLearning.practice.practiceAgain': '重新设置',
	'englishLearning.practice.continuePractice': '继续练习',
	'englishLearning.practice.continueEmpty': '没有更多未练过的单词了',
	'englishLearning.practice.back': '返回',
	'englishLearning.practice.saveMistakes': '加入错题集',
	'englishLearning.practice.saveMistakesSuccessTitle': '保存错题集成功',
	'englishLearning.practice.saveMistakesSuccess':
		'新加入 {added} 条，更新错拼 {updated} 条，{skipped} 条未变更',
	'englishLearning.practice.saveMistakesAllSkipped':
		'本轮错题已在错题集中，且错拼与上次相同',
	'englishLearning.practice.saveMistakesFailed': '保存错题集失败',
	'englishLearning.practice.sourceMistakes': '错题集',
	'englishLearning.practice.sourceDailyMemorize': '记词记录',
	'englishLearning.practice.sourceReview': '今日复习（单词）',
	'englishLearning.practice.sourceClassicReview': '今日复习（语句）',
	'englishLearning.practice.continueReview': '继续复习',
	'englishLearning.practice.continueReviewEmpty': '今日没有更多待复习项了',
	'englishLearning.practice.reviewRecordFailed': '复习进度保存失败，请稍后重试',
	'englishLearning.practice.reportSaveLabel': '报告保存',
	'englishLearning.practice.startLabel': '确认设置后点击「开始练习」进行练习吧',
	'englishLearning.practice.reportSaveManual': '手动保存',
	'englishLearning.practice.reportSaveManualHint': '结算页点保存，不点则不留档',
	'englishLearning.practice.reportSaveManualFit': '不想每场都进历史时选',
	'englishLearning.practice.reportSaveAuto': '自动保存',
	'englishLearning.practice.reportSaveAutoHint': '进入结算即写入报告，无需再点',
	'englishLearning.practice.reportSaveAutoFit': '想随手留档回看时选',
	'englishLearning.practice.saveReport': '保存报告',
	'englishLearning.practice.reportSaved': '已保存',
	'englishLearning.practice.saveReportFailed': '报告保存失败，请重试',
	'englishLearning.practice.viewReports': '查看报告',
	'englishLearning.practice.reportsTitle': '练习报告',
	'englishLearning.practice.reportsHomeDesc': '查看单词和语句的练习报告',
	'englishLearning.practice.reportsVocabNav': '单词练习报告',
	'englishLearning.practice.reportsClassicNav': '语句练习报告',
	'englishLearning.practice.reportsEmpty': '还没有保存过的练习报告',
	'englishLearning.practice.reportsLoadFailed': '报告列表加载失败',
	'englishLearning.practice.reportsLoadMore': '加载更多',
	'englishLearning.practice.reportsLoadingMore': '加载中…',
	'englishLearning.practice.reportsPctSuffix': '%',
	'englishLearning.practice.reportDetailLoadFailed': '报告加载失败',
	'englishLearning.practice.reportDetailMissing': '未找到该报告',
	'englishLearning.practice.reportsSelectAll': '全选',
	'englishLearning.practice.reportsRemoveSelected': '删除所选 {count} 项',
	'englishLearning.practice.reportsRemoving': '删除中…',
	'englishLearning.practice.reportsRemoveNoneHint': '请先勾选要删除的报告',
	'englishLearning.practice.reportsRemoveBatchConfirmTitle':
		'确认删除所选报告？',
	'englishLearning.practice.reportsRemoveBatchConfirmDesc':
		'将删除已勾选的 {count} 份练习报告，此操作不可撤销。',
	'englishLearning.practice.reportsRemoveConfirmTitle': '删除练习报告',
	'englishLearning.practice.reportsRemoveConfirmDesc':
		'确定删除「{title}」？此操作不可撤销。',
	'englishLearning.practice.reportsRemoveConfirmAction': '确认删除',
	'englishLearning.practice.reportsRemoveBatchSuccess': '已删除所选报告',
	'englishLearning.practice.reportsRemoveSuccess': '已删除该报告',
	'englishLearning.practice.reportsRemoveFail': '删除失败，请稍后重试',
	'englishLearning.practice.reportsRemoveAction': '删除报告',
	'englishLearning.practice.reportsToggleRow': '选择该报告',
	'englishLearning.practice.reportKindVocab': '词汇',
	'englishLearning.practice.reportKindClassic': '语句',
	'englishLearning.practice.reportModeDictation': '听写',
	'englishLearning.practice.reportModeSpelling': '拼写',
	'englishLearning.practice.reportSourceFallback': '练习',
	'englishLearning.practice.reportRetrySuffix': '重练错题',
	'route.englishLearning.practice.reportsTitle': '练习报告',

	'englishLearning.daily.loading': '准备今日词卡…',
	'englishLearning.daily.pendingCount': '待学词数 {count} 词',
	'englishLearning.daily.sidebarDesc': '听一听、选一选，约 {count} 个词',
	'englishLearning.daily.sidebarDescSplit':
		'复习 {dueCount} 词 · 词库 {libraryCount} 词',
	'englishLearning.daily.sidebarDescLibrary': '每天几分钟，轻松记住几个词',
	'englishLearning.daily.sidebarDescLibraryEmpty': '词库暂无未学词',
	'englishLearning.daily.sidebarStatsLabel': '记词进度',
	'englishLearning.daily.sidebarStatMemorized': '已记词',
	'englishLearning.daily.sidebarStatPending': '未记词',
	'englishLearning.daily.wordsPerRoundTrigger': '每轮数量',
	'englishLearning.daily.wordsPerRoundTitle': '每轮词数',
	'englishLearning.daily.wordsPerRoundDesc':
		'与单词练习题量一致，下次开始记词时生效',
	'englishLearning.daily.resetLibrary': '重置记词',
	'englishLearning.daily.resetting': '重置中…',
	'englishLearning.daily.resetConfirmTitle': '重置词汇库记词进度？',
	'englishLearning.daily.resetConfirmDesc':
		'将清除 {count} 条记词记录与今日复习进度，并仅移除这些词中由今日记词加入的错题；练习等其它来源的错题会保留。之后可重新从词库随机记词。',
	'englishLearning.daily.resetConfirmAction': '确认重置',
	'englishLearning.daily.resetSuccess': '已重置 {count} 条记词记录',
	'englishLearning.daily.resetSuccessGuest': '已重置本地记词进度',
	'englishLearning.daily.resetFailed': '重置失败，请稍后重试',
	'englishLearning.daily.memorizedLink': '记词记录',
	'englishLearning.daily.recordsTitle': '记词记录',
	'englishLearning.daily.recordsLoading': '加载记词记录…',
	'englishLearning.daily.recordsEmpty': '还没有词汇库随机练过的单词',
	'englishLearning.daily.recordsLoadFailed': '记词记录加载失败',
	'englishLearning.daily.sectionLabel': '今日词卡',
	'englishLearning.daily.startShort': '开始 {count} 词',
	'englishLearning.daily.introTitle': '每天几分钟，轻松记住几个词',
	'englishLearning.daily.introDesc': '约 {count} 词：听发音、看释义、四选一',
	'englishLearning.daily.introDescDictation': '约 {count} 词：听发音后拼写英文',
	'englishLearning.daily.introDescSpelling':
		'约 {count} 词：看中文释义拼写英文',
	'englishLearning.daily.introDescSplit':
		'复习 {dueCount} 词 · 词库可随机 {libraryCount} 词',
	'englishLearning.daily.introHint': '不用拼写，可到「今日复习」巩固',
	'englishLearning.daily.introHintDictation': '听音拼写，对错计入记词进度',
	'englishLearning.daily.introHintSpelling': '看义拼写，对错计入记词进度',
	'englishLearning.daily.modeRecognition': '认读',
	'englishLearning.daily.modeDictation': '听写',
	'englishLearning.daily.modeSpelling': '看中写',
	'englishLearning.daily.studyHintDictation': '先听一听单词发音',
	'englishLearning.daily.studyHintSpelling': '先看中文释义，准备拼写',
	'englishLearning.daily.spellHintDictation': '根据发音拼写出英文单词',
	'englishLearning.daily.startSpell': '开始拼写',
	'englishLearning.daily.sessionTitle': '今日记词',
	'englishLearning.daily.sessionTitleReview': '今日复习',
	'englishLearning.daily.sessionTitleLibrary': '认读记词',
	'englishLearning.daily.studyLabel': '先听一听、看一看',
	'englishLearning.daily.quizLabel': '选一选释义',
	'englishLearning.daily.quizHint': '哪个是正确的中文释义？',
	'englishLearning.daily.hintLabelWord': '单词',
	'englishLearning.daily.start': '开始今日 {count} 词',
	'englishLearning.daily.startReview': '复习待背词（{count}）',
	'englishLearning.daily.startLibrary': '开始记词',
	'englishLearning.daily.startDictation': '开始听写',
	'englishLearning.daily.startSpelling': '开始看中写',
	'englishLearning.daily.guestHint':
		'未登录时使用内置高频词；登录后可同步收藏与复习进度。',
	'englishLearning.daily.startQuiz': '考考我',
	'englishLearning.daily.quizPrompt': '「{word}」的意思是？',
	'englishLearning.daily.feedbackCorrect': '很好，记住了！',
	'englishLearning.daily.feedbackWrong': '没关系，下次会更熟',
	'englishLearning.daily.nextWord': '下一个',
	'englishLearning.daily.finish': '完成今日学习',
	'englishLearning.daily.doneTitle': '今天先到这里',
	'englishLearning.daily.doneDesc':
		'到期后可在「今日复习」听写巩固，系统会安排复习日程。',
	'englishLearning.daily.doneAllCorrect': '本轮全部答对，很棒！',
	'englishLearning.daily.doneWrongTitle': '本轮错词（{count}）',
	'englishLearning.daily.backHome': '返回英语学习',
	'englishLearning.daily.shortcuts.triggerAria': '查看记词快捷键',
	'englishLearning.daily.shortcuts.sectionStudy': '认读学习',
	'englishLearning.daily.shortcuts.sectionQuiz': '作答中',
	'englishLearning.daily.shortcuts.sectionFeedback': '反馈后',
	'englishLearning.daily.shortcuts.startQuiz': '开始测验',
	'englishLearning.daily.shortcuts.next': '下一词 / 完成',

	'englishLearning.review.homeDesc': '今日待复习 {count} 项',
	'englishLearning.review.loadingDue': '加载待复习…',
	'englishLearning.review.vocabNav': '单词复习 ({count})',
	'englishLearning.review.classicNav': '语句复习 ({count})',
	'englishLearning.review.vocabTab': '单词复习',
	'englishLearning.review.classicTab': '语句复习',
	'englishLearning.review.empty': '今日暂无待复习条目',
	'englishLearning.review.exportDocxEmpty': '暂无待复习可导出',

	'englishLearning.mistakes.desc': '练习中拼写错误的单词，可反复听写巩固',
	'englishLearning.mistakes.homeDesc': '查看单词和语句的错题集',
	'englishLearning.mistakes.vocabNav': '单词错题集',
	'englishLearning.mistakes.classicNav': '语句错题集',
	'englishLearning.mistakes.classicEmpty':
		'暂无语句错题，完成语句练习后可在结算页加入错题集',
	'englishLearning.mistakes.classicListLoadFailed': '加载语句错题集失败',
	'englishLearning.mistakes.classicRemoveConfirmDesc':
		'确定将「{english}」移出错题集吗？',
	'englishLearning.mistakes.empty': '暂无错题，完成练习后可在结算页加入错题集',
	'englishLearning.mistakes.listLoadFailed': '加载错题集失败',
	'englishLearning.mistakes.lastInput': '你的错拼：{answer}',
	'englishLearning.mistakes.removeAction': '移出错题集',
	'englishLearning.mistakes.removeConfirmTitle': '移出错题集',
	'englishLearning.mistakes.removeConfirmDesc':
		'确定将「{word}」从错题集中移除？',
	'englishLearning.mistakes.removeSuccess': '已移出错题集',
	'englishLearning.mistakes.selectAllLoaded': '全选',
	'englishLearning.mistakes.exportDocxEmpty': '暂无错题可导出',
	'englishLearning.mistakes.removeSelected': '删除所选 {count} 项',
	'englishLearning.mistakes.removing': '移除中...',
	'englishLearning.mistakes.removeBatchSuccess': '已移除所选错题',
	'englishLearning.mistakes.removeFail': '移除失败，请稍后重试',
	'englishLearning.mistakes.removeBatchConfirmTitle': '确认移除所选错题？',
	'englishLearning.mistakes.removeBatchConfirmDesc':
		'将移除已勾选的 {count} 条错题，此操作不可撤销。',
	'englishLearning.mistakes.removeConfirmAction': '确认移除',
	'englishLearning.mistakes.removeNoneHint': '请先勾选要移除的条目',
	'englishLearning.mistakes.toggleRow': '选择该行',

	'englishLearning.source.morphologyLink': '词根词缀',
	'englishLearning.source.grammarLink': '语法大全',
	'englishLearning.reference.morphology.pageTitle': '词根词缀大全',
	'englishLearning.reference.morphology.tabPrefixes': '前缀',
	'englishLearning.reference.morphology.tabSuffixes': '后缀',
	'englishLearning.reference.morphology.tabRoots': '词根',
	'englishLearning.reference.grammar.empty': '暂无语法条目',

	'englishLearning.stream.back': '返回英语学习',
	'englishLearning.stream.kindTabsAria': '拉取结果类型',
	'englishLearning.stream.vocab.nav': '单词',
	'englishLearning.stream.classic.nav': '语句',
	'englishLearning.stream.vocab.pageTitle': '单词拉取结果',
	'englishLearning.stream.classic.pageTitle': '语句拉取结果',
	'englishLearning.stream.topicLabel': '主题',
	'englishLearning.stream.openLivePage': '实时列表',
	'englishLearning.stream.liveSummaryCount':
		'已拉取 {count} 条，可在独立页实时查看',
	'englishLearning.stream.liveSummaryProgress':
		'拉取中 {collected} / {target} 条，可在独立页实时查看',
	'englishLearning.stream.vocab.empty':
		'暂无单词。请在英语学习页填写主题并点击「生成单词」，或从历史记录载入。',
	'englishLearning.stream.classic.empty':
		'暂无语句。请在英语学习页填写主题并点击「生成语句」，或从历史记录载入。',

	'englishLearning.import.titleVocab': '导入单词（JSON）',
	'englishLearning.import.titleClassic': '导入经典语句（JSON）',
	'englishLearning.import.back': '返回英语学习',
	'englishLearning.import.selectFile': '选择 JSON 文件',
	'englishLearning.import.hintVocab':
		'支持顶层为数组，或含 items 数组的对象。每项至少需要 word 与 ipa，可选 pos、segmentation（音节划分）、translationZh、example。数据示例如下：',
	'englishLearning.import.hintClassic':
		'支持顶层为数组，或含 items 数组的对象。每项至少需要 english 与 translationZh，可选 source、noteZh。数据示例如下：',
	'englishLearning.import.preview': 'JSON 预览',
	'englishLearning.import.parseError': '无法解析为合法 JSON',
	'englishLearning.import.readError': '读取文件失败',
	'englishLearning.import.validateError': '结构校验未通过：',
	'englishLearning.import.err.expect-array': '需要为数组或 { "items": [...] }',
	'englishLearning.import.err.no-vocab':
		'未解析到有效单词（每项需含非空 word 与 ipa）',
	'englishLearning.import.err.no-classic':
		'未解析到有效语句（每项需含非空 english 与 translationZh）',
	'englishLearning.import.parsedCount': '已解析 {count} 条',
	'englishLearning.import.apply': '导入到左侧列表',
	'englishLearning.import.applySuccess': '已写入左侧列表',
	'englishLearning.import.dropReject': '请拖拽或选择 .json 文件',
	'englishLearning.import.titlePlaceholder': '请先输入标题...',
	'englishLearning.import.saveToVocab': '保存到单词库',
	'englishLearning.import.saveToClassic': '保存到经典语句库',
	'englishLearning.import.titleRequired': '请先输入标题',
	'englishLearning.import.needParsed': '请先上传并成功解析 JSON 后再保存',
	'englishLearning.import.reupload': '重新上传',
	'englishLearning.import.saveVocabSuccess': '已将 {count} 个单词保存至单词库',
	'englishLearning.import.saveVocabLoading': '保存中...',
	'englishLearning.import.saveClassicSuccess':
		'已将 {count} 条经典语句保存至语句库',
	'englishLearning.import.saveClassicLoading': '保存中...',
	'englishLearning.import.dataExample': '展开导入单词 json 示例',
	'englishLearning.import.dataExampleClassic': '展开导入语句 json 示例',
	'englishLearning.import.dataExampleCollapse': '收起导入单词 json 示例',
	'englishLearning.import.dataExampleClassicCollapse': '收起导入语句 json 示例',
	'englishLearning.import.formatGuide': '数据格式示例',
	'englishLearning.import.topHintVocab': '拖拽或点击上传 .json 单词包文件',
	'englishLearning.import.topHintClassic': '拖拽或点击上传 .json 经典语句文件',

	'englishLearning.practice.favorites': '收藏',
	'englishLearning.intentSection': '选择意图',
	'englishLearning.library.vocab.title': '单词库',
	'englishLearning.library.vocab.descShort': '可导入单词，查看词汇库',
	'englishLearning.library.vocab.bank': '词汇库',
	'englishLearning.library.classic.title': '语句库',
	'englishLearning.library.classic.descShort': '可导入语句，查看语料库',
	'englishLearning.library.classic.bank': '语料库',
	'englishLearning.library.listHint': '选择左侧资源包查看词条',
	'englishLearning.library.listLoading': '加载资源列表…',
	'englishLearning.library.listEmpty': '暂无已导入的单词库',
	'englishLearning.library.listEmptyClassic': '暂无已导入的语句库',
	'englishLearning.library.deleteAction': '删除单词库',
	'englishLearning.library.deleteActionClassic': '删除语句库',
	'englishLearning.annotateSource.action': '标注',
	'englishLearning.annotateSource.menuOnline': '在线标注',
	'englishLearning.annotateSource.menuImport': '手动导入',
	'englishLearning.annotateSource.importBlockedRunning':
		'在线标注进行中，请先暂停或停止后再手动导入',
	'englishLearning.annotateSource.importSuccess':
		'已导入 {accepted} 句（可为部分标注）；跳过 {skipped}，覆盖 {overwritten}',
	'englishLearning.annotateSource.importAllSkipped':
		'没有可写入的标注（跳过 {skipped}）',
	'englishLearning.annotateSource.importClassicFile':
		'这是语句文件，不是标注文件',
	'englishLearning.annotateSource.importEmpty': '文件中没有有效标注',
	'englishLearning.annotateSource.importInvalid': '标注文件格式不正确',
	'englishLearning.annotateSource.importFailed': '导入失败，请稍后重试',
	'englishLearning.annotateSource.confirmTitle': '整集词标注预热',
	'englishLearning.annotateSource.confirmDesc':
		'将为本集合中尚未缓存的句子调用模型生成词性 / 音标 / 释义并写入缓存，可能较久，确认继续？',
	'englishLearning.annotateSource.confirmDescCount':
		'约 {count} 条句子：尚未缓存的将调用模型生成词性 / 音标 / 释义并写入缓存，可能较久，确认继续？',
	'englishLearning.annotateSource.confirmAction': '开始标注',
	'englishLearning.annotateSource.running': '标注中…',
	'englishLearning.annotateSource.preparing': '正在连接并筛缓存…',
	'englishLearning.annotateSource.cancelRunning': '停止',
	'englishLearning.annotateSource.progress':
		'已缓存 {hit} · 新标 {annotated}/{miss} · 剩余 {remaining}',
	'englishLearning.annotateSource.aborted': '已停止标注（已完成部分仍保留）',
	'englishLearning.annotateSource.failed': '标注失败，请稍后重试',
	'englishLearning.annotateSource.success':
		'标注完成：共 {total}，已有 {hit}，新写 {annotated}，失败 {failed}',
	'englishLearning.annotateTasks.pageTitle': '标注进度',
	'englishLearning.annotateTasks.back': '返回',
	'englishLearning.annotateTasks.pageRunningHint':
		'正在进行 {count} 个标注任务，可随时停止',
	'englishLearning.annotateTasks.pageIdleHint':
		'下方为最近标注任务；离开本页仍继续，刷新后可点「继续」',
	'englishLearning.annotateTasks.empty': '暂无标注任务',
	'englishLearning.annotateTasks.clearFinished': '清除已结束',
	'englishLearning.annotateTasks.dismiss': '移除',
	'englishLearning.annotateTasks.statusRunning': '进行中',
	'englishLearning.annotateTasks.statusDone': '已完成',
	'englishLearning.annotateTasks.statusPaused': '已暂停',
	'englishLearning.annotateTasks.statusAborted': '已停止',
	'englishLearning.annotateTasks.statusError': '失败',
	'englishLearning.annotateTasks.statusPartialFailed': '部分失败',
	'englishLearning.annotateTasks.resume': '继续',
	'englishLearning.annotateTasks.sourceLibrary': '语句库',
	'englishLearning.annotateTasks.sourcePack': 'Pack 会话',
	'englishLearning.annotateTasks.metricTotal': '去重句数',
	'englishLearning.annotateTasks.metricHit': '已有缓存',
	'englishLearning.annotateTasks.metricAnnotated': '新写入',
	'englishLearning.annotateTasks.metricFailed': '失败',
	'englishLearning.annotateTasks.metricTokens': 'Token 消耗',
	'englishLearning.annotateTasks.metricTokensValue':
		'{total}（入 {prompt} · 出 {completion}）',
	'englishLearning.annotateTasks.metricTokensDetail':
		'本任务累计：prompt {prompt} + completion {completion}',
	'englishLearning.annotateTasks.openPage': '打开标注进度',
	'englishLearning.annotateTasks.viewProgress': '查看进度',
	'englishLearning.annotateTasks.liveSummary': '{count} 个标注进行中 · {title}',
	'englishLearning.annotateTasks.liveFinished': '最近 {count} 个标注任务',
	'englishLearning.annotateTasks.alreadyRunning':
		'该集合已在标注，已打开进度页',
	'englishLearning.annotateTasks.alreadyActive':
		'该集合已有标注任务，已打开进度页（暂停中可点继续）',
	'englishLearning.annotateTasks.untitledLibrary': '未命名语句库',
	'englishLearning.annotateTasks.untitledPack': '未命名 Pack',
	'englishLearning.library.deleteConfirmTitle': '确认删除单词库？',
	'englishLearning.library.deleteConfirmTitleClassic': '确认删除语句库？',
	'englishLearning.library.deleteConfirmDesc':
		'将删除「{title}」及其中的 {count} 个单词，此操作不可恢复。',
	'englishLearning.library.deleteConfirmDescClassic':
		'将删除「{title}」及其中的 {count} 条语句，此操作不可恢复。',
	'englishLearning.library.deleteConfirmAction': '删除',
	'englishLearning.library.deleting': '删除中…',
	'englishLearning.library.deleteSuccess': '单词库已删除',
	'englishLearning.library.deleteSuccessClassic': '语句库已删除',
	'englishLearning.library.editAction': '编辑单词库',
	'englishLearning.library.editActionClassic': '编辑语句库',
	'englishLearning.library.publicBadge': '公共',
	'englishLearning.library.visibilityDialogTitle': '编辑单词库',
	'englishLearning.library.visibilityDialogTitleClassic': '编辑语句库',
	'englishLearning.library.visibilityLibrarySummary':
		'「{title}」共 {count} 个单词。',
	'englishLearning.library.visibilityLibrarySummaryClassic':
		'「{title}」共 {count} 条语句。',
	'englishLearning.library.editTitleLabel': '资源库名称',
	'englishLearning.library.editTitlePlaceholder': '请输入资源库名称',
	'englishLearning.library.editTitleRequired': '名称不能为空',
	'englishLearning.library.editTitleReadonlyHint': '仅创建者可修改名称',
	'englishLearning.library.editSaveSuccess': '资源库标题更新成功',
	'englishLearning.library.visibilityPublicLabel': '设为公共资源库',
	'englishLearning.library.visibilityPublicHelp':
		'开启后所有登录用户都能在资源库中看到并使用；关闭后仅创建者可见。',
	'englishLearning.library.visibilitySaveAction': '保存',
	'englishLearning.library.visibilitySaving': '保存中…',
	'englishLearning.library.visibilitySaveSuccess': '资源库可见性已更新',
	'englishLearning.library.goImport': '前往导入',
	'englishLearning.library.selectLibrary': '请从左侧选择一个单词库',
	'englishLearning.library.selectLibraryClassic': '请从左侧选择一个语句库',
	'englishLearning.library.wordsHeading': '共 {count} 个单词',
	'englishLearning.library.quotesHeading': '共 {count} 条语句',
	'englishLearning.library.listCount': '共 {count} {type}',
	'englishLearning.library.wordsLoading': '加载单词…',
	'englishLearning.library.wordsLoadFailed': '加载词条失败，请稍后重试',
	'englishLearning.library.wordsLoadMoreFailed':
		'加载更多失败，请继续下滑或稍后重试',
	'englishLearning.resume.settingsAria': '阅读进度设置',
	'englishLearning.resume.settingsLabel': '设置',
	'englishLearning.resume.clearAction': '清除阅读进度',
	'englishLearning.resume.disableAction': '关闭阅读进度',
	'englishLearning.resume.enableAction': '开启阅读进度',
	'englishLearning.resume.clearConfirmTitle': '清除阅读进度？',
	'englishLearning.resume.clearConfirmDesc':
		'将删除已保存的阅读位置，列表将从开头重新加载。',
	'englishLearning.resume.clearConfirmAction': '清除',
	'englishLearning.resume.clearSuccess': '阅读进度已清除',
	'englishLearning.resume.clearFailed': '清除阅读进度失败',
	'englishLearning.resume.disabledToast': '已关闭阅读进度记录',
	'englishLearning.resume.enabledToast': '已开启阅读进度记录',
	'englishLearning.resume.loginRequired': '请先登录后再修改阅读进度设置',
	'englishLearning.resume.toggleFailed': '更新阅读进度设置失败',
	'englishLearning.library.quotesLoading': '加载语句…',
	'englishLearning.favorites.bank': '收藏',
	'englishLearning.favorites.sidebarTitle': '收藏分类',
	'englishLearning.favorites.listHint': '选择分类查看收藏记录',
	'englishLearning.favorites.vocab.nav': '单词收藏',
	'englishLearning.favorites.classic.nav': '语句收藏',
	'englishLearning.favorites.desc': '查看收藏的单词或语句包',
	'englishLearning.notes.desc': '用插件记录学习收获',
	'englishLearning.notes.nav': '打开学习笔记',
	'englishLearning.notes.popout': '独立窗口打开',
	'englishLearning.favorites.toFavorites': '前往我的收藏',
	'englishLearning.favorites.listLoadFailed': '加载收藏失败，请稍后重试',
	'englishLearning.favorites.listLoadMoreFailed':
		'加载更多失败，请继续下滑重试',

	'englishLearning.toolbarSubtitle': '左侧选主题与单词，右侧对话练习',
	'englishLearning.toolbarSubtitleShort': '可选快捷意图，或直接在右侧对话',
	'englishLearning.quickIntents': '快捷意图',
	'englishLearning.quickIntentsCollapse': '收起快捷意图',
	'englishLearning.quickIntentsExpand': '展开快捷意图',
	'englishLearning.intro':
		'左侧点选「快捷意图」可设定本轮学习方向，涵盖单词背诵、词根词缀、固定搭配、易混辨析、发音规律、口语演练、翻译对照、短文精读、名著导读与语法讲解；再次点击同一项可取消，选中内容会在你下次发送时自动附带。',
	'englishLearning.disclaimer':
		'翻译与讲解仅供学习参考，重要文件请人工校对；名著导读遵守版权，长文以摘要与讨论为主。',
	'englishLearning.newChat': '新对话',
	'englishLearning.loading': '加载中…',
	'englishLearning.assistant.scrollToBottom': '滚动到底部',
	'englishLearning.assistant.scrollToTop': '滚动到顶部',
	'englishLearning.list.scrollToBottom': '滚动到底部',
	'englishLearning.list.scrollToTop': '滚动到顶部',
	'englishLearning.placeholder':
		'输入英语或中文，例如要查的词、要翻译的段落、想读的主题…',
	'englishLearning.chip.vocabulary': '单词背诵',
	'englishLearning.chip.morphology': '词根词缀',
	'englishLearning.chip.collocations': '固定搭配',
	'englishLearning.chip.confusables': '易混辨析',
	'englishLearning.chip.pronunciation': '发音规律',
	'englishLearning.chip.speaking': '口语演练',
	'englishLearning.chip.translate': '翻译对照',
	'englishLearning.chip.reading': '短文精读',
	'englishLearning.chip.literature': '名著导读',
	'englishLearning.chip.grammar': '语法讲解',
	'englishLearning.intent.vocabulary':
		'### 意图：请侧重词汇：释义、搭配与例句；如需可先了解我的大致基础。',
	'englishLearning.intent.morphology':
		'### 意图：请从词根、前缀、后缀与构词法入手，说明常见变形规律并各举一些代表词。',
	'englishLearning.intent.collocations':
		'### 意图：请侧重地道搭配与语块：给出常用搭配、简要辨析近义搭配，并各配短例句。',
	'englishLearning.intent.confusables':
		'### 意图：请对比易混词或近义表达：差异、典型误用与正例，可用表格或分条呈现。',
	'englishLearning.intent.pronunciation':
		'### 意图：请侧重发音与节律：重音、弱读、连读或同化等规律，可给 IPA 与跟读提示。',
	'englishLearning.intent.speaking':
		'### 意图：请侧重口语交际与流利度：情景对话、地道应答、常用话轮与填充语；给短对话范例并说明语境正式度。',
	'englishLearning.intent.translate':
		'### 意图：请提供中英对照翻译，可分句；标注专有名词取舍。',
	'englishLearning.intent.reading': '### 意图：请生成一篇精读短文并注释难点。',
	'englishLearning.intent.literature':
		'### 意图：请做名著/英语文献导读：摘要、背景与讨论题；并请从原文中摘取不少于 5 段优质段落（每段适度篇幅），逐段附带中文翻译；避免无关的冗长照抄。',
	'englishLearning.intent.grammar':
		'### 意图：请讲解下列语法点并给出正例与常见错误。',
	'englishLearning.pendingIntent': '已选意图（下次发送时附带）',
	'englishLearning.pendingIntentShort': '下次发送附带',

	'englishLearning.agentTool.internet': '联网搜索',
	'englishLearning.agentTool.knowledge': '知识库检索',
	'englishLearning.agentTool.date': '获取日期',
	'englishLearning.agentTool.other': '{name}',
	'englishLearning.agentTool.querySuffix': '：{q}',
	'englishLearning.agentTool.statusDoing': '资料助手 · {action} 进行中{detail}',
	'englishLearning.agentTool.statusDone': '资料助手 · {action} 已完成',
	'englishLearning.webSearch.viewPages': '联网检索网页 {n} 条',
	'englishLearning.webSearch.viewWebPages': '检索网页',
	'englishLearning.webSearch.viewPagesTitle': '本次联网搜索结果 {n} 条',
	'englishLearning.packHistory.webSearchRounds': '联网 {n} 次',
	'englishLearning.packHistory.deleteAction': '删除记录',
	'englishLearning.packHistory.deleteConfirmTitle': '删除生成记录？',
	'englishLearning.packHistory.deleteConfirmDesc':
		'将删除「{topic}」及其已拉取的 {count} 个单词，不可恢复。',
	'englishLearning.packHistory.deleteConfirmDescClassic':
		'将删除「{topic}」及其已拉取的 {count} 条语句，不可恢复。',
	'englishLearning.packHistory.deleteConfirmAction': '确认删除',
	'englishLearning.packHistory.deleteSuccess': '已删除生成记录',

	'englishLearning.favoritesDrawer.selectAllLoaded': '全选',
	'englishLearning.favoritesDrawer.removeSelected': '删除所选 {count} 项',
	'englishLearning.favoritesDrawer.removing': '移除中...',
	'englishLearning.favoritesDrawer.removeSuccess': '已移除所选收藏',
	'englishLearning.favoritesDrawer.removeFail': '移除失败，请稍后重试',
	'englishLearning.favoritesDrawer.removeConfirmTitle': '确认移除所选收藏？',
	'englishLearning.favoritesDrawer.removeConfirmDesc':
		'将移除已选中的 {count} 条收藏，移除后可在列表中重新收藏。',
	'englishLearning.favoritesDrawer.removeConfirmAction': '确认移除',
	'englishLearning.favoritesDrawer.removeNoneHint': '请先勾选要移除的条目',
	'englishLearning.favoritesDrawer.removeOneAction': '删除该条收藏',
	'englishLearning.favoritesDrawer.removeOneConfirmTitle': '确认删除这条收藏？',
	'englishLearning.favoritesDrawer.removeOneConfirmDescVocab':
		'将取消收藏词条「{word}」，移除后可在列表中重新收藏。',
	'englishLearning.favoritesDrawer.removeOneConfirmDescClassicIntro':
		'将取消收藏以下英文原句，移除后可在列表中重新收藏。',
	'englishLearning.favoritesDrawer.removeOneSuccess': '已删除该收藏',

	'englishLearning.favoritesDrawer.toggleRow': '选择该行',

	'englishLearning.vocab.title': '单词资料',
	'englishLearning.vocab.desc':
		'按你的主题生成一批单词：含国际音标（IPA）、中文释义与例句。点击小喇叭可朗读该英文单词（云端 TTS 优先）。',
	'englishLearning.vocab.descShort': '按主题生成单词表',
	'englishLearning.vocab.import': '导入单词',
	'englishLearning.vocab.topicFieldLabel': '学习主题',
	'englishLearning.vocab.topicPlaceholder':
		'例如：商务会议常用动词、旅游问路、易混形近词、本月四级高频词…',
	'englishLearning.vocab.topicRequired': '请先填写学习主题或需求',
	'englishLearning.vocab.count': '词数（选填）',
	'englishLearning.vocab.countPlaceholder': '留空按上限拉取',
	'englishLearning.vocab.countHint':
		'留空不指定条数，由服务端按单次上限生成；填写须为 1～12000 整数，或点快捷数量',
	'englishLearning.vocab.countInvalid': '须为 1～12000 整数，或留空',
	'englishLearning.vocab.progress':
		'已生成 {collected} / {target} 条 · 第 {round} 批模型请求',
	'englishLearning.vocab.fromDatabase':
		'已从资源库加载 {count} 个单词（同主题）',
	'englishLearning.classic.fromDatabase':
		'已从资源库加载 {count} 条语句（同主题）',
	'englishLearning.vocab.partialResult':
		'目标 {want} 条，本次返回 {got} 条（可能因重复过多或达到重试上限提前结束）',
	'englishLearning.vocab.aborted': '已取消生成',
	'englishLearning.vocab.streamDisconnected':
		'流式连接异常结束，请检查网络后重试',
	'englishLearning.vocab.stop': '停止生成',
	'englishLearning.vocab.generate': '生成单词',
	'englishLearning.vocab.empty': '未返回词条，请换主题重试',
	'englishLearning.vocab.playWord': '朗读该词',
	'englishLearning.vocab.favoriteWord': '收藏该词',
	'englishLearning.vocab.unfavoriteWord': '取消收藏',
	'englishLearning.vocab.listHeading': '单词列表',
	'englishLearning.vocab.collapseList': '收起单词列表',
	'englishLearning.vocab.expandList': '展开单词列表',
	'englishLearning.vocab.historyTitle': '历史生成记录',
	'englishLearning.vocab.historyOpenDrawer': '历史记录',
	'englishLearning.vocab.favoritesOpenDrawer': '收藏记录',
	'englishLearning.vocab.favoritesTitle': '收藏的单词',
	'englishLearning.vocab.favoritesLoading': '加载收藏…',
	'englishLearning.vocab.favoritesEmpty': '暂无收藏单词',
	'englishLearning.vocab.exportDocx': '导出',
	'englishLearning.vocab.exportDocxEmpty': '暂无收藏可导出',
	'englishLearning.vocab.exportDocxSuccess': '下载成功',
	'englishLearning.vocab.exportDocxFail': '导出失败，请稍后重试',
	'englishLearning.vocab.favoritesLoaded': '已载入到单词列表',
	'englishLearning.vocab.historyEmpty':
		'暂无记录；生成单词成功后会自动保存在此处',
	'englishLearning.vocab.historyWords': '{count} 词',
	'englishLearning.vocab.historyLoad': '载入',
	'englishLearning.vocab.historyLoaded': '已载入该次单词列表',
	'englishLearning.vocab.historyLoading': '加载历史...',
	'englishLearning.vocab.historyStreaming': '正在拉取',

	'englishLearning.classic.title': '经典语句',
	'englishLearning.classic.descShort': '按主题拉取名句、短语等',
	'englishLearning.classic.import': '导入语句',
	'englishLearning.classic.topicFieldLabel': '主题 / 方向',
	'englishLearning.classic.topicPlaceholder':
		'例如：关于勇气的名言、《傲慢与偏见》节选、励志演讲金句、莎士比亚十四行…',
	'englishLearning.classic.topicRequired': '请先填写主题或想要的方向',
	'englishLearning.classic.count': '条数（选填）',
	'englishLearning.classic.countPlaceholder': '留空按上限拉取',
	'englishLearning.classic.countHint':
		'留空不指定条数，由服务端按单次上限生成；填写须为 1～6000 整数，或点快捷数量',
	'englishLearning.classic.countInvalid': '须为 1～6000 整数，或留空',
	'englishLearning.classic.progress':
		'已生成 {collected} / {target} 条 · 第 {round} 批',
	'englishLearning.classic.partialResult':
		'目标 {want} 条，本次返回 {got} 条（可能因重复或上限提前结束）',
	'englishLearning.classic.aborted': '已取消生成',
	'englishLearning.classic.streamDisconnected':
		'流式连接异常结束，请检查网络后重试',
	'englishLearning.classic.stop': '停止生成',
	'englishLearning.classic.generate': '生成语句',
	'englishLearning.classic.empty': '未返回条目，请换主题重试',
	'englishLearning.classic.playQuote': '朗读该句',
	'englishLearning.classic.favoriteQuote': '收藏该句',
	'englishLearning.classic.unfavoriteQuote': '取消收藏',
	'englishLearning.classic.collapseList': '收起语句列表',
	'englishLearning.classic.expandList': '展开语句列表',
	'englishLearning.classic.sourceLabel': '出处：',
	'englishLearning.classic.historyTitle': '经典语句历史',
	'englishLearning.classic.historyOpenDrawer': '历史记录',
	'englishLearning.classic.favoritesOpenDrawer': '收藏记录',
	'englishLearning.classic.favoritesTitle': '收藏的经典句',
	'englishLearning.classic.favoritesLoading': '加载收藏…',
	'englishLearning.classic.favoritesEmpty': '暂无收藏语句',
	'englishLearning.classic.exportDocx': '导出',
	'englishLearning.classic.exportDocxEmpty': '暂无收藏可导出',
	'englishLearning.classic.exportDocxSuccess': '已开始下载',
	'englishLearning.classic.exportDocxFail': '导出失败，请稍后重试',
	'englishLearning.classic.exportJson': '导出 JSON',
	'englishLearning.classic.exportEmpty': '没有可导出的语句',
	'englishLearning.classic.exportFailed': '导出失败，请稍后重试',
	'englishLearning.classic.exportFileFallback': '语句',
	'englishLearning.classic.favoritesLoaded': '已载入到语句列表',
	'englishLearning.classic.historyEmpty': '暂无记录',
	'englishLearning.classic.historyQuotes': '{count} 条',
	'englishLearning.classic.historySentences': '{count} 句',
	'englishLearning.classic.historyLoaded': '已载入该次语句列表',
	'englishLearning.classic.historyLoading': '加载历史...',
	'englishLearning.classic.historyStreaming': '正在拉取',

	'englishLearning.tts.play': '朗读',
	'englishLearning.tts.stop': '停止',
	'englishLearning.tts.unsupported':
		'朗读不可用，请检查登录与云端语音配置，或换用支持本机语音的浏览器。',
	'assistant.selection.speak': '朗读内容',
	'assistant.selection.copy': '复制内容',
	'assistant.selection.speakBar': '朗读控制条',
	'assistant.selection.dragBar': '拖动控制条',
	'assistant.selection.resetBar': '回到默认位置',
	'assistant.selection.stopSpeak': '停止朗读',
	'assistant.selection.resizeBar': '调整控制条大小',
	'assistant.tts.unsupported':
		'朗读不可用，请检查登录与云端语音配置，或换用支持本机语音的浏览器。',
	'englishLearning.tts.cloudXfyunFailed': '讯飞云端朗读失败',
	'englishLearning.tts.cloudEdgeFailed': 'Edge 免费语音朗读失败',
	'englishLearning.tts.cloudMinimaxFailed': 'MiniMax 云端朗读失败',
	'englishLearning.tts.cloudFallbackLocal':
		'已改用本机语音播放，请检查讯飞应用凭证或服务端配置。',
	'englishLearning.tts.cloudFallbackEdge': '已改用 Edge 免费语音继续播放。',
	'englishLearning.tts.hint':
		'消息区朗读优先云端 TTS，失败时使用浏览器语音；单词表可逐词播放。',
	'route.document.title': '智能文档处理',
	'route.coding.title': '智能代码处理',
	'route.profile.title': '个人主页',
	'route.knowledge.title': '知识库',
	'route.skills.title': '技能',
	'route.ebook.title': '墨客书屋',
	'route.ebook.shelf': '我的书架',
	'route.ebook.read': '阅读',

	'skill.list.empty': '暂无 Skill，点击新建开始编写',
	'skill.toolbar.library': '技能库',
	'skill.toolbar.libraryShow': '显示技能列表',
	'skill.toolbar.libraryHide': '隐藏技能列表',
	'skill.new': '新建',
	'skill.save': '保存',
	'skill.save.unsaved': '保存（有未保存的修改）',
	'skill.delete': '删除',
	'skill.title.placeholder': 'Skill 标题',
	'skill.content.placeholder': '编写 Skill / Prompt 正文（Markdown）',
	'skill.try.placeholder': '用当前 Skill 试跑提问…',
	'skill.try.send': '发送',
	'skill.try.empty': '右侧可试跑当前 Skill',
	'skill.try.saveFirst': '请先保存 Skill，再在此试跑。',
	'skill.panel.mode.try': '试跑',
	'skill.panel.mode.generate': '生成',
	'skill.generate.placeholder': '描述需求，或说明如何改写当前编辑器中的 Skill…',
	'skill.generate.empty':
		'对话跟当前 Skill（新建草稿或已保存）绑定。可从零起草，也可基于编辑器内容追问改写；满意后点「写入编辑器」，检查后再保存',
	'skill.generate.applyToEditor': '写入编辑器',
	'skill.generate.applied': '已写入编辑器，请检查后保存',
	'skill.generate.emptyApply': '回复为空，无法写入',
	'skill.generate.parseFail':
		'未能解析出标题与正文，请让 Agent 按 # 标题格式重写',
	'skill.slash.placeholder': '搜索 Skill…',
	'skill.slash.hint': '继续输入以筛选，空格关闭',
	'skill.slash.filtering': '筛选：{q}',
	'skill.slash.empty': '没有匹配的 Skill',
	'skill.slash.confirm': '确认',
	'skill.slash.add': '增加技能',
	'skill.applied.label': '已应用 Skill',
	'skill.chips.aria': '已选 Skill',
	'skill.delete.confirmTitle': '删除 Skill？',
	'skill.delete.confirmDesc': '删除后不可恢复。',

	'ebook.shelf.title': '我的书架',
	'ebook.shelf.hint':
		'支持 EPUB、PDF；桌面端可直接打开本地文件，网页端请导入文件。（开通会员后可自动上传书籍至云端备份）',
	'ebook.shelf.hintTauri':
		'支持 epub、pdf，点击选择本地文件（开通会员后可自动上传书籍至云端备份）',
	'ebook.shelf.hintTauriMember': '支持 epub、pdf，导入后自动备份至云端',
	'ebook.shelf.hintWeb': '网页端导入需开通会员；桌面端非会员可本地阅读',
	'ebook.shelf.hintWebMember': '支持 epub、pdf，导入后自动备份至云端',
	'ebook.shelf.membershipRequiredUploadTitle': '需要会员',
	'ebook.shelf.membershipRequiredUploadMessage':
		'上传书籍至云端为会员权益，请先开通会员后再导入。',
	'ebook.shelf.pickLocal': '选择本地文件',
	'ebook.shelf.search': '搜索书籍',
	'ebook.shelf.searchClose': '关闭搜索',
	'ebook.shelf.searchPlaceholder': '根据书名搜索',
	'ebook.shelf.searchEmpty': '未找到匹配的书籍。',
	'ebook.shelf.pickFile': '导入文件',
	'ebook.shelf.empty': '暂无书籍。',
	'ebook.shelf.progress': '已读约 {pct}%',
	'ebook.shelf.read': '开始阅读',
	'ebook.shelf.continue': '继续阅读',
	'ebook.shelf.deleteConfirmTitle': '删除书籍？',
	'ebook.shelf.deleteConfirmDesc':
		'将移除「{title}」及阅读进度，此操作不可恢复。',
	'ebook.shelf.uploadReading': '正在读取本地文件「{name}」…',
	'ebook.shelf.uploading': '正在上传「{name}」',
	'ebook.shelf.uploadProgress': '{pct}%',
	'ebook.shelf.uploadHint': '大文件上传可能需要较长时间，请勿关闭页面。',
	'ebook.shelf.coverSetting': '设置封面',
	'ebook.shelf.coverSaved': '封面已更新',
	'ebook.shelf.coverFailed': '封面设置失败',
	'ebook.shelf.editTitle': '编辑书名',
	'ebook.shelf.editTitleHint': '点击编辑：{title}',
	'ebook.shelf.bookCategory': '书籍分类：{category}',
	'ebook.shelf.titleSaved': '书名已更新',
	'ebook.shelf.titleFailed': '书名更新失败',
	'ebook.shelf.alreadyImportedTitle': '书籍已在书架',
	'ebook.shelf.alreadyImportedMessage': '该书籍已在书架中，无需重复上传。',
	'ebook.shelf.category.all': '全部',
	'ebook.shelf.category.uncategorized': '未分类',
	'ebook.shelf.category.manage': '管理分类',
	'ebook.shelf.category.add': '添加分类',
	'ebook.shelf.category.rename': '重命名',
	'ebook.shelf.category.delete': '删除分类',
	'ebook.shelf.category.deleteConfirm':
		'删除后，该分类中的书籍将移至「未分类」。',
	'ebook.shelf.category.move': '移动到分类',
	'ebook.shelf.category.empty': '此分类暂无书籍。',
	'ebook.shelf.category.duplicateName': '分类名称已存在',
	'ebook.shelf.publicEmpty': '暂无他人公开的书籍。',
	'ebook.shelf.publicOwner': '书主',
	'ebook.public.makePublic': '公开此书',
	'ebook.public.confirmPublicTitle': '公开此书？',
	'ebook.public.confirmPublicDesc':
		'全书及你的读书想法将对所有用户可见，并出现在他人书架。',
	'ebook.public.confirmPrivateTitle': '取消公开？',
	'ebook.public.confirmPrivateDesc': '取消后，其他用户书架将不再展示此书。',
	'ebook.public.epubOnly': '仅支持公开 EPUB 书籍',
	'ebook.public.cloudRequired': '请先上传至云端后再公开',
	'ebook.public.localCannotShare': '本地书籍无法分享',
	'ebook.public.noLongerPublic': '该书已不再公开',
	'ebook.read.publicSourceRevoked': '作者已取消公开，无法继续阅读。',
	'ebook.public.sourceGone': '书籍不存在或已下架',
	'ebook.public.visibilityFailed': '更新公开状态失败',
	'ebook.read.publicReadingBanner':
		'该书已非公开展示，你仍可继续阅读自己的进度与批注。',
	'ebook.read.missing': '未找到该书籍',
	'ebook.read.sourceLocal': '本地',
	'ebook.read.sourceOnline': '在线',
	'ebook.read.backShelf': '返回书架',
	'ebook.read.toc': '书籍目录',
	'ebook.read.ebookIdeas.title': '全书想法',
	'ebook.read.ebookIdeas.open': '全书想法列表',
	'ebook.read.tocEmpty': '该文件没有书籍目录。',
	'ebook.read.tocScrollToBottom': '滚动到底部',
	'ebook.read.tocScrollToTop': '滚动到顶部',
	'ebook.read.tocScrollToCurrent': '滚动到当前',
	'ebook.read.settings': '阅读设置',
	'ebook.read.settings.fontSize': '字号',
	'ebook.read.settings.lineHeight': '行间距',
	'ebook.read.settings.pageFlow': '翻页方式',
	'ebook.read.settings.pageFlow.paginated': '分页翻页',
	'ebook.read.settings.pageFlow.scrolled': '连续滚动',
	'ebook.read.settings.textColor': '文字颜色',
	'ebook.read.settings.textColor.auto': '跟随应用主题',
	'ebook.read.settings.textColor.dark': '纯黑（Kindle）',
	'ebook.read.settings.textColor.softDark': '柔黑',
	'ebook.read.settings.textColor.brown': '棕色（sepia 配）',
	'ebook.read.settings.textColor.sepia': '棕褐',
	'ebook.read.settings.textColor.gray': '灰色',
	'ebook.read.settings.textColor.light': '白色',
	'ebook.read.settings.textColor.softLight': '浅灰（夜间）',
	'ebook.read.settings.textColor.green': '墨绿',
	'ebook.read.settings.textColor.blue': '深蓝灰',
	'ebook.read.settings.textColor.rose': '玫瑰棕（配粉色底）',
	'ebook.read.settings.textColor.warmGray': '暖灰',
	'ebook.read.settings.bgTheme': '阅读背景',
	'ebook.read.settings.bgTheme.default': '跟随应用',
	'ebook.read.settings.bgTheme.paper': '纸张白',
	'ebook.read.settings.bgTheme.cream': '羊皮纸（iBooks）',
	'ebook.read.settings.bgTheme.sepia': '护眼（Kindle）',
	'ebook.read.settings.bgTheme.warm': '暖黄',
	'ebook.read.settings.bgTheme.green': '薄荷绿（Kindle）',
	'ebook.read.settings.bgTheme.blue': '淡蓝灰',
	'ebook.read.settings.bgTheme.gray': '浅灰',
	'ebook.read.settings.bgTheme.pink': '护眼粉（微信读书）',
	'ebook.read.settings.bgTheme.lavender': '淡紫',
	'ebook.read.settings.bgTheme.night': '夜间模式',
	'ebook.read.settings.bgTheme.moon': '月影白',
	'ebook.read.settings.reset': '恢复默认',
	'ebook.read.prev': '上一页',
	'ebook.read.next': '下一页',
	'ebook.read.pdfZoomIn': '放大',
	'ebook.read.pdfZoomOut': '缩小',
	'ebook.read.pdfZoomHint': '相对适应宽度的缩放比例，100% 为满宽显示',
	'ebook.read.contextMenu.copy': '复制',
	'ebook.read.selectionPop.underline': '划线',
	'ebook.read.selectionPop.removeUnderline': '删除划线',
	'ebook.read.selectionPop.styleHighlight': '背景高亮',
	'ebook.read.selectionPop.styleUnderline': '直线下划线',
	'ebook.read.selectionPop.styleWavy': '波浪下划线',
	'ebook.read.selectionPop.colorPrefix': '划线颜色',
	'ebook.read.selectionPop.customColor': '自定义颜色',
	'ebook.read.selectionPop.share': '分享书摘',
	'ebook.read.quoteShare.title': '分享书摘',
	'ebook.read.quoteShare.hint': '生成书摘图片，可复制到微信等应用粘贴分享',
	'ebook.read.quoteShare.generating': '正在生成图片…',
	'ebook.read.quoteShare.previewAlt': '书摘分享预览',
	'ebook.read.quoteShare.copyImage': '复制图片',
	'ebook.read.quoteShare.copied': '已复制',
	'ebook.read.quoteShare.copySuccess': '图片已复制，可分享到其他应用',
	'ebook.read.quoteShare.copyFailed': '复制失败，请重试',
	'ebook.read.quoteShare.generateFailed': '生成分享图片失败',
	'ebook.read.quoteShare.downloadFailed': '下载失败，请重试',
	'ebook.read.quoteShare.brand': '墨客读书',
	'ebook.read.selectionPop.askBook': 'MK 问书',
	'ebook.read.selectionPop.listen': '听当前',
	'ebook.read.listen.followResume': '回到播放位置',
	'ebook.read.listen.followResumeAria': '滚动到正在播放的内容并恢复自动跟随',
	'ebook.read.listenBook': '听书',
	'ebook.read.listenBook.stop': '停止听书',
	'ebook.read.listenBook.pause': '暂停',
	'ebook.read.listenBook.resume': '继续',
	'ebook.read.listenBook.prevSentence': '上一句',
	'ebook.read.listenBook.nextSentence': '下一句',
	'ebook.read.listenBook.prevChapter': '上一章',
	'ebook.read.listenBook.nextChapter': '下一章',
	'ebook.read.listenBook.sentenceMenu': '分句',
	'ebook.read.listenBook.sentenceMenuEmpty': '暂无分句',
	'ebook.read.listenBook.scrollToCurrentSentence': '滚动到当前句',
	'ebook.read.listenBook.speed': '播放倍速',
	'ebook.read.listenBook.speedBookOnly': '设置为本书籍',
	'ebook.read.listenBook.progress': '第 {chapter} 章 · 第 {current}/{total} 句',
	'ebook.read.listenBook.loading': '正在加载…',
	'ebook.read.listenBook.barAria': '听书播放控制',
	'ebook.read.listenBook.emptySection': '本章暂无文字可读',
	'ebook.read.listenBook.notReady': '阅读器尚未就绪，请稍后再试',
	'ebook.read.listenBook.finished': '已读完本书',
	'ebook.read.contextMenu.addThought': '写想法',
	'ebook.read.contextMenu.assistant': '智能助手',
	'ebook.read.contextMenu.askSelection': 'MK 问书',
	'ebook.read.assistant.close': '关闭 MOKE 智能助手',
	'ebook.read.assistant.open': '开启 MOKE 智能助手',
	'ebook.read.assistant.toggleAria': '开关 MOKE 智能助手',
	'ebook.read.assistant.intro':
		'向 MOKE 智能助手提问关于《{title}》的内容、背景与读后感，也可以让 MOKE 智能助手梳理章节要点或解释疑难段落。选中正文后右键使用「MK 问书」，可带着上下文提问。',
	'ebook.read.assistant.placeholder': '向 MOKE 智能助手提问本书相关内容…',
	'ebook.read.assistant.systemHint':
		'用户正在阅读电子书《{title}》。请结合书籍语境作答；若用户引用摘录，请优先解释该段含义与上下文。',
	'ebook.read.assistant.askSelectionDraft': '请解释以下书中摘录：\n\n> {quote}',
	'ebook.read.thought.createTitle': '写想法',
	'ebook.read.thought.viewTitle': '读书想法',
	'ebook.read.thought.editTitle': '编辑想法',
	'ebook.read.thought.label': '想法',
	'ebook.read.thought.placeholder': '记录你的感想…',
	'ebook.read.thought.save': '保存',
	'ebook.read.thought.sendPublic': '公开发送',
	'ebook.read.thought.sendPrivate': '私密发送',
	'ebook.read.thought.cancel': '取消',
	'ebook.read.thought.delete': '删除',
	'ebook.read.thought.edit': '编辑',
	'ebook.read.thought.empty': '暂无内容',
	'ebook.read.thought.cfiFailed':
		'无法定位选中位置，请在正文区域内右键添加想法',
	'ebook.read.thought.loadFailed': '加载读书想法失败',
	'ebook.read.thought.saveFailed': '保存想法失败',
	'ebook.read.highlight.loadFailed': '加载划线失败',
	'ebook.read.highlight.saveFailed': '保存划线失败',
	'ebook.read.highlight.deleteFailed': '删除划线失败',
	'ebook.read.thought.deleteFailed': '删除想法失败',
	'ebook.read.thought.listTitle': '该段共有 {count} 条想法',
	'ebook.read.thought.totalCount': '共 {count} 条',
	'ebook.read.thought.bookExcerpt': '书中原文',
	'ebook.read.thought.quoteExpand': '展开引用内容',
	'ebook.read.thought.quoteCollapse': '收起引用内容',
	'ebook.read.thought.closeView': '关闭读书想法',
	'ebook.read.thought.closeEdit': '关闭想法编辑',
	'ebook.read.thought.unknownUser': '未知用户',
	'ebook.read.thought.clusterExcerpt': '摘录 ({length} 字)',
	'ebook.read.thought.viewDetail': '查看详情',
	'ebook.read.thought.selectedQuoteHint': '当前显示该想法对应的原文',
	'ebook.read.thought.visibilityPrivate': '私密',
	'ebook.err.open': '打开失败',
	'ebook.err.loadFailed': '无法加载该书籍，请检查书籍是否存在',
	'route.account.title': '账号设置',
	'route.pay.title': '会员充值',
	'route.setting.title': '系统设置',
	'route.plugins.title': '插件中心',
	'plugins.page.title': '插件中心',
	'plugins.page.desc': '默认全部关闭，关闭后基座不再加载，相关入口也会隐藏',
	'plugins.page.tabsAria': '插件与应用',
	'plugins.page.tab.plugin': '插件',
	'plugins.page.tab.app': '应用',
	'plugins.page.empty': '暂无插件（请确认 registry 可访问）',
	'plugins.page.empty.plugin': '暂无插件（业务内嵌 / surface 槽）',
	'plugins.page.empty.app': '暂无应用（独立路由注入）',
	'plugins.page.editRegistry': '编辑配置',
	'route.plugins.registry.title': '插件配置',
	'plugins.registry.title': '插件配置',
	'plugins.registry.back': '返回插件中心',
	'plugins.registry.reload': '重新加载',
	'plugins.registry.save': '保存到服务器',
	'plugins.registry.saving': '保存中…',
	'plugins.registry.loading': '加载中…',
	'plugins.registry.invalidJson': 'JSON 无效：须可解析且包含 plugins 数组',
	'plugins.registry.saveOk': '已保存到 remotes',
	'plugins.registry.saveFail': '保存失败',
	'plugins.registry.noChanges': '内容未修改，无需保存',
	'plugins.registry.missingHostApiRange': '插件 {id} 缺少 hostApiRange',
	'plugins.registry.hostApiIncompatible':
		'插件 {id} 的 hostApiRange「{range}」与当前 Host API {hostApi} 不兼容。破缓存请只改 version，保持 hostApiRange 为 ^{hostApi}',
	'plugins.registry.pluginNotFound': 'registry 中无插件 {id}',
	'plugins.registry.help.triggerAria': '字段说明',
	'plugins.registry.help.panelTitle': 'plugins-registry 字段说明',
	'plugins.registry.help.footnote':
		'发版破缓存请只改 version；hostApiRange 须覆盖当前 Host API（见 VITE_HOST_API_VERSION）。',
	'plugins.registry.help.sectionRoot': '根字段',
	'plugins.registry.help.sectionBasic': '插件基础',
	'plugins.registry.help.sectionMf': '加载与权限',
	'plugins.registry.help.sectionHost': '挂载与安全',
	'plugins.registry.help.updatedAt':
		'配置最近更新时间；保存时由 Host 自动写入。',
	'plugins.registry.help.plugins': '插件描述符数组；每一项对应一个可上架插件。',
	'plugins.registry.help.id': '插件唯一 ID；Host 用其挂路由/侧栏与本地状态。',
	'plugins.registry.help.fieldTitle':
		'多语言名称对象（zh-CN / en-US）；插件中心与面包屑展示。',
	'plugins.registry.help.description':
		'多语言说明，或旧版单语字符串；插件中心卡片描述。',
	'plugins.registry.help.routePath':
		'插件路由路径，如 /english-learning/notes。',
	'plugins.registry.help.entry':
		'MF 入口 URL，优先 mf-manifest.json；生产须 https，开发可用 localhost。',
	'plugins.registry.help.version':
		'插件自身版本。发版/破缓存时 bump 此字段即可，勿与 hostApiRange 混淆。',
	'plugins.registry.help.hostApiRange':
		'要求的 Host API 版本范围（如 ^1.0.0）。须覆盖当前 Host；仅 Host 契约升级时再改。',
	'plugins.registry.help.enabled':
		'目录建议默认值（现已默认 false）。实际上架/下架按账号保存在服务端（Web/桌面同步），不再写回此字段。',
	'plugins.registry.help.trust':
		'信任等级：first-party / partner / untrusted（后者走 iframe，不 loadRemote）。',
	'plugins.registry.help.remoteName':
		'MF registerRemotes.name；默认等于 id。多插件共用同一 Remote 时填 federation name。',
	'plugins.registry.help.expose':
		'MF expose 路径，默认 ./App；如 ./LearningNotes、./EbookIdeas。',
	'plugins.registry.help.framework':
		'可选 react | vue。vue 时 Remote 须导出 mount(el, bridge)；Host 不装 Vue，仅调用 mount。',
	'plugins.registry.help.injectRoute':
		'是否由 PluginManager 注入顶层路由。false 表示宿主业务树已挂好 PluginHostPage；true + 不写 menu = 仅路由、无侧栏。',
	'plugins.registry.help.preload':
		'加载时机：route/idle（默认懒加载）或 eager（启动后后台预拉，一般勿用）。',
	'plugins.registry.help.permissions':
		'权限声明，如 ui:toast、nav:subtree、http:plugin-api、modules:ebook。',
	'plugins.registry.iconUploadLabel': '设置插件图标',
	'plugins.registry.iconPickPlugin': '选择插件',
	'plugins.registry.iconUpload': '上传并写入',
	'plugins.registry.iconUploading': '上传中…',
	'plugins.registry.iconUploadOk': 'SVG 图标已上传并写入 registry',
	'plugins.registry.iconUploadFail': '图标上传失败',
	'plugins.registry.iconSvgOnly': '仅支持上传 SVG 文件',
	'plugins.registry.iconNoTarget': '该插件没有 menu / host，无法写入 icon',
	'plugins.registry.help.menu':
		'侧栏入口（可选）：有则注入侧栏；省略则仅路由/内嵌，不出现在左侧菜单。order 排序；icon 为 SVG 图片 URL。',
	'plugins.registry.help.host':
		'业务页自动挂载：surface（如 ebook.read）、slot（drawer/toolbar）、icon（SVG URL）、order。',
	'plugins.registry.help.iframeUrl':
		'trust 为 untrusted 时必填：独立 HTTPS 页，Host 用 iframe 打开。',
	'plugins.registry.help.integrity': '可选：entry 资源的 sha384 完整性校验。',
	'plugins.registry.help.signature':
		'可选：签名标记；值为 invalid 时拒绝加载。',
	'plugins.verify.hostApiIncompatible':
		'插件 {id}：Host API 当前为 {hostApi}，与 hostApiRange「{range}」不兼容。发版破缓存请只改 version；hostApiRange 仅在 Host 契约升级时再改',
	'plugins.shelf.on': '已上架',
	'plugins.shelf.off': '已下架',
	'plugins.shelf.toggle': '上架 / 下架',
	'plugins.card.route': '路由',
	'plugins.card.entry': '入口',
	'plugins.card.enter': '进入',
	'plugins.card.trust': '信任',
	'plugins.card.trust.first-party': '官方',
	'plugins.card.trust.partner': '合作方',
	'plugins.card.trust.untrusted': '未信任',
	'plugins.card.sidebar': '侧栏',
	'plugins.card.sidebar.on': '有',
	'plugins.card.sidebar.off': '无',
	'plugins.card.noDesc': '暂无说明',
	'plugins.host.delisted': '该插件已下架，可在「插件中心」重新上架。',
	'plugins.host.missingIframeUrl': '插件「{id}」为 untrusted，但缺少 iframeUrl',
	'plugins.host.loading': '正在加载中...',
	'plugins.host.loadingNamed': '插件「{id}」正在加载中...',
	'plugins.host.notLoaded': '未加载（请确认 Remote 已启动后重试）',
	'plugins.host.unavailable': '插件「{id}」不可用',
	'plugins.host.reload': '重新加载',
	'plugins.host.loadFailed': '插件「{id}」加载失败',
	'route.setting.about': '关于应用',
	'route.setting.theme': '主题设置',
	'route.setting.llm': '大模型配置',
	'route.setting.cloudTts': '语音设置',
	'route.setting.language': '语言设置',
	'route.legal.servicePolicy': '用户服务政策',
	'route.legal.userAgreement': '用户服务协议',
	'route.updateInfo.title': '更新信息',
	'route.projectGuide.title': '产品指南',
	'route.pluginDevGuide.title': '插件开发手册',
	'route.downloadDesktop.title': '下载桌面客户端',
	'route.guard.needLoginTitle': '请先登录后再访问',
	'route.guard.needLoginMessage': '「{page}」需要登录后才能访问。',
	'route.guard.unknownPage': '该页面',

	'downloadPage.hero.badge': '桌面客户端 · 正式发行版',
	'downloadPage.hero.title': '欢迎下载 dnhyxc-ai 桌面端',
	'downloadPage.hero.subtitle':
		'在 macOS 上获得完整系统能力：全局快捷键、本地文件夹、开机自启等。',
	'downloadPage.hero.versionLabel': 'v{version}',
	'downloadPage.hero.dateLabel': '发布：{date}',
	'downloadPage.hero.primaryCtaMac': '下载 macOS 安装包（DMG）',
	'downloadPage.hero.copyLink': '复制下载链接',
	'downloadPage.hero.pickPlatform': '请选择下方平台卡片查看可用安装包。',
	'downloadPage.links.releaseNotes': '更新说明',
	'downloadPage.links.userGuide': '产品指南',
	'downloadPage.links.backHome': '返回首页',
	'downloadPage.platforms.sectionLabel': '选择系统',
	'downloadPage.platforms.mac': 'macOS',
	'downloadPage.platforms.windows': 'Windows',
	'downloadPage.platforms.linux': 'Linux',
	'downloadPage.mac.cardTitle': 'macOS（Apple 芯片）',
	'downloadPage.mac.cardDesc':
		'推荐大多数用户使用 DMG：下载后打开，将应用拖入「应用程序」。如需命令行或自动更新通道，可选用 .tar.gz。',
	'downloadPage.mac.downloadDmg': '下载 .dmg',
	'downloadPage.mac.downloadTarGz': '下载 .tar.gz（更新通道）',
	'downloadPage.mac.installGuide': '安装说明',
	'downloadPage.mac.stepInstall':
		'打开下载的 DMG 安装包，将 dnhyxc-ai 拖入「应用程序」后即可从启动台打开使用。',
	'downloadPage.mac.stepGatekeeper':
		'若提示「无法验证开发者」，请到「系统设置 → 隐私与安全性」选择仍要打开，或右键点图标打开。',
	'downloadPage.mac.stepUpdater':
		'应用内置更新检查功能（关于 → 检查更新）；也可随时留意本站更新说明用以获取到最新发行要点。',
	'downloadPage.mac.noteIntel':
		'当前安装包面向 Apple 芯片（ARM64）。若使用 Intel Mac，请关注后续是否提供 x64 构建。',
	'downloadPage.mac.noteSource':
		'安装包由 GitHub Releases 托管下载；若下载较慢可更换网络或稍后重试。',
	'downloadPage.mac.historyIntro':
		'「全部版本」进入 Releases 列表，可逐个版本查看并下载附件（含 aarch64 等 dmg）。',
	'downloadPage.mac.openLatestTag': '打开 latest 标签页',
	'downloadPage.mac.openAllReleases': '浏览全部历史版本',
	'downloadPage.windows.cardTitle': 'Windows',
	'downloadPage.windows.cardDesc':
		'Windows 安装包正在准备中。你现在仍可在浏览器中使用网页版核心能力。',
	'downloadPage.windows.comingSoon': '敬请期待',
	'downloadPage.linux.cardTitle': 'Linux',
	'downloadPage.linux.cardDesc':
		'Linux 桌面包正在评估中。欢迎先在浏览器端体验或通过源码自行构建。',
	'downloadPage.linux.comingSoon': '敬请期待',
	'downloadPage.requirements.title': '使用前提示',
	'downloadPage.requirements.itemNet': '下载与首次配置需要可用的网络连接。',
	'downloadPage.requirements.itemMac':
		'macOS 建议使用较新系统版本，并为麦克风等权限按需授权。',
	'downloadPage.requirements.itemAccount':
		'部分功能需登录账号；未登录时知识库仍可本地使用。',
	'downloadPage.toast.linkCopied': '下载链接已复制',
	'downloadPage.toast.copyFailed': '复制失败，请手动选择链接',

	'updateInfoPage.header.subtitle':
		'能力说明随版本更新；与仓库内《更新信息》文档请保持同步。',
	'updateInfoPage.item.dateLabel': '更新：{date}',
	'pluginDevGuide.item.dateLabel': '更新：{date}',

	'coding.toolbar.selectTemplate': '选择模版',
	'coding.toolbar.preview.show': '👁️‍🗨️ 显示预览',
	'coding.toolbar.preview.hide': '👁️ 隐藏预览',
	'coding.toolbar.console.show': '👁️‍🗨️ 显示日志',
	'coding.toolbar.console.hide': '👁️ 隐藏日志',
	'coding.toolbar.forceRefresh': '强制刷新',
	'coding.toolbar.currentTemplate': '当前语言',

	'pay.productName.default': 'Pro 额度充值',
	'pay.toast.loginRequired': '请先登录后再发起支付',
	'pay.toast.missingStripePk.title': '缺少 VITE_STRIPE_PUBLISHABLE_KEY',
	'pay.toast.missingStripePk.message':
		'在前端 .env 中配置 Publishable key（pk_test_… / pk_live_…）。',
	'pay.toast.invalidAmount': '请输入有效金额',
	'pay.toast.amountTooSmall': '金额过小',
	'pay.toast.clientSecretMissing.title': '未返回 client_secret',
	'pay.toast.clientSecretMissing.message':
		'请确认后端已创建 ui_mode=embedded 的 Checkout Session。',
	'pay.toast.stripeLoadFailed': 'Stripe.js 加载失败',
	'pay.toast.paid.title': '支付已完成',
	'pay.toast.paid.message': '感谢支持。',
	'pay.toast.paid.membershipMessage': '会员已开通，个人资料页可查看权益。',
	'pay.toast.mountNodeNotReady': '挂载节点未就绪',

	'pay.title': '会员充值',
	'pay.loginRequiredHint': '发起支付前需要登录账号。',
	'pay.goLogin': '去登录',

	'pay.form.plan': '选择会员套餐',
	'pay.plan.monthly.label': '月度会员',
	'pay.plan.monthly.desc': '30 天会员权益',
	'pay.plan.quarterly.label': '季度会员',
	'pay.plan.quarterly.desc': '90 天会员权益',
	'pay.plan.yearly.label': '年度会员',
	'pay.plan.yearly.desc': '365 天会员权益',
	'pay.plan.price': '¥{price}',
	'pay.plan.selectedSummary': '已选：{label} · {price}',
	'pay.form.currency.placeholder': '选择货币',
	'pay.form.amount': '金额（{format}）',
	'pay.form.amount.format.integer': '整数，无小数',
	'pay.form.amount.format.decimal2': '含两位小数',
	'pay.form.product': '商品说明（可选）',
	'pay.form.product.placeholder': '例如：月度会员',

	'pay.actions.openEmbedded': '开启支付',
	'pay.actions.closeEmbedded': '关闭支付',

	'pay.currency.cny': 'CNY · 人民币',
	'pay.currency.usd': 'USD · 美元',
	'pay.currency.eur': 'EUR · 欧元',
	'pay.currency.hkd': 'HKD · 港币',
	'pay.currency.gbp': 'GBP · 英镑',
	'pay.currency.jpy': 'JPY · 日元',
} as const;
