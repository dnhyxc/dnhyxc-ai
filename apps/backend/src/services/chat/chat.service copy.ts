...
export class ChatService {
	...
	private cleanupSession(sessionId: string): void {
		...
		// 注意：partialContent 存储在数据库中，不在这里清理
	}
	...
	async getOrCreateSession(sessionId: string): Promise<ChatSessions> {
	...
	async updateSessionPartialContent(
		...
		const session = await this.getOrCreateSession(sessionId);
		console.log(partialContent, 'partialContentpartialContentpartialContent');
	...
	async chat(dto: ChatRequestDto): Promise<any> {
		...
		for (const msg of enhancedMessages) {
			...
			else if (msg.role === 'user') role = MessageRole.USER;
			else role = MessageRole.ASSISTANT;
			await this.saveMessageToDatabase(sessionId, role, msg.content);
		}
		...
		return new Observable<string>((subscriber) => {
			(async () => {
				try {
					...
					// 只有在正常完成时才保存消息
					// 使用 cancel$.closed 判断是否被用户取消，subscriber.closed 在自然完成时也为 true
					const isCancelled = cancel$.closed;
					...
					if (!wasCancelledDuringIteration) {
						...
						this.conversationMemory.set(sessionId, [...allMessages, aiMessage]);
						// 清除部分响应（因为已经完成）
						console.log('清除部分响应，流正常完成', finalContent);
						await this.updateSessionPartialContent(sessionId, null);
					} else {
						// 流被取消：保存部分响应用于继续生成
						if (finalContent.length > 0) {
							console.log('保存部分响应，流被取消', finalContent);
...
请继续从上面内容结束的地方继续生成，不要重复已经生成的内容。`,
	...
	// 加载会话历史记录
	async loadSessionHistory(sessionId: string): Promise<ChatMessageDto[]> {
