...
import { AgentMemoryService } from 'src/services/agent/agent-memory.service';
import { buildAgentLangchainMiddleware } from 'src/services/agent/agent-middleware';
 ...
 * 系统提示要求模型先归纳工具结果并控制在约 1800 汉字内；附录会重复拼进每一轮子模型请求，故仅在模型失控超长时截断并打日志。
 */
...
function extractEnglishPackAgentChunkText(
...
function englishPackAgentIsUserAbort(err: unknown): boolean {
	let cur: unknown = err;
 ...
 */
function isJsonStringClosingQuoteAt(s: string, quoteIdx: number): boolean {
	let j = quoteIdx + 1;
 ...
 */
function repairJsonUnescapedInteriorQuotes(slice: string): string {
	let out = '';
	let i = 0;
	let inString = false;
	let escaped = false;
...
export class EnglishLearningService {
	...
	private trimPackAgentThread(msgs: BaseMessage[], max: number): BaseMessage[] {
		if (msgs.length <= max) {
			let out = msgs;
			while (out.length && out[0]!.getType() !== 'human') {
				out = out.slice(1);
			}
			return out;
		}
	...
	private async runEnglishPackMasterResearchPhase(params: {
		...
		try {
			// 构造主召唤大模型（流式）与摘要模型（非流式）
			const { main: mainLlm, summary: summaryLlm } =
				this.buildSiliconFlowPackAgentModels({
					maxTokens: 8192,
					temperature: 0.35,
			...
			const agent = createAgent({
				...
				middleware: buildAgentLangchainMiddleware({
					...
					estimatePromptTokens: (msgs) =>
						this.agentMemory.estimatePromptTokens(msgs),
				}),
			});
...
请按系统提示调用工具完成检索与核对，然后输出一段简明要点（中文为主，可夹关键英文术语），供下游子模型扩展词条或句子方向使用；不要输出 JSON，不要输出 markdown 代码块。`;
	...
	/**
	 * 将子模型单次请求的 system 全文追加到独立日志文件（便于排查 prompt；失败仅打 warn 不影响主流程）。
	 * 目录：`{process.cwd()}/logs/english-learning/submodel-system.log`
	 */
	private appendEnglishPackSubModelSystemLog(systemText: string): void {
		...
		void (async () => {
			try {
				...
				this.logger.warn(
					'[EnglishLearning] 写入子模型 system 文件日志失败',
					err,
				);
			}
		})();
	...
	private async invokeEnglishPackSubModelJson(params: {
		...
		this.appendEnglishPackSubModelSystemLog(params.system);
...
格式严格为：{"items":[{"word":"","ipa":"","translationZh":"","example":""}]}`;
		...
		/** 主 Agent 检索要点（整场一次）；非空时并入子模型 system，避免在每轮 user 与 priorThread 中重复计费 */
		...
		const packSubModelSystem = agentResearchAppendix.trim()
			? `${system}\n\n${agentResearchAppendix}`
			: system;
...
本批 items 内部也不得出现彼此重复的 word。请输出与上述列表完全不同的词或短语。${diversityHint}`;
	...
	async runClassicQuotesGeneration(
		...
		// 限定生成条数：最少1，最多ENGLISH_CLASSIC_QUOTES_GENERATION_MAX
		const count = Math.min(
			ENGLISH_CLASSIC_QUOTES_GENERATION_MAX,
			Math.max(1, dto.count ?? 10),
		);
		...
		const level = dto.level ?? 'intermediate';
		const levelText = LEVEL_HINT[level];
		// 允许的最大轮数（与批容量有关，防止死循环）
		const maxRounds = this.resolveMaxRounds(count, TOPIC_PACK_ITEMS_PER_ROUND);

		// 系统提示词，详细规定生成内容与格式、去重等要求
		const system = `你是英语教学助手。用户会给出「主题/学习需求」与难度。
用户可能分多轮请求；每一轮都会给出该轮必须生成的确切条数，请严格遵守该轮条数（items 数组长度必须等于该数字）。
...
格式严格为：{"items":[{"english":"","translationZh":"","source":"","noteZh":""}]}`;
		...
		/**
		 * Agent 检索辅助内容初始化
		 * 若有 userId，提前触发一轮主 Agent 检索；非空时并入子模型 system，避免在每轮 user 与 priorThread 中重复计费。
		 */
		let agentResearchAppendix = '';
		if (context?.userId != null) {
			try {
			...
			while (accumulated.length < count && rounds < maxRounds) {
				...
				const excludeSnippet =
					accumulated.length === 0
						...
						: [...seen]
								.slice(-TOPIC_PACK_EXCLUDE_TAIL)
								.map((s) => s.replace(/`/g, "'").slice(0, 240))
								.join(', ');

				// 多轮无进展时，触发多样性提示语，敦促LLM换方向
				const diversityHint =
					stall >= 2 && accumulated.length > 0
						? `\n【多样性】请换不同作品/时代/体裁/演讲场合；避免仅改写标点或个别词的同一句式变体。`
						: '';

				// 用户 prompt 构建，分首轮/非首轮两种详尽规则
				const userBase =
					accumulated.length === 0
						? `主题/需求：${topic}
难度说明：${levelText}
请恰好生成 ${batch} 条 items（数组长度必须等于 ${batch}）。
要求：本批每条 english 必须各不相同（小写比较无重复）；禁止同一句改标点或微调几个词当作多条；禁止同一出处下机械堆叠、句式实质雷同的多条。`
						: `主题/需求：${topic}（与前几轮相同，请继续围绕该主题扩展）
难度说明：${levelText}
请再生成恰好 ${batch} 条新的 items（数组长度必须等于 ${batch}）。
...
本批 items 内部的 english 也不得彼此重复或雷同。请输出与上述列表完全不同的句子。${diversityHint}`;
				...
				const newItemsThisRound: ClassicQuoteItemDto[] = []; // 本轮新增
				let added = 0; // 本轮添加计数
				let batchItems: ClassicQuoteItemDto[] = []; // LLM本批生成结果
				/**
				 * 首轮（无已采集）最多尝试1次 dupPass（即未重试），后续若检测到高度重复，可尝试5次加急重新生成。
				 */
				const maxDupPasses = accumulated.length === 0 ? 1 : 5;

				// 去重重试小循环，杜绝LLM因 prompt 漏检而 batch 内/间重复
				for (let dupPass = 0; dupPass < maxDupPasses; dupPass++) {
					// 紧急分支 prompt，督促LLM彻底更换方向
					const urgency =
						dupPass === 0
							? ''
					...
					batchItems = [];
					const parseRetries = 3; // JSON解析重试次数（考虑LLM输出不规范）
					for (let att = 0; att < parseRetries; att++) {
						// 每次失败则增加输出tokens上限，以提升生成概率
						const maxTok =
						...
						try {
							...
							const parsed = this.extractJsonObject(text); // 解析出 JSON 对象
							batchItems = this.extractClassicQuoteItemsLoose(parsed); // 提取经典句条目
							// 若支持上下文，将消息记录入线程，裁减过长历史
							if (context?.userId != null && text.trim()) {
								// 人类输入和AI响应分别入栈
								packAgentThread.push(
									new HumanMessage(user),
