...
> **延伸阅读**：[../plugins/模块联邦插件宿主.md](../plugins/模块联邦插件宿主.md)（Host 动态加载）；[../english/学习笔记远程.md](../english/学习笔记远程.md)（学习笔记 Remote）；[上传存储路径.md](./上传存储路径.md)；[Nginx配置.md](./Nginx配置.md)；**第三方任意域名接入**：[../ideas/第三方联邦插件接入.md](../ideas/第三方联邦插件接入.md)。
...
**策略优先级**（详见 [../ideas/第三方联邦插件接入.md](../ideas/第三方联邦插件接入.md)）：
