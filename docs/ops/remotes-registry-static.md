...
**不要**开放代理（`proxy_pass $arg_url`）。对方无法 CORS 时才用固定 upstream 的 `/mf-proxy/<id>/`（见 [nginx.md](./nginx.md)）。
...
1. **推荐**：在 **各 Remote 源站**（如 9008）Nginx 用 `map` 放行 Host + Tauri 壳 origin（详见 [apps/remote-plugins/README.md](../../apps/remote-plugins/README.md)）：
...
**策略优先级**（详见 [../ideas/third-party-mf-plugin-onboarding.md](../ideas/third-party-mf-plugin-onboarding.md)）：
...
| 第三方接入手册 | `docs/ideas/third-party-mf-plugin-onboarding.md`                |
