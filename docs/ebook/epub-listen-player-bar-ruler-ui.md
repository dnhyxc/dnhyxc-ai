...
> **延伸阅读**：[epub-listen-player-bar.md](./epub-listen-player-bar.md)（首版分句菜单与倍速）、[../Influence-point/epub-listen-player-bar-ui.md](../Influence-point/epub-listen-player-bar-ui.md)（影响面）、[ebook-shelf-empty-tab-reset.md](./ebook-shelf-empty-tab-reset.md)（同轮书架改动，独立专题）。
