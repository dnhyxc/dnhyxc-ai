...
**姊妹文档**：[epub-reader-settings-dismiss.md](./epub-reader-settings-dismiss.md)（仅设置面板关闭，本轮扩展为听书菜单）、[epub-reader-chrome-contrast.md](./epub-reader-chrome-contrast.md)（chrome 字色与 Portal 背景）。
...
3. **常挂载 + ref**：避免 iframe 回调注册时序问题；与 [epub-reader-settings-dismiss.md](./epub-reader-settings-dismiss.md) 思路一致并扩展范围。
