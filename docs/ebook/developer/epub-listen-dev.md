...
> **改动追溯**：增量对比见 [epub-chapter-listen.md](../epub-chapter-listen.md)、[epub-listen-bar-playhead-toc.md](../epub-listen-bar-playhead-toc.md)、[epub-toc-cfi-navigate.md](../epub-toc-cfi-navigate.md)、[epub-listen-toc-anchor-start.md](../epub-listen-toc-anchor-start.md) 等；本文以**当前源码**为准自洽。  
...
PDF 听书、英语学习页喇叭、后端 Edge/MiniMax/讯飞服务实现、书架进度百分比展示、阅读设置字色对分句菜单的样式（见 chrome 专题）。播放背景与划线 DOM 深度协调见 [epub-listen-user-highlight-reconcile.md](../epub-listen-user-highlight-reconcile.md) 与 [epub-mark-layers-shared.md](./epub-mark-layers-shared.md)。
