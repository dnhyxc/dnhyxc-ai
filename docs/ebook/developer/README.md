...
| 文档 | 范围 |
|------|------|
| [epub-all-features-dev.md](./epub-all-features-dev.md) | **全功能总手册**：渲染生命周期 + 用户划线 + 读书想法 + 阅读设置 + 章节听读 + 引用听读 + 公开想法增量同步 + 跨功能互斥 + **书架管理**（§15）+ **阅读进度持久化**（§16）+ **分栏阅读**（§17）+ **引用分享**（§18）+ **后端 API 与实体**（§19）+ 复用清单（M1–M12、11 个源码符号逐行注释） |
