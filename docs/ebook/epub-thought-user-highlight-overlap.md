...
**延伸阅读**：[epub-thought-underline-impl.md](./epub-thought-underline-impl.md)（想法虚线主文档）、[epub-user-highlight-impl.md](./epub-user-highlight-impl.md)（用户划线与 blocker）、[epub-thought-partial-overlap.md](./epub-thought-partial-overlap.md)（想法间 patch 去重）。
...
2. **patch 想法间**：`compareThoughtMarksForLineDrawOrder` **短 span 先画**；较长选区后画并向 `thoughtBlockers` 贡献已画线段（延续 [epub-thought-partial-overlap.md](./epub-thought-partial-overlap.md)）。
