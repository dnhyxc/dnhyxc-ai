...
- [epub-annotation-sync-perf.md](./epub-annotation-sync-perf.md) — patch 读 rect 快路径、sync 作用域缓存、想法 DOM restack（本轮 **叠加** 在其上）
- [epub-public-thought-underline-overlay.md](./epub-public-thought-underline-overlay.md) — 公开书叠层 rank + CFI 投影扣减（投影缓存服务于该 patch）
- [ideas/epub-scroll-stutter-perf.md](../ideas/epub-scroll-stutter-perf.md) — 规划态架构图与调试手册
...
[epub-annotation-sync-perf.md](./epub-annotation-sync-perf.md) 已解决「每条 mark 每次 patch 都 CFI→getClientRects」与 sync 阻塞。公开书叠层上线后，**滚动热路径**仍出现：
...
- **功能不变**：划线/想法样式、叠层 spec、点击行为与 [epub-public-thought-underline-overlay.md](./epub-public-thought-underline-overlay.md) 一致。
...
**变更摘要**：sync 开头增加 **投影缓存清空** 与 **想法 DOM invalidate**；新增 `currentUserId` 参数（公开书叠层，见 [epub-public-thought-underline-overlay.md](./epub-public-thought-underline-overlay.md)）。
...
| AC5 | 翻页 / 分栏 resize | 与 [epub-annotation-sync-perf.md](./epub-annotation-sync-perf.md) 回归一致 |
