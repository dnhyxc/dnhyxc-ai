...
> **延伸阅读**：[ebook-membership-upload.md](./ebook-membership-upload.md)（会员闸门与 COS-only）、[ebook-cos-local-shelf.md](./ebook-cos-local-shelf.md)（书架云端备份主链路）。
...
| 会员策略 | 不变，见 [ebook-membership-upload.md](./ebook-membership-upload.md) |
