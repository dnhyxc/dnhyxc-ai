...
**增量专题**：在 [epub-reader-chrome-contrast.md](./epub-reader-chrome-contrast.md) 已贯通顶栏/侧栏/听书菜单等 chrome 的基础上，本轮将 **阅读背景与字色** 延伸到 **选区浮动 PopBar**（Portal 毛玻璃面板、箭头、投影），并调整 **划线样式/颜色顶栏** 的展示条件：**全新选区** 不展示顶栏，**已有用户划线**（含 partial）才展示。

**姊妹文档**：[epub-selection-popbar-visual.md](./epub-selection-popbar-visual.md)、[epub-reader-chrome-contrast.md](./epub-reader-chrome-contrast.md)、[epub-highlight-dom-match.md](./epub-highlight-dom-match.md)、[epub-quote-share-dialog-chrome.md](./epub-quote-share-dialog-chrome.md)。
...
**变更摘要**：新增 PopBar 三件套，供 Panel 与箭头引用；投影与 [epub-selection-popbar-visual.md](./epub-selection-popbar-visual.md) 规则对齐并随阅读背景分流。
