...
- [EPUB标注分层.md](./EPUB标注分层.md) — 用户线 / 想法线 / 播放层三层编排与 `syncEpubReadingAnnotations` 总览
- [EPUB公开想法下划线修复.md](./EPUB公开想法下划线修复.md) — 公开书叠层投影（`thoughtStackProjectionCache` 与滚动 patch 共用）
...
| 想法 **DOM restack** 替代全书 reapply | 听书节间 iframe 续播（见 [EPUB滚动多iframe听书.md](./EPUB滚动多iframe听书.md)） |
