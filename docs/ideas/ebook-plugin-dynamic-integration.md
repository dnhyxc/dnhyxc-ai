...
- 学习笔记 DOCX 导出端到端实现手册（同款「规划 + 实现手册」体裁）：[docs/ideas/learning-notes-docx-export-handbook.md](./learning-notes-docx-export-handbook.md)
...
| 样式隔离（`@scope([data-mf-plugin])`） | Shadow DOM 方案（已否决，见 [mf-css-isolation.md](./mf-css-isolation.md)） |
...
| epub.js 与 plugin Drawer 焦点冲突 | 低 | Drawer 打开时 epub.js selection 仍可能触发 popbar | 阅读页已有 `popbar dismiss` 逻辑（见 [epub-listen-popbar-dismiss.md](../ebook/epub-listen-popbar-dismiss.md)），Drawer 打开会同步 dismiss |
...
按 `feature-implementation-idea` skill 的 [references/self-check.md](../../../.trae-cn/skills/feature-implementation-idea/references/self-check.md) 逐项核对：

- [x] 落盘路径 `docs/ideas/ebook-plugin-dynamic-integration.md`（kebab-case）
- [x] 文首含「状态 / 日期 / 需求摘要」
- [x] §0 读本文你将得到什么（5 条要点）
