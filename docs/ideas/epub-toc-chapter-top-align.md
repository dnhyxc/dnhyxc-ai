...
- 既有实现专题：[epub-scrolled-toc-nav-align.md](../ebook/epub-scrolled-toc-nav-align.md)（连续滚动 `displayEpubScrolledHref` 初版，固定 0/100/220ms 轮询）
- 滚动模式边界：[epub-reader-settings-scroll.md](../ebook/epub-reader-settings-scroll.md)
- 目录高亮：[ebook-toc-active-highlight.md](../ebook/ebook-toc-active-highlight.md)
- 滚动卡顿（relocated patch 时序）：[epub-scroll-stutter-perf.md](../ebook/epub-scroll-stutter-perf.md)
...
| 后端 | 无 |
| 文档（实现后） | 更新 `docs/ebook/epub-scrolled-toc-nav-align.md`（标注升级为稳定性循环）；新增 `docs/ebook/epub-paginated-toc-top-align.md` |
