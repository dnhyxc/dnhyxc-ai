...
> 延伸阅读：放大后正文贴左、需刷新才居中的**前端**重排问题见 [ebook/epub-window-resize-relayout.md](../ebook/epub-window-resize-relayout.md)（EPUB 侧栏/分栏 resize 后的视口重定位）。本文聚焦**窗口壳与页面尺寸异步**这一桌面层根因。
...
- [ ] 放大后切换 EPUB 章节 / 打开侧栏，布局**正常**（与 [epub-window-resize-relayout.md](../ebook/epub-window-resize-relayout.md) 不冲突）。
...
- 前端层放大后正文重定位：[ebook/epub-window-resize-relayout.md](../ebook/epub-window-resize-relayout.md)
- 拖拽分栏白屏 / 彩色划线消失：[ebook/epub-split-soft-resize.md](../ebook/epub-split-soft-resize.md)
