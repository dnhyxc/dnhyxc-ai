...
- 第三方 MF 插件接入（CORS / capabilities / registry 上架）：[docs/ideas/第三方联邦插件接入.md](./第三方联邦插件接入.md)
- MF 主子样式互不影响：[docs/ideas/模块联邦CSS隔离.md](./模块联邦CSS隔离.md)
- 插件开发模板手册：[apps/remote-plugins/plugin-info.md](../plugins/插件图标系统.md)
- 学习笔记 DOCX 导出端到端实现手册（同款「规划 + 实现手册」体裁）：[docs/ideas/学习笔记DOCX导出手册.md](./学习笔记DOCX导出手册.md)
...
| 样式隔离（`@scope([data-mf-plugin])`） | Shadow DOM 方案（已否决，见 [模块联邦CSS隔离.md](./模块联邦CSS隔离.md)） |
