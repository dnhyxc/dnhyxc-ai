...
> **延伸阅读**：[plugin-entry-cache-bust.md](./plugin-entry-cache-bust.md)（bust token 生成与 afterResolve 补 `remoteEntry.js?v=`）；[mf-plugin-host.md](./mf-plugin-host.md)（Host 侧插件生命周期）；[dynamic-plugin-system.md](./dynamic-plugin-system.md)（Vite MF 配置与动态路由注入）。
