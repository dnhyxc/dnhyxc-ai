...
| [tauri-fullscreen-sync.md](./tauri-fullscreen-sync.md) | **Tauri 全屏状态同步（macOS 绿钮/菜单零延迟）**：`fullscreen_watch.rs` 注册 `NSWindowWillExitFullScreenNotification`、Rust 发 `host://will-exit-fullscreen` 事件、前端 `installAppFullscreenExitSync()` 三源统一（will-exit/Resized/document）+ `flushSync` 同步刷新 + `ignoreNativeUntil` 防抖（含改动前/后对比与逐行注释） |
...
| [selection-drag-shell-scroll-guide.md](./selection-drag-shell-scroll-guide.md) | **助手拖选移出窗口整页上移**：`#root` fixed 钉视口、`html/body` 与 Layout `overflow-clip`、ScrollArea `overscroll-behavior: contain`；弃用 JS 钉 `scrollTop`（功能实现详解与复刻指南） |
| [build-optimization.md](./build-optimization.md) | **前端打包优化**：路由 `React.lazy` + `manualChunks` + mermaid 动态 `import()`（含 **§4.6.1** 动态加载后模块形态修复，勿让消费方再装 mermaid）+ CSS 显式引入 + Monaco/Prettier 懒加载 + barrel 瘦身 |
| [plugin-host-error-actions.md](./plugin-host-error-actions.md) | **插件宿主错误态新增操作**：`PluginHostPage` 卡片错误态除「重新加载」外新增「返回首页」「插件开发指南」按钮，三按钮并排居中加图标，提升错误态可恢复性与导流（含改动前/后对比与逐行注释） |
| [ui-tint-polish.md](./ui-tint-polish.md) | **UI 配色透明度调优**：首页 `HUE_STYLES` 五色相 `icon`/`btn` 渐变加 `/80` `/90` 透明度、CTA 按钮公共类提取；全局 `Button` default/outline variant 配色微调；`ScrollFab` 尺寸/圆角调整（`h-8 w-8 rounded-md`，english 变体 `h-5 w-8 rounded-sm`）；`DropdownMenuContent` 视口内边距 `p-1` → `p-2`（含改动前/后对比与逐行注释） |
| [tooltip-controlled-align.md](./tooltip-controlled-align.md) | **Tooltip 受控模式与对齐**：`TooltipSide` 新增 `align`（'start'/'center'/'end'）、`open`、`onOpenChange` 三个可选 prop，透传 Radix `TooltipContent.align` 与 `Tooltip Root.open/onOpenChange`；不传时行为同改前（含改动前/后对比与逐行注释） |
| [monaco-loading-restore.md](./monaco-loading-restore.md) | **Monaco loading 恢复与 Suspense 占位背景**：`MarkdownEditorProps` 新增 `loading?: React.ReactNode \| null`（默认 null），4 处 `loading={null}` 改为 `loading={loading ? loading : null}`；`Suspense` fallback div 追加 `bg-theme/5` 微透明背景（含改动前/后对比与逐行注释） |
