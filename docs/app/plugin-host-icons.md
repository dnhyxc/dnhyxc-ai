...
> **延伸阅读**：[Host 接入总览](./mf-plugins/host-plugin-integration-guide.md)、[Ebook Host Surface](./ebook-host-surface-plugins.md)、[动态插件系统](./dynamic-plugin-system.md)。旧文中「`menu.icon` / `host.icon` = Lucide 名 + Host `ICON_MAP` / `DEFAULT_PLUGIN_HOST_ICONS` 白名单」已由本方案取代（静态侧栏 `MENUS`/`PLUGINS` 仍用 `ICON_MAP`）。
>
