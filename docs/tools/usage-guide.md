...
桌面端将 `openExternalUrl` 换成 Tauri `plugin-opener` 即可（见 [`frontend/tauri-browser.md`](../tauri/tauri-browser.md)）。本仓库等价实现：`apps/frontend/src/utils/external-link-click.ts`、`open-external.ts`，在 `ChatAssistantMessage`、`Monaco/preview` 已接入。
