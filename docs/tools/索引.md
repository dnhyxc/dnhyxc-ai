...
| 路径                                      | 角色                                                                                                                    |
| ----------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| `src/markdown/parser.ts`                  | `MarkdownIt` + `markdown-it-katex` + **`markdown-it-task-lists`（GFM 待办）** + 自研「纯 `[x]`/`[ ]`」补丁 + **外链 `target="_blank"`** + `highlight.js` 高亮；可选围栏工具栏；构造末尾调用主题注入。详见 [外链新标签打开.md](./外链新标签打开.md)。 |
