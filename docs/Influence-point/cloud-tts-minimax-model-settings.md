...
- [cloud-tts-user-credentials-fallback.md](./cloud-tts-user-credentials-fallback.md) — 云端凭证、降级链与 MP3 缓存 key
- [../english/cloud-tts-minimax-model-settings.md](../english/cloud-tts-minimax-model-settings.md) — **实现说明**（改动前后代码与逐行注释）
- [minimax-cloud-tts.md](../english/minimax-cloud-tts.md) — MiniMax T2A 全链路（文档中 8 项 model 白名单描述可能滞后，见 §7）
- [cloud-tts-settings.md](../english/cloud-tts-settings.md) — 设置页结构与偏好字段
...
| 文档仍写 8 项 model 或默认 hd | 低 | `minimax-cloud-tts.md` 等可能滞后 | 见 §7；发布前扫一遍 english 域文档 |
...
| 文档 | 说明 |
|------|------|
| `docs/english/minimax-cloud-tts.md` | 仍可能列举 8 项 model、默认 hd；需与 `minimax-tts-models.ts` 对齐 |
| `docs/english/cloud-tts-settings.md` | 模型字段若仍描述为纯文本框，需改为 Combobox + 两项预设 |
| `docs/Influence-point/cloud-tts-user-credentials-fallback.md` | §1 表「MiniMax 模型校验为任意字符串」已过时，应改为 `@IsIn` 两项 |
