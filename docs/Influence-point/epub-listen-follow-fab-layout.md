...
- [epub-listen-auto-follow-fab.md](../ebook/epub-listen-auto-follow-fab.md) — `autoFollow`、用户滚动打断与 FAB 原实现
- [epub-window-resize-relayout.md](./epub-window-resize-relayout.md) — 同批 `EpubPane.applyHostResize` 中的窗口/分栏 resize 与 view relayout
- [epub-listen-resize-relayout.md](./epub-listen-resize-relayout.md) — 播放背景 `relayoutListenMarkHighlight` 接线
- [epub-quote-listen-player-bar.md](./epub-quote-listen-player-bar.md) — 听书播放条与听当前互斥
...
| **听书播放中窗口放大/全屏** | 中（体验） | 同上，与 [epub-window-resize-relayout](./epub-window-resize-relayout.md) 同帧触发 |
...
| 文档 | 说明 |
|------|------|
| `docs/ebook/epub-listen-auto-follow-fab.md` | 仍写「仅用户滚动打断」；需补 **布局变化离屏** 与 `checkEpubListenFollowAfterLayout` |
| `docs/ebook/developer/epub-listen-dev.md` | FAB 验收表可增「分栏/窗口 resize 离屏」 |
| `docs/ebook/developer/epub-mark-layers-shared.md` | `isDomRangeInReaderView` 已改名为 `isEpubRangeInReaderView` 并导出 |
| `docs/Influence-point/epub-window-resize-relayout.md` | 本篇描述 Follow FAB 逻辑；该文侧重 view relayout，二者同调 `applyHostResize` |
