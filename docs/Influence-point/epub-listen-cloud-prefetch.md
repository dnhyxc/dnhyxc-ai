...
- [epub-listen-dev.md](../ebook/developer/epub-listen-dev.md) — 听书 / 听当前总手册（逐句 `playEnglishPreferred` 循环）
...
评估 **听书 / 听当前逐句播放时的云端 TTS 句间预取**（`prefetchCloudEnglishTts` + `prefetchedCloud`）是否破坏或意外改变已有功能。改前每播完一句才发起下一句云端合成，句间等待网络 + 收 MP3；改后在播第 N 句时并行预取第 N+1 句（首句开播前即预取第二句）。

**对照的既有能力**：

- **听书**（`useEpubChapterListen`）：章内逐句循环、`playEnglishPreferred`、播放条切句 / 暂停 / 倍速
- **听当前**（`useEbookQuoteListen`）：选区逐句循环、共用 `EpubListenPlayerBar`
- **English TTS 公共 API**（`playEnglishPreferred`）：PopBar 单词、英语学习各页、设置页试听
...
| 文件 | 变更 |
|------|------|
| `apps/frontend/src/utils/englishTts.ts` | 新增 `prefetchCloudEnglishTts`、`EnglishTtsSentencePrefetch`；`PlayEnglishPreferredOptions.prefetchedCloud`；`resolveCloudTtsReady` 替代首段 `startCloudTts` |
...
| 维度 | 是否影响原有功能 | 说明 |
|------|------------------|------|
| `playEnglishPreferred` 对外签名 | **否** | 仅增可选 `prefetchedCloud`；未传时行为与改前一致 |
| 听书 / 听当前逐句播放语义 | **否** | 仍一句一调；分句索引、高亮、暂停 / 切句逻辑未改 |
| 句间等待（云端会员） | **低（增强）** | 预取命中时句间空白缩短；听感更连贯 |
| 本机 Web Speech 路径 | **否** | `prefetchCloudEnglishTts` 返回 `null`；不增选项时零差异 |
...
| 播放背景 DOM 时序 | **否** | 高亮仍在 `playEnglishPreferred` 前后，未因预取提前 |
...
for 每句 si:
  showHighlight(si)
  await playEnglishPreferred(spokenRaw)   // 句末才开始下一句 startCloudTts
  clearHighlight(si)
```
...
| **本机音色 / 非会员** | 无 | `shouldUseCloudEnglishTts` 为 false 时 `prefetchCloudEnglishTts` 返回 null，Map 存 null 等价于未预取 |
| **设置页 `preferLocal` 试听** | 无 | 未传 `prefetchedCloud` |
| **英语学习各页喇叭** | 无 | 单次 `playEnglishPreferred`，无逐句 Map |
...
| 停止后多余云端请求 | 低 | 已发起的 `prefetchCloudEnglishTts` 无法取消，仅写缓存 | 快速连点停止；观察网络面板无播放仍请求属预期 |
...
| `playEnglishPreferred` 调用方（除两 hook） | 可选字段默认 undefined |
