...
- [miniprogram-epub-parse-logic.md](../ideas/miniprogram-epub-parse-logic.md) — 解析编排、API 与小程序消费链路（规划/已落地说明）
- [miniprogram-epub-server-parse.md](../ebook/miniprogram-epub-server-parse.md) — **实现归档**：BullMQ、`startParseTask`/`waitForParse` 改动前后对比与逐行注释
- [ebook-cos-object-key-resolve.md](../ebook/ebook-cos-object-key-resolve.md) — `resolveCosObjectKey` / `uploadEbookAssetBuffer` 动机与调用点
- [ebook-reading-progress-save.md](../ideas/ebook-reading-progress-save.md) — Web 端 CFI / percent 进度保存（未改调用方）
- [ebook-progress-remote-debounce.md](./ebook-progress-remote-debounce.md) — Web 进度 PUT 防抖与 keepalive
...
| 文档 | 说明 |
|------|------|
| `docs/ideas/miniprogram-epub-parse-logic.md` | 已同步 BullMQ、`waitForParse` ready 快路径、已修复坑 |
| `docs/ebook/ebook-cos-object-key-resolve.md` | 写明 `getObjectBuffer` 走 resolve；**未**说明 `pipeObjectToWritable` 未走 resolve |
