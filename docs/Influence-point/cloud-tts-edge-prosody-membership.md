...
| 文档 | 说明 |
|------|------|
| `docs/english/cloud-tts-settings.md` | 仍描述「三选一」「会员才云端」；缺 Edge 区块顺序与分模式字段 |
| `docs/english/tts-playback-source.md` | 未含 `playbackSource: edge` 与非会员 Edge |
| `docs/ideas/xfyun-cloud-tts.md` | 状态机仍为 local/cloud/xfyun 三值 |
| `docs/Influence-point/cloud-tts-user-credentials-fallback.md` | §1 仍写「三选一」；延伸阅读可链本篇 |
| `docs/english/xfyun-cloud-tts.md` | 若存在共用 `speed` 映射描述，需改为独立 `xfyunSpeed` 等 |
