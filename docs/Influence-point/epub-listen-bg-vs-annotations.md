...
| # | 曾出现问题 | 根因 | 现行规避 |
|---|-------------|------|----------|
| D1 | 听完后用户线消失、删不掉 | 播放清除对 **任意 CFI** `annotations.remove(cfi,'highlight')` | 仅删 `moke-epub-listen` class；详见 [epub-listen-user-highlight-reconcile.md](../ebook/epub-listen-user-highlight-reconcile.md) |
...
| 历史 reconcile 专题 | `docs/ebook/epub-listen-user-highlight-reconcile.md` |
| 开发者总手册 | `docs/ebook/developer/epub-listen-dev.md` |
