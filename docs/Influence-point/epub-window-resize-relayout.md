...
| `docs/Influence-point/epub-listen-resize-relayout.md` | 仍描述 `relayoutListenMarkHighlight` 接线；本篇补充 **view relayout** 与 **window.resize** 对同一 `applyHostResize` 的叠加影响 |
| `docs/Influence-point/epub-listen-follow-fab-layout.md` | 布局离屏触发 FAB 的专项影响面；与本篇共享 `applyHostResize` 调用点 |
