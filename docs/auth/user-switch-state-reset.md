...
> **文档角色（主文档）**：说明登出、切换账号、401 失效时如何统一清空与用户绑定的 MobX 内存态；与 [route-auth.md](../auth/route-auth.md)（鉴权收口）互补，后者侧重 token 与路由跳转。

**延伸阅读**

- [route-auth.md](../auth/route-auth.md) — 路由守卫与 `notifyUnauthorized` 两层鉴权
...
- **本机朗读音色**（`local_tts_voice:{userId}`）— 按账号分键保存在 localStorage，换号时**不**删除；见 [`../english/english-tts-local-voice.md`](../english/english-tts-local-voice.md)
...
4. **循环依赖修复**  
   ...
   Store 链上模块改用 `loggedInUserId` / `membershipActive` 纯函数，详见 [membership-store-circular-deps.md](../auth/membership-store-circular-deps.md)。
