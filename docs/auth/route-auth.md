...
- **同步**删除 `localStorage` 的 `token`、`userInfo`，调用 **`resetUserState()`** 清空与用户绑定的前端 MobX 缓存（知识库草稿、助手对话、英语 Agent 等，详见 [user-switch-state-reset.md](../auth/user-switch-state-reset.md)），并 `dispatchEvent('userInfoChanged')`，保证与 `useStorageInfo` 等监听一致；**避免**先 `replace` 再异步清理导致清理未执行完。
...
2. **前端侧**：Web 生产与 **本地开发**（`import.meta.env.DEV`，含 Tauri dev）将以 `VITE_COS_PUBLIC_DOMAIN` 匹配的 URL 改写为 `/ext-cos/...`；**Tauri 生产包**使用原始 COS HTTPS URL（allowlist）。详见 [cos-dev-http-proxy.md](../cos/cos-dev-http-proxy.md)。
...
实现细节、Vite 开发代理与 Tauri ATS → [cos-dev-http-proxy.md](../cos/cos-dev-http-proxy.md)。
...
源码与注释见 [cos-dev-http-proxy.md](../cos/cos-dev-http-proxy.md) §5.1。
...
- **可维护性**：域名与环境变量集中在 `resolveCosUrlForWebDisplay`；换桶见 [cos-dev-http-proxy.md](../cos/cos-dev-http-proxy.md) §8。
