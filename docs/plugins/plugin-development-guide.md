...
| `optimizeDeps.include` | 推荐 | 预打包重依赖（如 `@tiptap/*`），避免 HMR 二次 reload；见 [remote-plugin-hmr.md](../plugins/remote-plugin-hmr.md) |
...
> **HMR**：禁止与组件同文件 `export function activate/deactivate`（会整页刷新）。推荐挂在 default 静态属性上；旧写法拆 `lifecycle.ts` 仍兼容。详见 [remote-plugin-hmr.md](../plugins/remote-plugin-hmr.md)。
...
- [mf-implementation-guide.md](../plugins/mf-implementation-guide.md)：实现过程文档
- [host-plugin-integration-guide.md](../plugins/host-plugin-integration-guide.md)：主项目接入手册
