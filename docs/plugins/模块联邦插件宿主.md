...
| 样式隔离     | 生产者 scoped CSS（见 [模块联邦CSS隔离.md](../ideas/模块联邦CSS隔离.md)）；untrusted → iframe | Host 劫持 head + Shadow          |
...
详见 `apps/frontend/src/vite-env.d.ts`。第三方接入步骤见 [../ideas/第三方联邦插件接入.md](../ideas/第三方联邦插件接入.md)。
