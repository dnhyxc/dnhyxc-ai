...
> 已从单文件巨石 `host/styleIsolation.ts` **按 qiankun experimentalStyleIsolation 分层拆分**；公开 API 与行为契约不变（协议仍为 **`/*mf-iso:3*/`**）。逐行实现以源码为准；历史 `@scope` / 巨石全文摘录见姊妹稿 [`style-isolation-qiankun-harden.md`../../../app/style-isolation-qiankun-harden.md)。
...
> 仓库归档副本：[`docs/app/plugin-entry-cache-bust.md`../../../app/plugin-entry-cache-bust.md)（与本节同步维护；若冲突以源码与该归档文为准）。
