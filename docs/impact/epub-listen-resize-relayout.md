...
- [epub-listen-bg-resize-relayout.md](../ebook/epub-listen-bg-resize-relayout.md) — **实现思路**与改动前后代码对比
...
| # | 影响点 | 表现 |
|---|--------|------|
| C1 | 播放中同区域用户色块 / 想法虚线 | 与 [epub-listen-bg-vs-annotations](./epub-listen-bg-vs-annotations.md) 相同：淡黄底 **临时叠层**，不删 DOM |
...
**与批注层关系**：与用户划线、想法虚线 **仍三层分离**（详见 [epub-listen-bg-vs-annotations.md](./epub-listen-bg-vs-annotations.md)）。本次改动 **只增加 listen 层在 resize 后的重绘频率与 group 重新解析**，不扩大 purge 范围。
...
| 播放层 vs 批注隔离 | [epub-listen-bg-vs-annotations.md](./epub-listen-bg-vs-annotations.md) |
