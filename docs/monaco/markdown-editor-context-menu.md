...
6. **与知识库助手输入框联动（选区→输入框）**  
   ...
   - 知识库页对右键「复制选中内容到助手」可能出现的**同一次操作重复追加**，另有宿主层合并与去重说明，见 **`docs/knowledge/knowledge-editor-send-selection-to-assistant-dedupe.md`**。  
   - 复制到助手后的**自动聚焦**（Radix `onCloseAutoFocus` 拦截 + `focusInputAtEndKey`）见 **`docs/knowledge/assistant-insert-focus.md`**。
