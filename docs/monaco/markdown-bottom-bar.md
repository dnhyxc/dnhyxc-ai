...
- **与按钮同路径**：快捷键命中后调用与 **`onClick={resetBarPosition}`** 相同的 `resetBarPosition`（通过 hook 入参 **`resetMarkdownBottomBarPosition`** 传入），不重复实现 snap。
- **与其它操作栏 chord 一致**：`window` **捕获阶段** `keydown`；命中后 **`preventDefault` + `stopPropagation`**，避免浏览器将 **`Meta + -`** 当作页面缩放等默认行为。
- **hook 注册顺序**：`MarkdownBottomBar` 内须先 **`useCallback` 定义 `resetBarPosition`**，再调用 **`useMarkdownBottomBarShortcuts({ resetMarkdownBottomBarPosition: resetBarPosition })`**，保证传入稳定回调。
...
// 入参：由 MarkdownBottomBar 传入，语义上等于「复位按钮 onClick」
resetMarkdownBottomBarPosition: () => void;

// …在 markdownBarAction0 分支之后：

if (hit(chords.markdownBarResetPosition)) {
	// 拦截默认：避免 Meta + - 触发浏览器缩小页面等
	e.preventDefault();
	e.stopPropagation();
	// 与 §9.9.2 按钮一致：setDragOffset(0) + rAF snap
