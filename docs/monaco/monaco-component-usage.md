...
export type ShortcutSource = {
  ...
  loadChords: () => Promise<Partial<MarkdownBottomBarChords> | null | undefined>; // 2) 加载 chord：可部分覆盖
...
const shortcutSource = useMemo<ShortcutSource>(() => ({
  defaultChords: { /* ...使用 KNOWLEDGE_SHORTCUT_DEFAULT_CHORDS ... */ },
  loadChords: async () => {
    const c = await loadKnowledgeShortcutChords();
    return { /* ...映射为底部栏 chords... */ };
  },
  subscribeChordsChanged: (onChange) => {
    const handler = () => onChange();
    window.addEventListener(KNOWLEDGE_SHORTCUTS_CHANGED_EVENT, handler);
    return () => window.removeEventListener(KNOWLEDGE_SHORTCUTS_CHANGED_EVENT, handler);
  },
