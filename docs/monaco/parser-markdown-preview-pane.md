...
> 延伸阅读：滚动层完整根因表与流式贴底合并见 [../knowledge/knowledge-preview-scroll-jank.md](../knowledge/knowledge-preview-scroll-jank.md)；编辑↔预览滚动恢复见 [markdown-preview-edit-scroll-restore.md](./markdown-preview-edit-scroll-restore.md)；TOC/hash 跳转见 [markdown-preview-toc-hash-navigation.md](./markdown-preview-toc-hash-navigation.md)。
...
本文聚焦该组件本身的实现归档（含未在 diff 中变动、但属于本轮功能闭环的既有结构，以纯新增形式摘录）。`useChatCodeFloatingToolbar` hook 内部 `enabled` 门禁与 scroll rAF 合并、`useStickToBottomScroll` 流式贴底合并由 [../knowledge/knowledge-preview-scroll-jank.md](../knowledge/knowledge-preview-scroll-jank.md) 主文档负责，本文不重复粘贴。
...
**变更摘要**：始终传 options 对象，新增 `enabled` 字段；`enableCodeFloatingToolbar=false` 时 hook 内部所有 effect 早 return（hook 内部改造见 [../knowledge/knowledge-preview-scroll-jank.md](../knowledge/knowledge-preview-scroll-jank.md) §4.3）。
...
| 滚动层完整根因表与流式贴底合并（主文档） | `docs/knowledge/knowledge-preview-scroll-jank.md` |
| 编辑↔预览滚动恢复 | `docs/monaco/markdown-preview-edit-scroll-restore.md` |
| TOC / hash 跳转 | `docs/monaco/markdown-preview-toc-hash-navigation.md` |
