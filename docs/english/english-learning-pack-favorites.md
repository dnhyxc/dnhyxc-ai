...
- **单词朗读**：`VocabularyPackList` 等对单词调用 `playPreferred(word, { preferLocal: true })`（本机 Web Speech 优先）；经典句仍默认云端 TTS。播放竞态与世代号见 [`english-tts-playback.md`](./english-tts-playback.md)。
