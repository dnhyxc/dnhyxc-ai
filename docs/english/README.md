...
| [rich-editor-doc-edge-nav.md](./rich-editor-doc-edge-nav.md) | **Cmd/Ctrl+↑↓ 文档首尾导航**：新增 `DocEdgeNav` 扩展与 `scrollEditorViewport` 辅助函数、`TitleNode.appendTransaction` 选区纠正从 `Selection.atEnd` 改为 `TextSelection.near`（含改动前/后对比与逐行注释） |
