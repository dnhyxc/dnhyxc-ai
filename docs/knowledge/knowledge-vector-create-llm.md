...
> **完整请求 URL、分片档位与 400/404 修复**见主文档 [siliconflow-vector-full-url.md](./siliconflow-vector-full-url.md)（`baseURL` = 完整 URL，已无 `endpoint` 拼接）。  
> **会员双库 / Qwen3 档位**见 [knowledge-member-vector-tier.md](./knowledge-member-vector-tier.md)。  
...
- 曾手动拼接 `endpoint = baseURL + '/embeddings' | '/rerank'`（**已废弃**，见 [siliconflow-vector-full-url.md](./siliconflow-vector-full-url.md)）
...
**说明**：`SILICONFLOW_BASE_URL` 仅用于对话 Chat，**不再**与 `/embeddings` 拼接；详见 [siliconflow-vector-full-url.md](./siliconflow-vector-full-url.md)。
...
> **历史摘录（已过时）**：以下代码展示旧版 `endpoint = baseURL + path` 逻辑；当前实现见 [siliconflow-vector-full-url.md](./siliconflow-vector-full-url.md) §4.2。
