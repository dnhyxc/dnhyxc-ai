...
操作栏显隐由 **`markdownBottomBarOpen`** 等控制；**拖动偏移**与 **当前位置淡入淡出（opacity）** 均在 `MarkdownBottomBar` 内实现，不改变快捷键 chord 的读取与匹配逻辑。拖动边界相对 `MarkdownEditor` 的 **`rootRef`** 夹紧；收起时需结束进行中的 pointer 拖动，避免 `open=false` 后仍写入 `dragOffset`。**复位按钮**将 `dragOffset` 归零并复用水平 snap，与快捷键无关。完整思路与代码节选见 [`docs/monaco/markdown-bottom-bar.md` 第 9 节](../monaco/markdown-bottom-bar.md)（含 **§9.9 复位**）。
