...
- 写入 AI / RAG 双输入框：`knowledge-assistant-insert-selection-ai-rag.md`
- rAF 合并与重复写入防护：`knowledge-editor-send-selection-to-assistant-dedupe.md`
- Monaco 右键菜单与快捷键同源：`docs/monaco/markdown-editor-context-menu.md`
- 电子书 MOKE 问书同类聚焦机制（`focusInputAtEndKey`）：`../ebook/ebook-moke-assistant.md` §3.7、`../ebook/` 阅读页助手
...
6. **保留既有 rAF 合并 + 160ms 去重**  
   聚焦改动不替换 `knowledge-editor-send-selection-to-assistant-dedupe.md` 中的写入防护；`onInsertSelectionToAssistant` 仍 `setTimeout(0)` 再 `requestAnimationFrame(flush)`，与菜单关闭时序解耦。
...
- **AI / RAG 模式**：写入与聚焦均走当前模式对应输入框（见 `knowledge-assistant-insert-selection-ai-rag.md`）。
