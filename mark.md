...
1. 根据当前 chatId 记录当前 finishReason，为什么要根据 chatId 记录，因为可能在多个 sessionId 所在的会话发起消息，因此，要根据 chatId 分别记录，或者你可以选择更好的，记录方式。finishReason 包含了大模型停止的原因等字段如： chunkType，根据我给你提供的文件，选择最正确的数据的存储方式，以及数据存放的位置，但是你要保证，我在 ChatAssistantMessage 组件中能够实时动态获取到这个 finishReason 数据，要根据这个数据状态实时渲染【点击接着回答】的按钮。不能出现第一条消息不出现该按钮，第二条消息就出现，决不允许出现这种状态设置不正常的情况。

2. 在当前 sessionId 下的对话中的同一分支下发送消息，如 A 用户消息 -> B1 AI 回复消息，B2 是 A 用户消息的第二个 AI 回复 消息，因为用户点击了重新生成，让 A 消息有了多个回复内容，就产生了分支，它们的 parentId 都指向 A，那么 B1 和 B2 就是不同的分支，当 B1 出现【点击接着回答】按钮时，只有在 B1 下方发送新消息时，需要将 B1 的 finishReason 清空，如果在 B2 下方发送新消息时，则不需要清空 B1 的 finishReason，因为 B2 和 B1 是两个不同的分支。注意：你需要在最合适的地方清空 finishReason。

3. 完成 onContinueAnswering 接着回答的逻辑功能。这里在执行这个方法时，也要清空对应 message 的 finishReason。

## 继续生成

1. 根据当前 chatId 记录当前停止的消息，为什么要根据 chatId 记录，因为可能在多个 sessionId 所在的会话发起消息，因此，要根据 chatId 分别记录，或者你可以选择更好的，记录方式。根据我给你提供的文件，选择最正确的数据的存储方式，以及数据存放的位置，但是你要保证，我在 ChatAssistantMessage 组件中能够实时动态获取到这个停止状态的数据，要根据这个数据状态实时渲染【继续生成】的按钮。不能出现第一条消息不出现该按钮，第二条消息就出现，决不允许出现这种状态设置不正常的情况。

2. 在当前 sessionId 下的对话中的同一分支下发送新消息，如 A 用户消息 -> B1 AI 回复消息，B2 是 A 用户消息的第二个 AI 回复消息，因为用户点击了重新生成，让 A 消息有了多个回复内容，就产生了分支，它们的 parentId 都指向 A，那么 B1 和 B2 就是不同的分支，当 B1 出现【继续生成】按钮时，只有在 B1 下方发送新消息时，需要将 B1 的继续生成按钮的状态清空，如果在 B2 下方发送新消息时，则不需要清空 B1 的继续生成按钮控制，因为 B2 和 B1 是两个不同的分支。注意：你需要在最合适的地方清空继续生成按钮的状态。

3. 你要保证，用户在点击【继续生成】按钮时，会清空对应 message 的继续生成按钮的状，同时还要确保存在继续生成按钮的消息还能点击继续生成按钮成功调用接口继续生成。

注意：代码修改完成之后，通过 markodwn 将所有改动文件的完整代码给出来。
