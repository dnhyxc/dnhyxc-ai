...
本次改动新增 8 个迁移文件，其中 4 个是空迁移（占位），4 个是实际建表 / 加列：
...
| `1789572471457-skill-try-message-applied-skills` | 占位 | 空 |
| `1789572476968-skill-try-message-applied-skills` | `skill_try_messages` 加 `applied_skills` 列 | 实际 |
...
### 4. `1789572476968-skill-try-message-applied-skills.ts`（实际加列）

#### 改动原因
`skill_try_messages` 表加 `applied_skills` json 列，让 Skill 试跑 / 生成场景的助手消息也能携带已应用 Skill 快照，与 `assistant_messages` 表对齐。

#### 改动前
`skill_try_messages` 无 `applied_skills` 列，试跑时 Skill 快照只在 SSE 推送给前端，不落库。

#### 改动后
```ts
// 导入 TypeORM 的迁移接口 MigrationInterface 和查询执行器 QueryRunner
import { MigrationInterface, QueryRunner } from "typeorm";

// 导出迁移类 SkillTryMessageAppliedSkills1789572476968，实现 MigrationInterface 接口
// 时间戳 1789572476968 标识此迁移在迁移链中的执行顺序，晚于 assistant 的 applied_skills 迁移
export class SkillTryMessageAppliedSkills1789572476968 implements MigrationInterface {
	...
	// up 方法：正向迁移，给 skill_try_messages 表新增 applied_skills 列
	// 参数 queryRunner 为 SQL 执行器，返回 Promise<void> 异步执行
	public async up(queryRunner: QueryRunner): Promise<void> {
		// 给 skill_try_messages 表新增 applied_skills 列，类型为 json，允许为 NULL
		// 存储试跑/生成场景下助手回复已应用的 Skill 快照数组，格式为 [{ id, title }]
		// 与 assistant_messages.applied_skills 列保持一致，方便统一展示「已应用 Skill」标签
		// 设为可空是为了兼容历史试跑消息数据，避免迁移时旧行因无值而报错
		await queryRunner.query(`ALTER TABLE \`skill_try_messages\` ADD \`applied_skills\` json NULL`);
	}

	// down 方法：回滚迁移，删除 skill_try_messages 表的 applied_skills 列
	public async down(queryRunner: QueryRunner): Promise<void> {
		// 从 skill_try_messages 表删除 applied_skills 列，将表结构恢复到迁移前的状态
...
#### 改动说明
- `applied_skills` 是可空 json 列，存 `[{ id, title }]`，与 `SkillTryMessage.appliedSkills` 实体字段对应。
- 此迁移让 Skill 试跑 / 生成的助手消息也能持久化 Skill 快照，流崩溃重启后仍可展示「已应用 Skill」标签。
- 与 `assistant_messages.applied_skills` 列语义完全一致，便于前端统一处理。

---

### 5. 四个空迁移

`1789406044043-agent-business.ts`、`1789480821731-assistant-session-summary.ts`、`1789485482173-assistant-message-applied-skills.ts`、`1789572471457-skill-try-message-applied-skills.ts` 的 `up` / `down` 为空实现。
...
graph TD
    ...
    subgraph SkillTry["Skill 试跑 / 生成（skill_try_*）"]
        ...
        STM["<b>skill_try_messages</b> 消息表<br/>━━━━━━━━━━━━━━━━━<br/>• session_id FK → skill_try_sessions<br/>• role / turn_id / content<br/>• search_organic<br/>• applied_skills：已应用 Skill 快照"]
...
5. **`applied_skills` 在 assistant 与 skill_try 表**：`assistant_messages` 和 `skill_try_messages` 都有 `applied_skills` json 列；`english_agent_messages` / `agent_messages` 无此列。
